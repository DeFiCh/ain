...
class TokensMintingTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        # Not apllied tx
        assert_equal(
            self.nodes[0].isappliedcustomtx(
        ...
        self.nodes[0].sendmany(
            "", {collateralGold: 1, collateralSilver: 1, collateralCupper: 1}
        )
        self.nodes[0].generate(1)
        self.nodes[0].sendmany(
            "", {collateralGold: 1, collateralSilver: 1, collateralCupper: 1}
        ...
        self.nodes[0].minttokens(
            ["300@" + symbolGold, "3000@" + symbolSilver, "500@" + symbolCupper]
        )
        self.nodes[0].generate(1)
        self.sync_blocks()
        ...
        assert_equal(
            self.nodes[0].getaccount(collateralSilver, {}, True)[idSilver], 3000
        )
        assert_equal(
            self.nodes[0].getaccount(collateralCupper, {}, True)[idCupper], 500
        )
        ...
        assert_equal(
            self.nodes[0].gettoken(symbolGold)[idGold]["collateralAddress"],
            collateralGold,
        )
        assert_equal(
            self.nodes[0].gettoken(symbolSilver)[idSilver]["collateralAddress"],
            collateralSilver,
        )
