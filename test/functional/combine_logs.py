#!/usr/bin/env python3
"""Combine logs from multiple defi nodes as well as the test_framework log.
...
def read_logs(tmp_dir):
    ...
    if chain:
        ...
        chain = re.search(r"node0/(.+?)/debug\.log$", chain).group(
            1
...
def print_node_warnings(tmp_dir, colors):
    ...
    for stream in ["stdout", "stderr"]:
        for i in itertools.count():
            ...
            for _, _, fns in os.walk(folder):
                for fn in fns:
                    warning = (
                        pathlib.Path("{}/{}".format(folder, fn)).read_text().strip()
                    )
                    if warning:
    ...
    for w in warnings:
        print(
            "{} {} {} {}".format(colors[w[0].split()[0]], w[0], w[1], colors["reset"])
        )
