...
class InvalidAddressErrorMessageTest(DefiTestFramework):
    ...
    def test_validateaddress(self):
        ...
        assert not info["isvalid"]
        assert_equal(
            info["error"], "Version 1+ witness address must use Bech32m checksum"
        )

        info = node.validateaddress(BECH32_INVALID_BECH32M)
        assert not info["isvalid"]
        assert_equal(
            info["error"], "Version 0 witness address must use Bech32 checksum"
        )
    ...
    def test_getaddressinfo(self):
        ...
        assert_raises_rpc_error(
            -5, "Invalid address format", node.getaddressinfo, INVALID_ADDRESS
        )
