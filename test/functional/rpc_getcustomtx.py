...
class TokensRPCGetCustomTX(DefiTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            self.nodes[0].getaccount(collateral_a, {}, True)[token_silver], 300
        )
        ...
        assert (
            "No such mempool or wallet transaction. Use -txindex or provide a block hash."
            in errorString
        )

        # Try and get history node 2 using block hash
        result = self.nodes[2].getcustomtx(
            minttx, self.nodes[0].getblockhash(self.nodes[0].getblockcount())
        )
        self.check_result(result)
        assert_equal(result["type"], "MintToken")
        assert_equal(result["results"][str(token_silver)], Decimal("300.00000000"))
        ...
        # Update token
        updatetx = self.nodes[0].updatetoken(
            token_silver,
            {
                "symbol": "SILVER",
                "name": "silver",
        ...
        accounttoaccount_tx = self.nodes[0].accounttoaccount(
            collateral_b, {collateral_a: "100@" + token_gold}
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
        ...
        remove_liquidity_tx_rawtx = self.nodes[0].getrawtransaction(
            remove_liquidity_tx, 1
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
        ...
        appoint_oracle_tx = self.nodes[0].appointoracle(
            oracle_address, [{"currency": "GBP", "token": "TSLA"}], 1
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
        ...
        assert_equal(
            result["results"]["availablePairs"], [{"token": "TSLA", "currency": "GBP"}]
        )

        # Test appoint oracle
        new_oracle_address = self.nodes[0].getnewaddress("", "legacy")
        update_oracle_tx = self.nodes[0].updateoracle(
            appoint_oracle_tx,
            new_oracle_address,
            [{"currency": "USD", "token": "DFI"}, {"currency": "USD", "token": "TSLA"}],
            10,
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
        ...
        oracle_data_tx = self.nodes[0].setoracledata(
            appoint_oracle_tx, timestamp, oracle_prices
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
        ...
        self.nodes[0].createtoken(
            {"symbol": "GOOGL", "name": "Google", "collateralAddress": collateral_a}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].addpoolliquidity(
            {collateral_a: ["100@" + token_silver, "100@" + token_googl]}, pool_share
        )
        self.nodes[0].generate(1)

        self.nodes[0].updatepoolpair(
            {"pool": poolpair_tx, "status": True, "commission": 0.001}
        )
        self.nodes[0].generate(1)
        ...
        setgov_height_tx = self.nodes[0].setgovheight(
            {"ORACLE_DEVIATION": Decimal("0.04000000")}, 1000
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
        ...
        update_token_tx = self.nodes[0].updateloantoken(
            loan_token_tx,
            {"name": "Tesla stock token", "mintable": True, "interest": 1},
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
        ...
        self.nodes[0].addpoolliquidity(
            {
                self.nodes[0]
                .get_genesis_keys()
                .ownerAuthAddress: ["100@TSLA", "100@DUSD"]
            },
            pool_share,
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].addpoolliquidity(
            {
                self.nodes[0]
                .get_genesis_keys()
                .ownerAuthAddress: ["100@DUSD", "100@DFI"]
            },
            pool_share,
        )
        self.nodes[0].generate(1)
        ...
        update_vault_tx = self.nodes[0].updatevault(
            create_vault_tx,
        ...
        deposit_vault_tx = self.nodes[0].deposittovault(
            create_vault_tx, collateral_a, "10@DFI"
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
        ...
        # Test take loan
        take_loan_tx = self.nodes[0].takeloan(
            {"vaultId": create_vault_tx, "amounts": "1@TSLA"}
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
        ...
        assert_equal(
            result["results"][token_loan_tesla], Decimal(vault["loanAmounts"][0][:10])
        )

        # Test withdraw from vault
        vault_withdraw_tx = self.nodes[0].withdrawfromvault(
            create_vault_tx, new_vault_owner, "0.5@DFI"
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
        ...
        # Test auction bid
        auction_tx = self.nodes[0].placeauctionbid(
            new_vault, 0, self.nodes[0].get_genesis_keys().ownerAuthAddress, "7.1@TSLA"
        )
        self.nodes[0].generate(1)
        self.sync_blocks(self.nodes[0:2])
