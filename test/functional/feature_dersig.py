...
class BIP66Test(DefiTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
        self.extra_args = [
            ["-whitelist=127.0.0.1", "-par=1", "-enablebip61"]
    ...
    def run_test(self):
        ...
        spendtx = create_transaction(
            self.nodes[0], self.coinbase_txids[0], self.nodeaddress, amount=1.0
        )
        unDERify(spendtx)
        spendtx.rehash()
        ...
        block = create_block(
            int(tip, 16), create_coinbase(DERSIG_HEIGHT - 1), block_time
        )
        block.nVersion = 2
        ...
        with self.nodes[0].assert_debug_log(
            expected_msgs=["{}, bad-version(0x00000002)".format(block.hash)]
        ):
            self.nodes[0].p2p.send_and_ping(msg_block(block))
        ...
        wait_until(
            lambda: "reject" in self.nodes[0].p2p.last_message.keys(),
            lock=mininode_lock,
        )
        with mininode_lock:
            ...
            assert (
                b"Non-canonical DER signature"
                in self.nodes[0].p2p.last_message["reject"].reason
            )
