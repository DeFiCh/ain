...
class AnchorRewardsTest(DefiTestFramework):
    ...
    def mock_time(self, offset_hours, hours=0):
        for i in range(0, self.num_nodes):
            self.nodes[i % self.num_nodes].set_mocktime(
                int((time.time() - offset_hours * 60 * 60) + (hours * 60 * 60))
            )
            self.nodes[i % self.num_nodes].setmocktime(
                int((time.time() - offset_hours * 60 * 60) + (hours * 60 * 60))
            )
    ...
    def run_test(self):
        ...
        wait_until(
            lambda: len(self.nodes[0].getanchorteams()["auth"]) == 3
            and len(self.nodes[0].getanchorteams()["confirm"]) == 3,
            timeout=10,
        )
        ...
        wait_until(
            lambda: len(self.nodes[0].spv_listanchorauths()) > 0
            and self.nodes[0].spv_listanchorauths()[0]["signers"] == 3,
            timeout=10,
        )
        ...
        assert_raises_rpc_error(
            None,
            "does not refer to a P2PKH or P2SH or P2WPKH address",
            self.nodes[1].spv_createanchor,
            [
                {
                    "txid": "a0d5a294be3cde6a8bddab5815b8c4cb1b2ebf2c2b8a4018205d6f8c576e8963",
        ...
        wait_until(
            lambda: len(self.nodes[0].spv_listanchorrewardconfirms()) == 1
            and self.nodes[0].spv_listanchorrewardconfirms()[0]["signers"] == 2,
            timeout=10,
        )
        ...
        # Reward before
        assert_equal(
            self.nodes[0].listcommunitybalances()["AnchorReward"], Decimal("6.10000000")
        )
        ...
        # Reward after
        assert_equal(
            self.nodes[0].listcommunitybalances()["AnchorReward"], Decimal("0.10000000")
        )

        # confirms should disappear
        wait_until(
            lambda: len(self.nodes[0].spv_listanchorrewardconfirms()) == 0, timeout=10
        )
        ...
        assert_equal(
            rew0tx["vout"][1]["scriptPubKey"]["addresses"][0], conf0[0]["rewardAddress"]
        )
        ...
        wait_until(
            lambda: len(self.nodes[0].spv_listanchorrewardconfirms()) == 1, timeout=10
        )  # while rollback, it should appear w/o wait
        assert_equal(len(self.nodes[0].spv_listanchorrewards()), 0)
        wait_until(
            lambda: len(self.nodes[2].spv_listanchorrewardconfirms()) == 1, timeout=10
        )  # but wait here
        ...
        # Reward before
        assert_equal(
            self.nodes[0].listcommunitybalances()["AnchorReward"], Decimal("6.30000000")
        )  # 2 more blocks on this chain
        ...
        # Reward before
        assert_equal(
            self.nodes[1].listcommunitybalances()["AnchorReward"], Decimal("7.60000000")
        )  # 2 more blocks on this chain
        ...
        # Reward after
        assert_equal(
            self.nodes[1].listcommunitybalances()["AnchorReward"], Decimal("0.10000000")
        )  # Subsidy halving!
