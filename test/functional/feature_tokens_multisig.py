...
class TokensMultisigOwnerTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        createTokenTx = self.nodes[0].createtoken(
            {"symbol": "GOLD", "name": "shiny", "collateralAddress": multisig_address},
            [],
        )
        ...
        creationTxReversed = "".join(
            reversed(
                [createTokenTx[i : i + 2] for i in range(0, len(createTokenTx), 2)]
            )
        )
        name_change_1 = (
            "446654786e" + creationTxReversed + "04474f4c44034f4e4508000000000000000003"
        )  # name ONE
        name_change_2 = (
            "446654786e" + creationTxReversed + "04474f4c440354574f08000000000000000003"
        )  # name TWO
        name_change_3 = (
            "446654786e"
            + creationTxReversed
            + "04474f4c4405544852454508000000000000000003"
        )  # name THREE
        ...
        rawtx_1 = self.nodes[0].createrawtransaction(
            [{"txid": txid_owner_1, "vout": vout_owner_1}],
            [{"data": name_change_1}, {owner_1: 0.9999}],
        )
        signed_rawtx_1 = self.nodes[0].signrawtransactionwithkey(
            rawtx_1,
            [owner_1_privkey],
            [
                {
                    "txid": txid_owner_1,
                    "vout": vout_owner_1,
                    "scriptPubKey": owner_1_scriptpubkey,
                }
            ],
        )
        ...
        rawtx_1 = self.nodes[0].createrawtransaction(
            [{"txid": txid_1, "vout": vout_1}],
            [{"data": name_change_1}, {owner_1: 0.9999}],
        )
        rawtx_2 = self.nodes[0].createrawtransaction(
            [{"txid": txid_2, "vout": vout_2}],
            [{"data": name_change_2}, {owner_2: 0.9999}],
        )
        rawtx_3 = self.nodes[0].createrawtransaction(
            [{"txid": txid_3, "vout": vout_3}],
        ...
        signed_rawtx_2 = self.nodes[0].signrawtransactionwithkey(
            rawtx_2,
            [owner_2_privkey],
            [
                {
        ...
        signed_rawtx_3 = self.nodes[0].signrawtransactionwithkey(
            rawtx_3,
            [owner_3_privkey],
            [
                {
        ...
        multisig = self.nodes[0].createmultisig(
            2, [owner_1_pubkey, owner_2_pubkey, owner_3_pubkey]
        )
        multisig_address = multisig["address"]
        multisig_scriptpubkey = self.nodes[0].getaddressinfo(multisig_address)[
            "scriptPubKey"
