...
class LoanTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        # take loan
        self.nodes[0].takeloan({"vaultId": vaultId1, "amounts": "1000@TSLA"})
        self.nodes[0].generate(1)
        accountBal = self.nodes[0].getaccount(account)
        assert_equal(
            accountBal, ["1000.00000000@DFI", "1000.00000000@BTC", "1000.00000000@TSLA"]
        )
        self.nodes[0].generate(60)  # ~10 hours
        vault1 = self.nodes[0].getvault(vaultId1)
        interests = self.nodes[0].getinterest(vault1["loanSchemeId"], "TSLA")
        ...
        accountBal = self.nodes[0].getaccount(account)
        assert_equal(
            accountBal, ["1000.00000000@DFI", "1000.00000000@BTC", "1450.00000000@TSLA"]
        )
        ...
        try:
            self.nodes[0].placeauctionbid(
                vaultId1, 0, account2, "555@TSLA"
            )  # just under 1%
        except JSONRPCException as e:
            errorString = e.error["message"]
        assert (
            "Bid override should be at least 1% higher than current one" in errorString
        )
        ...
        assert_equal(
            account2Bal, ["400.00000000@DFI", "400.00000000@BTC", "444.50000000@TSLA"]
        )
        ...
        assert_equal(
            auction["auctionWon"].sort(),
            ["400.00000000@DFI", "400.00000000@BTC"].sort(),
        )
        ...
        assert_equal(
            auction["auctionWon"].sort(),
            ["399.99999600@DFI", "399.99999600@DFI"].sort(),
        )
        ...
        assert_equal(
            accountBal, ["1600.00000000@DFI", "1600.00000000@BTC", "226.00000000@TSLA"]
        )
        ...
        self.nodes[0].sendtokenstoaddress(
            {}, {newAddress: "1600@TSLA"}
        )  # newAddress has now the highest amount of TSLA token
        self.nodes[0].generate(1)

        self.nodes[0].placeauctionbid(
            vaultId1, 0, "*", "1600@TSLA"
        )  # Autoselect address with highest amount of TSLA token
        self.nodes[0].generate(40)  # let auction end
