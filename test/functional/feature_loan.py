...
class LoanTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        price_feeds = [{"currency": "USD", "token": "DFI"}, {"currency": "USD", "token": "BTC"},
        ...
        self.nodes[0].generate(1)
        oracle2_prices = [{"currency": "USD", "tokenAmount": "15@TSLA"}, {"currency": "USD", "tokenAmount": "15@DFI"},
        ...
        self.nodes[0].placeauctionbid(vaultId1, 0, account,
                                      "515@TSLA")  # above 5% and leave vault with some loan to exit liquidation state
        ...
        self.nodes[0].sendtokenstoaddress({}, {
            newAddress: "1600@TSLA"})  # newAddress has now the highest amount of TSLA token
        self.nodes[0].generate(1)

        self.nodes[0].placeauctionbid(vaultId1, 0, "*",
                                      "1600@TSLA")  # Autoselect address with highest amount of TSLA token
        self.nodes[0].generate(40)  # let auction end
