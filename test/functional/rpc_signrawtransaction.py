...
class SignRawTransactionsTest(DefiTestFramework):
    ...
    def witness_script_test(self):
        ...
        unspent_output = self.nodes[1].listunspent(
            0, 999999, [p2sh_p2wsh_address["address"]]
        )[0]
        assert_equal(
            unspent_output["witnessScript"], p2sh_p2wsh_address["redeemScript"]
        )
        p2sh_redeemScript = CScript(
            [OP_0, sha256(hex_str_to_bytes(p2sh_p2wsh_address["redeemScript"]))]
        )
        ...
        spending_tx = self.nodes[0].createrawtransaction(
            [unspent_output], {self.nodes[1].getnewaddress(): Decimal("49.998")}
        )
