...
class TokensRPCListAccountHistory(DefiTestFramework):
    ...
    def run_test(self):
        ...
        for txs in results:
            assert hasattr(txs["amounts"], "__len__") and (
                not isinstance(txs["amounts"], str)
            )
        ...
        results = self.nodes[0].listaccounthistory(
            collateral_a, {"maxBlockHeight": 103, "txn": 0}
        )
        ...
        for txs in results:
            ...
            if txs["blockHeight"] == 103:
                ...
                assert_equal(
                    txs["txn"] >= 0, True
                )  # means txn:0 only applicable to block 103 only
        ...
        # check amounts field is type of array
        assert hasattr(results[0]["amounts"], "__len__") and (
            not isinstance(results[0]["amounts"], str)
        )
        ...
        assert_equal(
            self.nodes[0].listaccounthistory("all", {"txtype": "MintToken"}),
        ...
        assert_equal(
            self.nodes[0].listaccounthistory(
                collateral_a, {"txtypes": ["MintToken", "BurnToken"]}
            ),
        ...
        assert_equal(
            ...
            self.nodes[0].accounthistorycount(
                collateral_a, {"txtypes": ["MintToken", "BurnToken"]}
            ),
        )
        ...
        assert_equal(
            ...
            self.nodes[0].listaccounthistory(
                collateral_a,
                {"txtype": "BurnToken", "txtypes": ["MintToken", "BurnToken"]},
            ),
        )
        assert_equal(
            self.nodes[0].accounthistorycount(
        ...
        # test pagination
        res0 = self.nodes[0].listaccounthistory(
            collateral_a, {"start": 0, "including_start": True}
        )
        res1 = self.nodes[0].listaccounthistory(
            collateral_a, {"start": 1, "including_start": True}
        )
        res2 = self.nodes[0].listaccounthistory(
            collateral_a, {"start": 1, "including_start": False}
        )
        res3 = self.nodes[0].listaccounthistory(
            collateral_a, {"start": 2, "including_start": False}
        )
