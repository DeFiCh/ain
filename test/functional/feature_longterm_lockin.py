...
class MasternodesTimelockTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        nodeid5 = self.nodes[0].createmasternode(
            collateral5, "", [], "FIVEYEARTIMELOCK"
        )
        nodeid10 = self.nodes[0].createmasternode(
            collateral10, "", [], "TENYEARTIMELOCK"
        )
        self.nodes[0].generate(1)

        # Test MN creation with non-standard 20 year lock-in
        nodeid20 = self.nodes[0].createmasternode(
            collateral20, "", [], "TENYEARTIMELOCK"
        )
        ...
        self.nodes[1].invalidateblock(
            self.nodes[1].getblockhash(self.nodes[1].getblockcount() - 1)
        )
        result = self.nodes[1].getmininginfo()
        assert_equal(result["masternodes"][0]["targetMultipliers"], [5, 1])
        ...
        self.nodes[0].set_mocktime(
            int(time.time()) + (5 * 365 * 24 * 60 * 60) + (24 * 60 * 60)
        )
        ...
        self.nodes[0].set_mocktime(
            int(time.time()) + (10 * 365 * 24 * 60 * 60) + (24 * 60 * 60)
        )
