...
class PoolPairTest(DefiTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            [
                ...
                "-fortcanninghillheight=" + str(self.FCH_HEIGHT),
                "-fortcanningroadheight=" + str(self.FCR_HEIGHT),
                "-simulatemainnet",
                "-jellyfish_regtest=1",
            ]
        ]
    ...
    def create_tokens(self):
        ...
        self.nodes[0].generate(1)
        self.symbol_key_GOLD = "GOLD#" + str(
            get_id_token(self.nodes[0], self.symbolGOLD)
        )
        self.symbol_key_SILVER = "SILVER#" + str(
            get_id_token(self.nodes[0], self.symbolSILVER)
        )
        self.symbol_key_DOGE = "DOGE#" + str(
            get_id_token(self.nodes[0], self.symbolDOGE)
        )
    ...
    def mint_tokens(self, amount=1000):
        ...
        self.nodes[0].accounttoaccount(
            self.account0, {self.account_gs: "50000000@" + self.symbol_key_GOLD}
        )
        self.nodes[0].accounttoaccount(
            self.account0, {self.account_gs: "50000000@" + self.symbol_key_SILVER}
        )
        self.nodes[0].generate(1)
        self.nodes[0].accounttoaccount(
            self.account0, {self.account_sd: "50000000@" + self.symbol_key_SILVER}
        )
        self.nodes[0].accounttoaccount(
            self.account0, {self.account_sd: "50000000@" + self.symbol_key_DOGE}
        )
        self.nodes[0].generate(1)
    ...
    def add_1satoshi_liquidity_empty_pool(self):
        ...
        try:
            self.nodes[0].addpoolliquidity(
                {
                    self.account_gs: [
                        "0.0000001@" + self.symbol_key_GOLD,
                        "0.0000001@" + self.symbol_key_SILVER,
                    ]
                },
                self.account_gs,
                [],
    ...
    def add_liquidity(self):
        self.nodes[0].addpoolliquidity(
            {
                self.account_gs: [
                    "5000000@" + self.symbol_key_GOLD,
                    "500000@" + self.symbol_key_SILVER,
                ]
            },
            self.account_gs,
            [],
        ...
        self.nodes[0].addpoolliquidity(
            {
                self.account_sd: [
                    "100000@" + self.symbol_key_DOGE,
                    "1000000@" + self.symbol_key_SILVER,
                ]
            },
            self.account_sd,
            [],
    ...
    def add_1satoshi_liquidity_non_empty_pool(self):
        ...
        try:
            self.nodes[0].addpoolliquidity(
                {
                    self.account_gs: [
                        "0.00000001@" + self.symbol_key_GOLD,
                        "0.00000001@" + self.symbol_key_SILVER,
                    ]
                },
                self.account_gs,
                [],
