...
class WalletTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        def test_balances(*, fee_node_1=0):
            ...
            assert_equal(
                self.nodes[1].getbalance(), Decimal("30") - fee_node_1
            )  # change from node 1's send
            # Same with minconf=0
            assert_equal(self.nodes[0].getbalance(minconf=0), Decimal("9.99"))
            assert_equal(
                self.nodes[1].getbalance(minconf=0), Decimal("30") - fee_node_1
            )
            ...
            assert_equal(
                self.nodes[0].getbalances()["mine"]["untrusted_pending"], Decimal("60")
            )
            assert_equal(
                self.nodes[0].getwalletinfo()["unconfirmed_balance"], Decimal("60")
            )
            ...
            assert_equal(
                self.nodes[1].getbalances()["mine"]["untrusted_pending"], Decimal("0")
            )
            assert_equal(
        ...
        assert_equal(
            self.nodes[0].getbalance(), Decimal("69.99")
        )  # node 1's send plus change from node 0's send
        assert_equal(
            self.nodes[1].getbalance(), Decimal("29.98")
        )  # change from node 0's send
