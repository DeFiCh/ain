...
class LoanPaybackWithCollateralTest(DefiTestFramework):
    ...
    def setup(self):
        ...
        self.nodes[0].addpoolliquidity(
            {self.mn_address: ["10000@" + self.symbolDFI, "8000@" + self.symbolDUSD]},
            self.mn_address,
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    "v0/token/" + self.idDUSD + "/loan_payback_collateral": "true"
                }
            }
        )
        self.nodes[0].generate(1)
    ...
    def test_guards(self):
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    "v0/token/" + self.idDUSD + "/loan_payback_collateral": "false"
                }
            }
        )
        self.nodes[0].generate(1)
        assert_raises_rpc_error(
            -32600,
            "Payback of DUSD loan with collateral is not currently active",
            self.nodes[0].paybackwithcollateral,
            self.vaultId,
        )
        self.nodes[0].setgov(
            {
        ...
        self.nodes[0].deposittovault(
            self.vaultId,
            self.account0,
            str(self.collateralAmount) + "@" + self.symbolDFI,
        )
        self.nodes[0].generate(1)

        assert_raises_rpc_error(
            -32600,
            "Vault does not have any DUSD collaterals",
            self.nodes[0].paybackwithcollateral,
            self.vaultId,
        )
        ...
        self.nodes[0].deposittovault(
            self.vaultId,
            self.account0,
            str(self.collateralAmount) + "@" + self.symbolDUSD,
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId, "amounts": "1@" + self.symbolTSLA}
        )
        self.nodes[0].generate(1)

        assert_raises_rpc_error(
            -32600,
            "Vault does not have any DUSD loans",
            self.nodes[0].paybackwithcollateral,
            self.vaultId,
        )
    ...
    def test_guard_against_liquidation(self):
        ...
        # Deposit DUSD and DFI to vault
        self.nodes[0].deposittovault(
            self.vaultId, self.account0, "151@" + self.symbolDFI
        )
        self.nodes[0].generate(1)

        self.nodes[0].deposittovault(
            self.vaultId, self.account0, "1000@" + self.symbolDUSD
        )
        self.nodes[0].generate(1)

        self.nodes[0].takeloan(
            {"vaultId": self.vaultId, "amounts": "1900@" + self.symbolTSLA}
        )
        self.nodes[0].generate(1)

        self.nodes[0].takeloan(
            {"vaultId": self.vaultId, "amounts": "100@" + self.symbolDUSD}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idDUSD}/loan_minting_interest": "50000000"
                }
            }
        )
        self.nodes[0].generate(
            2
        )  # accrue enough interest to drop below collateralization ratio
    ...
    def test_collaterals_greater_than_loans(self, payback_with_collateral):
        ...
        self.nodes[0].deposittovault(
            self.vaultId,
            self.account0,
            str(collateralDUSDAmount) + "@" + self.symbolDUSD,
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].takeloan(
            {
                ...
                "amounts": str(loanDUSDAmount) + "@" + self.symbolDUSD,
            }
        )
        self.nodes[0].generate(1)

        mintedAmountBefore = self.nodes[0].gettoken(self.symbolDUSD)[self.idDUSD][
            "minted"
        ]
        ...
        assert not any(
            "DUSD" in loan for loan in vault["loanAmounts"]
        )  # Payback all DUSD loans
        assert not any(
            "DUSD" in interest for interest in vault["interestAmounts"]
        )  # Payback all DUSD interests
        assert_equal(
            Decimal(collateralAmount),
            Decimal(collateralAmountBefore) - Decimal(loanAmountBefore),
        )
    ...
    def test_loans_greater_than_collaterals(self, payback_with_collateral):
        ...
        assert not any(
            "DUSD" in collateral for collateral in vault["collateralAmounts"]
        )  # Used all DUSD collateral
        assert_equal(
            Decimal(loanAmount),
            Decimal(loanAmountBefore)
            - Decimal(collateralDUSDAmount)
        ...
        assert_equal(
            Decimal(interestAmount),
            Decimal(storedInterest["interestPerBlock"]).quantize(
    ...
    def test_loans_equal_to_collaterals(self, payback_with_collateral):
        ...
        self.nodes[0].deposittovault(
            ...
            self.account0,
            str(collateralDUSDAmount) + "@" + self.symbolDUSD,
        )  # Deposit enough to match amount of loans + interest after one block
        self.nodes[0].generate(1)
        ...
        assert_equal(
            mintedAmountBefore,
            Decimal(mintedAmount + Decimal(collateralDUSDAmount)).quantize(
    ...
    def test_interest_greater_than_collaterals(self, payback_with_collateral):
        ...
        assert_equal(
            ...
            Decimal("0.000008512937595129375951"),
        )

        assert_equal(
            Decimal(interestAmount),
            Decimal(
                Decimal(storedInterest["interestPerBlock"]) * 2
                - Decimal(collateralDUSDAmount)
            ).quantize(Decimal("1E-8"), rounding=ROUND_UP),
        )
    ...
    def test_interest_equal_to_collaterals(self, payback_with_collateral):
        ...
        self.nodes[0].deposittovault(
            ...
            self.account0,
            str(collateralDUSDAmount) + "@" + self.symbolDUSD,
        )  # Deposit enough to match amount of interest after one block
        self.nodes[0].generate(1)
        ...
        assert_equal(
            ...
            float(vaultBefore["collateralValue"]) - expected_IPB,
        )
    ...
    def test_negative_interest_collaterals_greater_than_loans(
        self, payback_with_collateral
    ):
        height = self.nodes[0].getblockcount()
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idDUSD}/loan_minting_interest": "-500"}}
        )
        self.nodes[0].generate(1)
        ...
        assert not any(
            "DUSD" in loan for loan in vault["loanAmounts"]
        )  # Paid back all DUSD loans
        assert not any(
            "DUSD" in interest for interest in vault["interestAmounts"]
        )  # Paid back all DUSD interests
    ...
    def test_negative_interest_loans_greater_than_collaterals(
        self, payback_with_collateral
    ):
        height = self.nodes[0].getblockcount()
        ...
        [DUSDInterestAmount, _] = vault["interestAmounts"][0].split("@")
        assert_equal(
            Decimal(DUSDLoanAmount),
            Decimal(DUSDLoanAmountBefore)
            - Decimal(collateralDUSDAmount)
            + Decimal(DUSDInterestAmount),
        )
        assert not any(
            "DUSD" in collateral for collateral in vault["collateralAmounts"]
        )  # Used all DUSD collateral
        ...
        storedInterest = self.nodes[0].getstoredinterest(self.vaultId, self.idDUSD)
        assert_equal(
            Decimal(storedInterest["interestPerBlock"]),
            Decimal("-0.000474546864344558599695"),
        )
        assert_equal(Decimal(storedInterest["interestToHeight"]), Decimal("0"))
