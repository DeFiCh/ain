...
class PoolSwapTest(DefiTestFramework):
    ...
    def create_pools(self, owner):
        for i in range(self.COUNT_POOLS):
            ...
            tokenA = tokenA + "#" + get_id_token(self.nodes, tokenA)
            tokenB = tokenB + "#" + get_id_token(self.nodes, tokenB)
    ...
    def mint_tokens(self, owner):
        ...
        for item in self.tokens:
            ...
            self.nodes[0].generate(1)
            self.nodes[0].minttokens(mint_amount + "@" + get_id_token(self.nodes, item), [])
            self.nodes[0].generate(1)
            self.sync_blocks()
        return mint_amount
        ...
        for token in self.tokens:
            for start in range(0, self.COUNT_ACCOUNT, 10):
                ...
                for idx in range(start, end):
                    outputs[self.accounts[idx]] = send_amount + "@" + get_id_token(self.nodes, token)
                self.nodes[0].sendmany("", {owner: 0.02})
                self.nodes[0].generate(1)
    ...
    def add_pools_liquidity(self, owner):
        for item in range(self.COUNT_POOLS):
            ...
            self.liquidity[get_id_token(self.nodes, tokenA)] = 0
            self.liquidity[get_id_token(self.nodes, tokenB)] = 0
            for start in range(0, self.COUNT_ACCOUNT, 10):
                if start + 10 > self.COUNT_ACCOUNT:
                    end = self.COUNT_ACCOUNT
                else:
                ...
                for idx in range(start, end):
                    ...
                    self.liquidity[get_id_token(self.nodes, tokenA)] += amountA
                    self.liquidity[get_id_token(self.nodes, tokenB)] += amountB
                    amountA = str(amountA) + "@" + get_id_token(self.nodes, tokenA)
                    amountB = str(amountB) + "@" + get_id_token(self.nodes, tokenB)
        ...
        for item in range(self.COUNT_POOLS):
            ...
            for start in range(0, self.COUNT_ACCOUNT, 10):
                ...
                for idx in range(start, end):
                    ...
                    amountsB[idx] = self.nodes[0].getaccount(self.accounts[idx], {}, True)[get_id_token(self.nodes, tokenB)]
                    blockCommissionB += (amount * self.DECIMAL) * (
                                commission * self.DECIMAL) / self.DECIMAL / self.DECIMAL
                    self.nodes[0].poolswap({
                        "from": self.accounts[idx],
                        "tokenFrom": get_id_token(self.nodes, tokenB),
                        "amountFrom": amount,
                        "to": self.accounts[idx],
                        "tokenTo": str(get_id_token(self.nodes, tokenA)),
                    }, [])
                self.nodes[0].generate(1)
                self.sync_blocks(nodes)
                ...
                for idx in range(start, end):
                    ...
                    assert_equal(amountsB[idx] - amount + Decimal(str(feeB / self.DECIMAL)),
                                 self.nodes[0].getaccount(self.accounts[idx], {}, True)[get_id_token(self.nodes, tokenB)])
    ...
    def run_test(self):
        ...
        for account in self.accounts:
            assert_equal(self.nodes[0].getaccount(account, {}, True)[get_id_token(self.nodes, self.tokens[0])],
                         self.AMOUNT_TOKEN)
