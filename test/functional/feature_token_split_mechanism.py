...
class TokenSplitMechanismTest(DefiTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            [
                ...
                f"-fortcanningcrunchheight={self.FCC_HEIGHT}",
                "-jellyfish_regtest=1",
                "-subsidytest=1",
            ]
        ]
    ...
    def setup_pools(self):
        ...
        for _ in range(10):
            self.nodes[0].addpoolliquidity(
                {self.account1: ["5000@DFI", "15000@DUSD"]}, self.account1
            )
            self.nodes[0].generate(1)
            self.nodes[0].addpoolliquidity(
                {self.account2: ["4990@DFI", "14970@DUSD"]}, self.account2
            )
            self.nodes[0].generate(1)
            self.nodes[0].addpoolliquidity(
                {self.account3: ["10@DFI", "30@DUSD"]}, self.account3
            )
            self.nodes[0].generate(1)

        for _ in range(10):
            self.nodes[0].addpoolliquidity(
                {self.account1: ["500@T1", "5000000@DUSD"]}, self.account1
            )
            self.nodes[0].generate(1)
            self.nodes[0].addpoolliquidity(
                {self.account2: ["499@T1", "4990000@DUSD"]}, self.account2
            )
            self.nodes[0].generate(1)
            self.nodes[0].addpoolliquidity(
                {self.account3: ["1@T1", "10000@DUSD"]}, self.account3
            )
            self.nodes[0].generate(1)

        for _ in range(10):
            self.nodes[0].addpoolliquidity(
                {self.account1: ["10000@T2", "500000@DUSD"]}, self.account1
            )
            self.nodes[0].generate(1)
            self.nodes[0].addpoolliquidity(
                {self.account2: ["9980@T2", "499000@DUSD"]}, self.account2
            )
            self.nodes[0].generate(1)
            self.nodes[0].addpoolliquidity(
                {self.account3: ["20@T2", "1000@DUSD"]}, self.account3
            )
            self.nodes[0].generate(1)

        for _ in range(10):
            self.nodes[0].addpoolliquidity(
                {self.account1: ["5000000@T3", "0.05@DUSD"]}, self.account1
            )
            self.nodes[0].generate(1)
            self.nodes[0].addpoolliquidity(
                {self.account2: ["4990000@T3", "0.0499@DUSD"]}, self.account2
            )
            self.nodes[0].generate(1)
            self.nodes[0].addpoolliquidity(
                {self.account3: ["10000@T3", "0.0001@DUSD"]}, self.account3
            )
            self.nodes[0].generate(1)

        for _ in range(10):
            self.nodes[0].addpoolliquidity(
                {self.account1: ["5000@T1", "250@T2"]}, self.account1
            )
            self.nodes[0].generate(1)
            self.nodes[0].addpoolliquidity(
                {self.account2: ["4990@T1", "249.5@T2"]}, self.account2
            )
            self.nodes[0].generate(1)
            self.nodes[0].addpoolliquidity(
                {self.account3: ["10@T1", "0.5@T2"]}, self.account3
            )
            self.nodes[0].generate(1)
    ...
    def split(self, tokenId, keepLocked=False):
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/oracles/splits/{str(splitHeight)}": f"{tokenId}/2"}}
        )
    ...
    def check_attributes_on_split(self, tokenId, revert=False):
        ...
        for poolId in pools:
            self.nodes[0].setgov(
                {
                    "ATTRIBUTES": {
                        f"v0/poolpairs/{poolId}/token_a_fee_pct": "0.01",
    ...
    def check_amounts_on_split(self, poolId, tokenId, revert=False):
        ...
        if amountLPBeforeAcc1 != "0":
            self.nodes[0].removepoolliquidity(
                self.account1, amountLPBeforeAcc1 + "@" + poolSymbol, []
            )
            self.nodes[0].generate(1)
        if amountLPBeforeAcc2 != "0":
            self.nodes[0].removepoolliquidity(
                self.account2, amountLPBeforeAcc2 + "@" + poolSymbol, []
            )
            self.nodes[0].generate(1)
        if amountLPBeforeAcc3 != "0":
            self.nodes[0].removepoolliquidity(
                self.account3, amountLPBeforeAcc3 + "@" + poolSymbol, []
            )
            self.nodes[0].generate(1)
        ...
        self.nodes[0].removepoolliquidity(
            self.account1, amountLPAfterAcc1 + "@" + poolSymbol, []
        )
        self.nodes[0].generate(1)
        self.nodes[0].removepoolliquidity(
            self.account2, amountLPAfterAcc2 + "@" + poolSymbol, []
        )
        self.nodes[0].generate(1)
        self.nodes[0].removepoolliquidity(
            self.account3, amountLPAfterAcc3 + "@" + poolSymbol, []
        )
        self.nodes[0].generate(1)
        ...
        assert (
            Decimal(amountTokenB_BeforeAcc1) - Decimal(amountTokenB_AfterAcc1)
        ).copy_abs() <= (Decimal(0.00001) * Decimal(amountTokenB_BeforeAcc1))
        assert (
            Decimal(amountTokenB_BeforeAcc2) - Decimal(amountTokenB_AfterAcc2)
        ).copy_abs() <= (Decimal(0.00001) * Decimal(amountTokenB_BeforeAcc2))
        assert (
            Decimal(amountTokenB_BeforeAcc3) - Decimal(amountTokenB_AfterAcc3)
        ).copy_abs() <= (Decimal(0.00001) * Decimal(amountTokenB_BeforeAcc3))
        assert (
            (Decimal(amountTokenBeforeAcc1) * 2) - Decimal(amountTokenAfterAcc1)
        ).copy_abs() <= Decimal(0.00001) * Decimal(amountTokenBeforeAcc1)
        assert (
            (Decimal(amountTokenBeforeAcc2) * 2) - Decimal(amountTokenAfterAcc2)
        ).copy_abs() <= Decimal(0.00001) * Decimal(amountTokenBeforeAcc2)
        assert (
            (Decimal(amountTokenBeforeAcc3) * 2) - Decimal(amountTokenAfterAcc3)
        ).copy_abs() <= Decimal(0.00001) * Decimal(amountTokenBeforeAcc3)
