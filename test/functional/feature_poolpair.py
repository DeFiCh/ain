...
class PoolPairTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        assert (
            "Error, there is already a poolpair with same tokens, but different poolId"
            in errorString
        )
        ...
        spendTx = self.nodes[0].createrawtransaction(
            [{"txid": decodeTx["txid"], "vout": 1}], [{collateral0: 9.999}]
        )
        ...
        self.nodes[0].updatepoolpair(
            {"pool": "PTGOLD", "status": False, "commission": 0.01}, []
        )
        self.nodes[0].generate(1)

        assert_equal(self.nodes[0].getpoolpair("PTGOLD")["2"]["status"], False)
        assert_equal(
            str(self.nodes[0].getpoolpair("PTGOLD")["2"]["commission"]), "0.01000000"
        )
        self.sync_blocks([self.nodes[0], self.nodes[2]])
        assert_equal(self.nodes[2].getpoolpair("PTGOLD")["2"]["status"], False)
        assert_equal(
            str(self.nodes[2].getpoolpair("PTGOLD")["2"]["commission"]), "0.01000000"
        )
        ...
        assert_equal(self.nodes[0].getpoolpair("PTGOLD")["2"]["status"], False)
        assert_equal(
            str(self.nodes[0].getpoolpair("PTGOLD")["2"]["commission"]), "0.10000000"
        )
        ...
        self.nodes[0].updatepoolpair(
            {"pool": "PTGOLD", "ownerAddress": collateral1}, []
        )
        self.nodes[0].generate(1)
        assert_equal(self.nodes[0].getpoolpair("PTGOLD")["2"]["status"], True)
        assert_equal(
        ...
        self.nodes[0].updatepoolpair(
            {"pool": "PTGOLD", "ownerAddress": collateral0}, []
        )
        self.nodes[0].generate(1)
        assert_equal(
            self.nodes[0].getpoolpair("PTGOLD")["2"]["ownerAddress"], ownerAddress
        )
