...
class ICXOrderbookErrorTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].addpoolliquidity(
            {accountDFI: ["100@" + symbolDFI, "1@" + symbolBTC]}, accountDFI, []
        )
        self.nodes[0].generate(1)
        ...
        try:
            ...
            errorString = e.error["message"]
        assert (
            "Not enough balance for Token DFI on address " + accountDFI in errorString
        )
        ...
        try:
            ...
            errorString = e.error["message"]
        assert (
            "orderTx (76432beb2a667efe4858b4e1ec93979b621c51c76abaab2434892655dd152e3d) does not exist"
            in errorString
        )
        ...
        try:
            self.nodes[1].icx_makeoffer(
                {"orderTx": orderTx, "amount": 0.01, "ownerAddress": accountDFI}
            )
        except JSONRPCException as e:
            errorString = e.error["message"]
        ...
        try:
            self.nodes[1].icx_makeoffer(
                {"orderTx": orderTx, "amount": 0.01, "ownerAddress": accountBTC}
            )
        except JSONRPCException as e:
            errorString = e.error["message"]
        ...
        assert (
            "offer ("
            + offerTx
            + ") needs to have dfc htlc submitted first, but no dfc htlc found!"
            in errorString
        )
        ...
        try:
            ...
            errorString = e.error["message"]
        assert (
            "dfc htlc with creation tx 76432beb2a667efe4858b4e1ec93979b621c51c76abaab2434892655dd152e3d does not exists"
            in errorString
        )
        ...
        try:
            ...
            errorString = e.error["message"]
        assert (
            "hash generated from given seed is different than in dfc htlc"
            in errorString
        )
        ...
        assert (
            "offer ("
            + offerTx
            + ") needs to have ext htlc submitted first, but no external htlc found"
            in errorString
        )
