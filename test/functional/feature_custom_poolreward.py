...
class TokensCustomPoolReward(DefiTestFramework):
    ...
    def run_test(self):
        ...
        # Create token
        self.nodes[0].createtoken(
            {"symbol": "SILVER", "name": "silver", "collateralAddress": collateral_b}
        )
        self.nodes[0].generate(1)
        self.sync_blocks()
        ...
        assert_equal(
            result["results"]["customRewards"],
            ["1.00000000@" + token_a, "1.00000000@" + token_b],
        )
        ...
        assert_equal(
            ...
            ["0.99999000@SILVGOLD", "0.99999000@GOLD#128"],
        )
        ...
        assert_equal(
            ...
            ["0.00001000@GOLD#128", "0.00001000@SILVER#129"],
        )
        self.nodes[0].generate(1)
        self.sync_blocks()
        ...
        tx1 = self.nodes[0].accounttoaccount(
            collateral_a, {pool_collateral: "10@" + token_a}
        )
        tx2 = self.nodes[0].accounttoaccount(
            collateral_b, {pool_collateral: "10@" + token_b}
        )
        ...
        assert_equal(
            ...
            ["1.00000000@128", "1.00000000@129"],
        )
        assert_equal(
            self.nodes[0].getpoolpair(1)["1"]["customRewards"],
            ["1.00000000@128", "1.00000000@129"],
        )
        ...
        assert_equal(
            self.nodes[1].getaccount(pool_collateral),
            ["0.00011000@GOLD#128", "0.00011000@SILVER#129"],
        )
        ...
        assert_equal(
            self.nodes[0].getpoolpair(1)["1"]["customRewards"], ["1.00000000@128"]
        )
        ...
        # Check only token a reward shows
        assert_equal(
            self.nodes[0].getpoolpair(1)["1"]["customRewards"], ["1.00000000@129"]
        )
        ...
        self.nodes[0].createtoken(
            {"symbol": "BRONZE", "name": "bronze", "collateralAddress": collateral_c}
        )
        self.nodes[0].generate(1)
        self.sync_blocks()
        ...
        try:
            self.nodes[0].updatepoolpair(
                {"pool": "1", "customRewards": ["0@" + token_c]}
            )
        except JSONRPCException as e:
            errorString = e.error["message"]
        assert "Amount out of range" in errorString
        ...
        updatepoolpair_tx = self.nodes[0].updatepoolpair(
            {"pool": "1", "customRewards": ["1@" + token_c]}
        )
        self.nodes[0].generate(1)
        self.sync_blocks()
        ...
        assert_equal(
            result["results"]["customRewards"],
            ["1.00000000@" + token_a, "1.00000000@" + token_c],
        )
        ...
        updatepoolpair_tx = self.nodes[0].updatepoolpair(
            {
                "pool": "1",
                "customRewards": ["1@" + token_a, "1@" + token_b, "1@" + token_c],
            }
        )
        self.nodes[0].generate(1)
        self.sync_blocks()
