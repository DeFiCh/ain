...
class TokensCustomPoolReward(DefiTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [['-txnotokens=0', '-amkheight=50', '-bayfrontheight=50', '-bayfrontgardensheight=50',
                            '-clarkequayheight=50'],
                           ['-txnotokens=0', '-amkheight=50', '-bayfrontheight=50', '-bayfrontgardensheight=50',
                            '-clarkequayheight=50']]
    ...
    def run_test(self):
        ...
        assert_equal(self.nodes[1].getaccount(provider),
                     ['0.99999000@SILVGOLD', '0.99999000@GOLD#128', '0.99999000@SILVER#129'])
        ...
        assert_equal(self.nodes[1].getaccount(provider),
                     ['0.99999000@SILVGOLD', '10.99989000@GOLD#128', '10.99989000@SILVER#129'])
        ...
        assert_equal(self.nodes[1].getaccount(provider),
                     ['0.99999000@SILVGOLD', '20.99979000@GOLD#128', '20.99979000@SILVER#129', '0.99999000@BRONZE#130'])
        assert_equal(self.nodes[1].getaccount(pool_collateral),
        ...
        assert_equal(self.nodes[1].getpoolpair(1)['1']['customRewards'], ['1.00000000@130'])
        assert_equal(self.nodes[1].getaccount(provider),
                     ['0.99999000@SILVGOLD', '20.99979000@GOLD#128', '20.99979000@SILVER#129', '1.99998000@BRONZE#130'])
        assert_equal(self.nodes[1].getaccount(pool_collateral),
                     ['10.00021000@GOLD#128', '0.00021000@SILVER#129', '8.00002000@BRONZE#130'])

        # Add back token a
        updatepoolpair_tx = self.nodes[0].updatepoolpair(
            {"pool": "1", "customRewards": ["1@" + token_a, "1@" + token_c]})
        self.nodes[0].generate(1)
        self.sync_blocks()
        ...
        assert_equal(self.nodes[1].getaccount(provider),
                     ['0.99999000@SILVGOLD', '21.99978000@GOLD#128', '20.99979000@SILVER#129', '2.99997000@BRONZE#130'])
        assert_equal(self.nodes[1].getaccount(pool_collateral),
        ...
        assert_equal(self.nodes[1].getpoolpair(1)['1']['customRewards'], ['1.00000000@128', '1.00000000@130'])
        assert_equal(self.nodes[1].getaccount(provider),
                     ['0.99999000@SILVGOLD', '22.99977000@GOLD#128', '20.99979000@SILVER#129', '3.99996000@BRONZE#130'])
        assert_equal(self.nodes[1].getaccount(pool_collateral),
                     ['8.00023000@GOLD#128', '10.00021000@SILVER#129', '6.00004000@BRONZE#130'])

        # Add back token b
        updatepoolpair_tx = self.nodes[0].updatepoolpair(
            {"pool": "1", "customRewards": ["1@" + token_a, "1@" + token_b, "1@" + token_c]})
        self.nodes[0].generate(1)
        self.sync_blocks()
        ...
        assert_equal(result['results']['customRewards'],
                     ["1.00000000@" + token_a, "1.00000000@" + token_b, "1.00000000@" + token_c])

        # Check for new block reward and payout for token a, b and c
        assert_equal(self.nodes[1].getpoolpair(1)['1']['customRewards'],
                     ['1.00000000@128', '1.00000000@129', '1.00000000@130'])
        assert_equal(self.nodes[1].getaccount(provider),
                     ['0.99999000@SILVGOLD', '23.99976000@GOLD#128', '21.99978000@SILVER#129', '4.99995000@BRONZE#130'])
        assert_equal(self.nodes[1].getaccount(pool_collateral),
        ...
        assert_equal(self.nodes[1].getpoolpair(1)['1']['customRewards'],
                     ['1.00000000@128', '1.00000000@129', '1.00000000@130'])
        assert_equal(self.nodes[1].getaccount(provider),
                     ['0.99999000@SILVGOLD', '24.99975000@GOLD#128', '22.99977000@SILVER#129', '5.99994000@BRONZE#130'])
        assert_equal(self.nodes[1].getaccount(pool_collateral),
