...
class PoolPairAsymmetricTest(DefiTestFramework):
    ...
    def pool_swaps(self):
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/poolpairs/{self.idDD}/token_a_fee_direction": "out"}}
        )
        self.nodes[0].generate(1)
        ...
        self.test_swap(
            self.symbolDFI, self.symbolDUSD, Decimal("0.05"), Decimal("0.05")
        )

        # Test DUSD to DFI, 5% fee on both
        self.test_swap(
            self.symbolDUSD, self.symbolDFI, Decimal("0.05"), Decimal("0.05")
        )
