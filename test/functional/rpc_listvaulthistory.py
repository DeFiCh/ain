...
class TokensRPCGetVaultHistory(DefiTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].setcollateraltoken(
            {"token": "DUSD", "factor": 1, "fixedIntervalPriceId": "DUSD/USD"}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].generate(1)
        deposit_tx = self.nodes[0].deposittovault(
        ...
        update_tx = self.nodes[0].updatevault(
            create_another_tx, {"loanSchemeId": "LOAN0001"}
        )

        # Change loan scheme
        update_tx = self.nodes[0].updatevault(
            create_vault_tx, {"loanSchemeId": "LOAN0001"}
        )
        self.nodes[0].generate(1)
        ...
        # Take loan
        takeloan_tx = self.nodes[0].takeloan(
        ...
        withdraw_tx = self.nodes[0].withdrawfromvault(
            create_vault_tx, withdraw_address, "0.5@DFI"
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            result[4]["address"], self.nodes[0].get_genesis_keys().ownerAuthAddress
        )
        ...
        result = self.nodes[0].listvaulthistory(
            create_vault_tx, {"maxBlockHeight": withdraw_height, "depth": 1}
        )
        assert_equal(len(result), 4)
        ...
        assert_equal(result[3]["type"], "PaybackLoan")
        assert_equal(
            result[3]["address"], self.nodes[0].get_genesis_keys().ownerAuthAddress
        )
        ...
        assert_equal(
            result[0]["address"], self.nodes[0].get_genesis_keys().ownerAuthAddress
        )
        ...
        # Take loan
        takeloan_tx = self.nodes[0].takeloan(
            {"vaultId": new_vault, "amounts": "6.68896@TSLA"}
        )
        ...
        # Place bids
        bid1_tx = self.nodes[0].placeauctionbid(
            new_vault, 0, self.nodes[0].get_genesis_keys().ownerAuthAddress, "8@TSLA"
        )
        self.nodes[0].generate(1)
        bid2_tx = self.nodes[0].placeauctionbid(
            new_vault, 0, new_bidding_address, "9@TSLA"
        )
        ...
        assert_equal(
            result[0]["vaultSnapshot"]["collateralAmounts"], ["1.87309837@DFI"]
        )
        assert_equal(
            result[0]["vaultSnapshot"]["collateralValue"], Decimal("187.30983700")
        )
        ...
        if result[1]["address"] == new_bidding_address:
            ...
            assert_equal(
                result[2]["address"], self.nodes[0].get_genesis_keys().ownerAuthAddress
            )
        ...
        assert_equal(
            result[4]["vaultSnapshot"]["collateralValue"], Decimal("0.00000000")
        )
        ...
        assert_equal(
            result[4]["vaultSnapshot"]["batches"][0]["collaterals"], ["10.00000000@DFI"]
        )
        assert_equal(
            result[4]["vaultSnapshot"]["batches"][0]["loan"], "6.68896763@TSLA"
        )
        ...
        # Create DFI tokens
        self.nodes[0].utxostoaccount(
            {self.nodes[0].get_genesis_keys().ownerAuthAddress: "10@DFI"}
        )
        self.nodes[0].generate(1)

        # Deposit DUSD to vault
        self.nodes[0].deposittovault(
            vault_id, self.nodes[0].get_genesis_keys().ownerAuthAddress, f"2@DFI"
        )
        self.nodes[0].deposittovault(
            vault_id, self.nodes[0].get_genesis_keys().ownerAuthAddress, f"2@DUSD"
        )
        self.nodes[0].generate(1)
