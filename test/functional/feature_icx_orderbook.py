...
class ICXOrderbookTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].addpoolliquidity(
            {accountDFI: ["1@" + symbolBTC, "100@" + symbolDFI]}, accountDFI, []
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            order[orderTx]["expireHeight"],
            self.nodes[0].getblockchaininfo()["blocks"] + 2880,
        )

        beforeOffer = self.nodes[1].getaccount(accountBTC, {}, True)[idDFI]

        offerTx = self.nodes[1].icx_makeoffer(
            {"orderTx": orderTx, "amount": 0.10, "ownerAddress": accountBTC}
        )["txid"]
        ...
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI],
            beforeOffer - Decimal("0.01000000"),
        )
        ...
        for vout in authTx["vout"]:
            if (
                ...
                and vout["scriptPubKey"]["addresses"][0] == accountBTC
            ):
                found = True
        assert found
        ...
        for vout in authTx["vout"]:
            if (
                "addresses" in vout["scriptPubKey"]
                and vout["scriptPubKey"]["addresses"][0] == accountDFI
            ):
                found = True
        assert found
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeOrder - Decimal("15.00000000"),
        )
        ...
        assert_equal(
            offer[offerTx]["expireHeight"],
            self.nodes[0].getblockchaininfo()["blocks"] + 20,
        )
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeDFCHTLC - Decimal("0.01000000"),
        )
        ...
        assert_equal(
            htlcs[exthtlcTx]["hash"],
            "957fc0fd643f605b2938e0631a61529fd70bd35b2162a21d978c41e5241a5220",
        )
        assert_equal(
            htlcs[exthtlcTx]["htlcScriptAddress"], "13sJQ9wBWh8ssihHUgAaCmNWJbBAG5Hr9N"
        )
        assert_equal(
            htlcs[exthtlcTx]["ownerPubkey"],
        ...
        htlcs = self.nodes[0].icx_listhtlcs({"offerTx": offerTx, "closed": True})
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeClaim0 + Decimal("0.01250000"),
        )
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI],
            beforeClaim1 + Decimal("10.00000000"),
        )
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeClaim0 + Decimal("0.00625000"),
        )
        # claimed DFI on taker address
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI],
            beforeClaim1 + Decimal("5.00000000"),
        )
        ...
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI],
            beforeOffer - Decimal("0.30000000"),
        )
        ...
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI],
            beforeOffer - Decimal("0.20000000"),
        )
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeClaim + Decimal("2000.00000000") + Decimal("0.25000000"),
        )
        ...
        offerTx = self.nodes[1].icx_makeoffer(
            {"orderTx": orderTx, "amount": 1, "ownerAddress": accountBTC}
        )["txid"]
        ...
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI],
            beforeOffer - Decimal("0.10000000"),
        )
        ...
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI],
            beforeOffer - Decimal("0.01500000"),
        )
        ...
        offerTx = self.nodes[1].icx_makeoffer(
            {"orderTx": orderTxDFI, "amount": 0.1, "ownerAddress": accountBTC}
        )["txid"]
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI], beforeDFCHTLC
        )

        htlcs = self.nodes[0].icx_listhtlcs({"offerTx": offerTx, "closed": True})
        offer = self.nodes[0].icx_listorders({"orderTx": orderTxDFI, "closed": True})
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeEXTHTLC - Decimal("0.10000000"),
        )
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI], beforeEXTHTLC
        )

        htlcs = self.nodes[0].icx_listhtlcs({"offerTx": offerTx, "closed": True})
        offer = self.nodes[0].icx_listorders({"orderTx": orderTxBTC, "closed": True})
        ...
        offerTx = self.nodes[1].icx_makeoffer(
            {"orderTx": orderTx, "amount": 0.1, "ownerAddress": accountBTC}
        )["txid"]
        ...
        beforeOrder = self.nodes[0].getaccount(accountDFI, {}, True)[idBTC]
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idBTC],
            beforeOrder - Decimal("0.50000000"),
        )

        order = self.nodes[0].icx_listorders()

        assert_equal(len(order), 2)
        assert_equal(order[orderTx]["tokenFrom"], symbolBTC)
        assert_equal(order[orderTx]["chainTo"], "BTC")
        assert_equal(order[orderTx]["ownerAddress"], accountDFI)
        assert_equal(
            order[orderTx]["receivePubkey"],
        ...
        assert_equal(order[orderTx]["amountFrom"], Decimal("0.5"))
        assert_equal(order[orderTx]["amountToFill"], Decimal("0.5"))
        assert_equal(order[orderTx]["orderPrice"], Decimal("1"))
        assert_equal(order[orderTx]["amountToFillInToAsset"], Decimal("0.5"))
        assert_equal(
            order[orderTx]["expireHeight"],
            self.nodes[0].getblockchaininfo()["blocks"] + 2880,
        )
        ...
        offerTx = self.nodes[1].icx_makeoffer(
            {"orderTx": orderTx, "amount": 0.5, "ownerAddress": accountBTC}
        )["txid"]
        ...
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI],
            beforeOffer - Decimal("0.05"),
        )  # why 0.05 DFI? 0.5 * 0.001 * 100 (order amount * fee per btc * DFI price in BTC)

        offer = self.nodes[0].icx_listorders({"orderTx": orderTx})

        assert_equal(len(offer), 2)
        assert_equal(offer[offerTx]["orderTx"], orderTx)
        assert_equal(offer[offerTx]["amount"], Decimal("0.50000000"))
        assert_equal(offer[offerTx]["ownerAddress"], accountBTC)
        assert_equal(offer[offerTx]["takerFee"], Decimal("0.05000000"))
        assert_equal(
            offer[offerTx]["expireHeight"],
            self.nodes[0].getblockchaininfo()["blocks"] + 20,
        )
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeDFCHTLC - Decimal("0.05000000"),
        )

        # Check burn
        assert_equal(
            self.nodes[0].getburninfo()["tokens"][0], "0.80000000@DFI"
        )  # 0.7 from previous, 0.1 for this order
        result = self.nodes[0].listburnhistory()
        assert_equal(result[0]["owner"], burn_address)
        assert_equal(result[0]["type"], "ICXSubmitDFCHTLC")
        assert_equal(result[0]["amounts"][0], "0.10000000@DFI")
        ...
        assert_equal(htlcs[dfchtlcTx]["status"], "OPEN")
        assert_equal(htlcs[dfchtlcTx]["offerTx"], offerTx)
        assert_equal(htlcs[dfchtlcTx]["amount"], Decimal("0.5"))
        assert_equal(htlcs[dfchtlcTx]["amountInEXTAsset"], Decimal("0.5"))
        assert_equal(
            htlcs[dfchtlcTx]["hash"],
            "957fc0fd643f605b2938e0631a61529fd70bd35b2162a21d978c41e5241a5220",
        )
        ...
        assert_equal(htlcs[exthtlcTx]["status"], "OPEN")
        assert_equal(htlcs[exthtlcTx]["offerTx"], offerTx)
        assert_equal(htlcs[exthtlcTx]["amount"], Decimal("0.5"))
        assert_equal(htlcs[exthtlcTx]["amountInDFCAsset"], Decimal("0.5"))
        assert_equal(
            htlcs[exthtlcTx]["hash"],
            "957fc0fd643f605b2938e0631a61529fd70bd35b2162a21d978c41e5241a5220",
        )
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeClaim0 + Decimal("0.05") + Decimal("0.0375"),
        )  # 0.05 lock refund + 0.75 * 0.05 taker fee refund
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI], beforeClaim1
        )  # no refunds for taker

        # dBTC/BTC Scenario, tests 25% taker fee refund
        # both accounts have >=1BTC
        self.nodes[1].accounttoaccount(accountBTC, {accountDFI: "1@" + symbolBTC})
        self.nodes[1].generate(1)
        self.sync_blocks()
        ...
        assert_equal(order[orderTx]["amountFrom"], Decimal("0.5"))
        assert_equal(order[orderTx]["amountToFill"], Decimal("0.5"))
        assert_equal(order[orderTx]["orderPrice"], Decimal("0.9"))
        assert_equal(order[orderTx]["amountToFillInToAsset"], Decimal("0.45"))
        assert_equal(
            order[orderTx]["expireHeight"],
            self.nodes[0].getblockchaininfo()["blocks"] + 2880,
        )
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeDFCHTLC - Decimal("0.04500000"),
        )

        # Check burn
        assert_equal(
            self.nodes[0].getburninfo()["tokens"][0], "0.89000000@DFI"
        )  # 0.8 from previous, 0.09 for this order
        result = self.nodes[0].listburnhistory()
        assert_equal(result[0]["owner"], burn_address)
        assert_equal(result[0]["type"], "ICXSubmitDFCHTLC")
        assert_equal(result[0]["amounts"][0], "0.09000000@DFI")
        ...
        assert_equal(htlcs[dfchtlcTx]["offerTx"], offerTx)
        assert_equal(htlcs[dfchtlcTx]["amount"], Decimal("0.5"))
        assert_equal(htlcs[dfchtlcTx]["amountInEXTAsset"], Decimal("0.45"))
        assert_equal(
            htlcs[dfchtlcTx]["hash"],
            "957fc0fd643f605b2938e0631a61529fd70bd35b2162a21d978c41e5241a5220",
        )
        ...
        assert_equal(htlcs[exthtlcTx]["status"], "OPEN")
        assert_equal(htlcs[exthtlcTx]["offerTx"], offerTx)
        assert_equal(htlcs[exthtlcTx]["amount"], Decimal("0.45"))
        assert_equal(htlcs[exthtlcTx]["amountInDFCAsset"], Decimal("0.5"))
        assert_equal(
            htlcs[exthtlcTx]["hash"],
            "957fc0fd643f605b2938e0631a61529fd70bd35b2162a21d978c41e5241a5220",
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeClaim0 + Decimal("0.045") + Decimal("0.01125"),
        )  # 0.045 lock refund + 0.25 * 0.045 taker fee refund
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI], beforeClaim1
        )  # no refunds for taker
        ...
        assert_raises_rpc_error(
            -32600,
            "Cannot create tx, ICX is not enabled",
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeClaim0DFI + Decimal("0.0125"),
        )
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idBTC], beforeClaim0BTC
        )
        # claimed DFI on taker address
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI],
            beforeClaim1DFI + Decimal("10.00000000"),
        )
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idBTC], beforeClaim1BTC
        )
        ...
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI],
            beforeFee1 - Decimal("0.1"),
        )
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeFee - Decimal("0.1"),
        )
        ...
        assert_equal(
            self.nodes[0].getaccount(accountDFI, {}, True)[idDFI],
            beforeClaim0DFI + Decimal("0.175"),
        )
        assert_equal(len(self.nodes[0].getaccount(accountDFI, {}, True)), 2)
        # claimed DFI on taker address
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idDFI], beforeClaim1DFI
        )
        assert_equal(
            self.nodes[1].getaccount(accountBTC, {}, True)[idBTC],
            beforeClaim1BTC + Decimal("1.00000000"),
        )
