...
class StoredInterestTest(DefiTestFramework):
    ...
    def setup_test_oracles(self):
        ...
        # Define price feeds
        price_feed = [{"currency": "USD", "token": "DFI"}]
    ...
    def test_token_interest_change(self):
        ...
        self.nodes[0].accounttoaccount(
            self.address, {vault_address: f"10@{self.symbolDFI}"}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idDUSD}/loan_minting_interest": "1"}}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(stored_interest["interestToHeight"]), positive_stored_ipb * 10
        )
        assert_equal(stored_interest["height"], self.nodes[0].getblockcount())

        # Set negative interest to be inverse of positive interest
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idDUSD}/loan_minting_interest": "-3"}}
        )
        ...
        assert_equal(stored_interest["interestPerBlock"], "-0.000000380517503805175038")
        assert_equal(
            Decimal(stored_interest["interestToHeight"]), positive_stored_ipb * 5
        )
        ...
        assert_equal(
            attrs["v0/live/economy/negative_interest_current"],
            [f"0.00000190@{self.symbolDUSD}"],
        )
        ...
        assert_equal(stored_interest["interestPerBlock"], "-0.000000380517503805175038")
        assert_equal(
            Decimal(stored_interest["interestToHeight"]), negative_stored_ipb * 5
        )
        ...
        assert_equal(stored_interest["interestPerBlock"], "-0.000000380517503805175038")
        assert_equal(
            Decimal(stored_interest["interestToHeight"]), negative_stored_ipb * 10
        )
        assert_equal(stored_interest["height"], self.nodes[0].getblockcount())
    ...
    def test_take_loan_neg_ipb_pos_ith(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": vault_id, "amounts": f"0.00000001@{self.symbolDUSD}"}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            stored_interest["interestToHeight"], "0.000001902587500000000000"
        )  # Reduced 6 * IPB
        ...
        # Take another loan to update stored interest
        self.nodes[0].takeloan(
            {"vaultId": vault_id, "amounts": f"0.00000040@{self.symbolDUSD}"}
        )
        self.nodes[0].generate(1)
        ...
        attrs = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attrs["v0/live/economy/negative_interest"],
            [f"0.00000038@{self.symbolDUSD}"],
        )
        ...
        # Revert and restore positive ITH
        self.nodes[0].invalidateblock(
            self.nodes[0].getblockhash(self.nodes[0].getblockcount() - 1)
        )
        self.nodes[0].clearmempool()
        ...
        self.nodes[0].takeloan(
            {"vaultId": vault_id, "amounts": f"0.50000000@{self.symbolDUSD}"}
        )
        self.nodes[0].generate(1)
    ...
    def test_payback_loan(self):
        ...
        self.nodes[0].paybackloan(
            {
                "vaultId": vault_id,
                "from": self.address,
                "amounts": f"0.00000001@{self.symbolDUSD}",
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(stored_interest["interestPerBlock"], "0.000000380517503805175038")
        assert_equal(
            stored_interest["interestToHeight"], "0.000003795175038051750380"
        )  # Less 1 Sat payback
        assert_equal(stored_interest["height"], self.nodes[0].getblockcount())
        ...
        self.nodes[0].paybackloan(
            {
                "vaultId": vault_id,
                "from": self.address,
                "amounts": self.nodes[0].getloantokens(vault_id)[0],
            }
        )
        self.nodes[0].generate(6)
    ...
    def test_takeloan_close_loan(self):
        # Set mega negative interest
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idDUSD}/loan_minting_interest": "-200000"}}
        )
        self.nodes[0].generate(1)
    ...
    def test_payback_partial_close_loan(self):
        # Set mega negative interest
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idDUSD}/loan_minting_interest": "-100000"}}
        )
        self.nodes[0].generate(1)
    ...
    def test_take_loan_pos_ipb_neg_ith(self):
        ...
        # Take DUSD loan
        self.nodes[0].takeloan(
            {"vaultId": vault_id, "amounts": f"0.00000400@{self.symbolDUSD}"}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].takeloan(
            {"vaultId": vault_id, "amounts": f"0.00000100@{self.symbolDUSD}"}
        )
        self.nodes[0].generate(1)
    ...
    def check_minimum_interest_takeloan(self):
        # Set negative interest
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idDUSD}/loan_minting_interest": "-2"}}
        )
        self.nodes[0].generate(1)
        ...
        # Fund vault address
        self.nodes[0].accounttoaccount(
            self.address, {vault_address: f"1@{self.symbolDFI}"}
        )
        self.nodes[0].generate(1)
    ...
    def check_minimum_interest_payback(self):
        ...
        # Take DUSD loan
        self.nodes[0].takeloan(
            {"vaultId": vault_id, "amounts": f"0.00000003@{self.symbolDUSD}"}
        )
        self.nodes[0].generate(1)

        # Check IPB is sub satoshi
        stored_interest = self.nodes[0].getstoredinterest(vault_id, self.symbolDUSD)
        assert_equal(stored_interest["interestPerBlock"], "-0.000000000000005707762557")
        assert_equal(stored_interest["interestToHeight"], "0.000000000000000000000000")
        assert_equal(stored_interest["height"], self.nodes[0].getblockcount())
        ...
        stored_interest = self.nodes[0].getstoredinterest(vault_id, self.symbolDUSD)
        assert_equal(stored_interest["interestPerBlock"], "-0.000000000000003805175038")
        assert_equal(stored_interest["interestToHeight"], "-0.000000000000005707762557")
        assert_equal(stored_interest["height"], self.nodes[0].getblockcount())
        ...
        stored_interest = self.nodes[0].getstoredinterest(vault_id, self.symbolDUSD)
        assert_equal(stored_interest["interestPerBlock"], "-0.000000000000001902587519")
        assert_equal(stored_interest["interestToHeight"], "-0.000000000000009512937595")
        assert_equal(stored_interest["height"], self.nodes[0].getblockcount())
    ...
    def test_withdraw_from_vault(self):
        ...
        self.nodes[0].withdrawfromvault(
            vault_id, vault_address, f"0.5@{self.symbolDFI}"
        )
        self.nodes[0].generate(60)
        ...
        assert_equal(
            attrs["v0/live/economy/negative_interest"],
            [f"3.02739008@{self.symbolDUSD}"],
        )

        # Current total
        assert_equal(
            attrs["v0/live/economy/negative_interest_current"],
            [f"7.51369661@{self.symbolDUSD}"],
        )
        ...
        account = self.nodes[0].getaccount(vault_address)
        assert_equal(
            account, [f"0.50000000@{self.symbolDFI}", f"1.00000000@{self.symbolDUSD}"]
        )
        ...
        account = self.nodes[0].getaccount(vault_address)
        assert_equal(
            account, [f"1.00000000@{self.symbolDFI}", f"1.00000000@{self.symbolDUSD}"]
        )
