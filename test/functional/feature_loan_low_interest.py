...
class LowInterestTest(DefiTestFramework):
    ...
    def test_load_account0_with_DFI(self):
        ...
        self.nodes[0].generate(
            100
        )  # get initial UTXO balance from immature to trusted -> check getbalances()
    ...
    def test_setup_oracles(self):
        ...
        self.nodes[0].generate(1)
        oracle1_prices = [
            {"currency": "USD", "tokenAmount": "0.0001@DOGE"},
    ...
    def test_setup_poolpairs(self):
        ...
        self.nodes[0].addpoolliquidity(
            {
                self.account0: [
                    "1000000@" + self.symboldUSD,
                    "1000000@" + self.symbolDFI,
                ]
            },
            self.account0,
            [],
    ...
    def get_new_vault_and_deposit(self, loan_scheme=None, amount=10000):
        ...
        self.nodes[0].deposittovault(
            vault_id, self.account0, str(amount) + "@DFI"
        )  # enough collateral to take loans
        self.nodes[0].generate(1)
        return vault_id
    ...
    def test_new_loan_with_interest_lower_than_1satoshi(self, payback=False):
        ...
        self.nodes[0].takeloan({"vaultId": vault_id, "amounts": "0.001314@DOGE"})
        self.nodes[0].generate(1)
    ...
    def test_new_loan_with_interest_exactly_25satoshi(self, payback=False):
        vault_id = self.get_new_vault_and_deposit()
        self.nodes[0].takeloan({"vaultId": vault_id, "amounts": "0.1314@DOGE"})
        self.nodes[0].generate(1)
        vault_data = self.nodes[0].getvault(vault_id)
        ...
        # Payback
        self.nodes[0].paybackloan(
            {"vaultId": vault_id, "from": self.account0, "amounts": "0.13140900@DOGE"}
        )
        self.nodes[0].generate(1)
        vault_data = self.nodes[0].getvault(vault_id)
        assert_equal(vault_data["interestAmounts"], [])
    ...
    def test_new_loan_with_interest_over_1satoshi(self, payback=False):
        vault_id = self.get_new_vault_and_deposit()
        self.nodes[0].takeloan({"vaultId": vault_id, "amounts": "100@DOGE"})
        self.nodes[0].generate(1)
        vault_data = self.nodes[0].getvault(vault_id)
    ...
    def test_low_loan(self, payback=False):
        vault_id = self.get_new_vault_and_deposit()
        self.nodes[0].takeloan({"vaultId": vault_id, "amounts": "0.00000001@DOGE"})
        self.nodes[0].generate(1)
        vault_data = self.nodes[0].getvault(vault_id)
        assert_equal(vault_data["interestAmounts"][0], "0.00000001@DOGE")
    ...
    def test_high_loan(self, payback=False):
        vault_id = self.get_new_vault_and_deposit()
        self.nodes[0].takeloan({"vaultId": vault_id, "amounts": "2345.225@DOGE"})
        self.nodes[0].generate(1)
        vault_data = self.nodes[0].getvault(vault_id)
    ...
    def test_1satoshi_loan_pre_post_fork(self, payback=False):
        ...
        # Payback
        self.nodes[0].paybackloan(
            {"vaultId": vault_id, "from": self.account0, "amounts": "0.00000030@DOGE"}
        )
        self.nodes[0].generate(1)
        vault_data = self.nodes[0].getvault(vault_id)
        assert_equal(vault_data["interestAmounts"], [])

    def test_new_loan_with_interest_lower_than_1satoshi_pre_post_fork(
        self, payback=True
    ):
        self.go_before_FCH(30)
        vault_id = self.get_new_vault_and_deposit()
        ...
        # Payback
        self.nodes[0].paybackloan(
            {"vaultId": vault_id, "from": self.account0, "amounts": "0.00131434@DOGE"}
        )
    ...
    def test_new_loan_with_interest_over_1satoshi_pre_post_fork(self, payback=True):
        ...
        # Payback
        self.nodes[0].paybackloan(
            {"vaultId": vault_id, "from": self.account0, "amounts": "200.01331815@DOGE"}
        )
        self.nodes[0].generate(10)
        vault_data = self.nodes[0].getvault(vault_id)
        assert_equal(vault_data["interestAmounts"], [])
