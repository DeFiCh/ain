...
class OraclesTest(DefiTestFramework):
    ...
    def assert_compare_oracle_data(
        self,
        oracle_data_json,
        oracle_id=None,
        oracle_address=None,
        ...
        if weightage is not None:
            if not math.isclose(
                decimal.Decimal(weightage),
                decimal.Decimal(oracle_data_json["weightage"]),
            ):
        ...
        if price_feeds is not None:
            ...
            if not sorted(price_feeds, key=self.cmp_price_feed) == sorted(
                oracle_data_json["priceFeeds"], key=self.cmp_price_feed
            ):
        ...
        if token_prices is not None:
            ...
            if not functools.reduce(
                lambda x, y: x and y,
                map(
                    lambda p, q: p["token"] == q["token"]
                    and p["currency"] == q["currency"]
                    and math.isclose(
                        decimal.Decimal(p["amount"]), decimal.Decimal(q["amount"])
                    ),
                    tpjs,
                    tpps,
                ),
    ...
    def run_test(self):
        ...
        send_money_tx1, send_money_tx2 = [
            self.find_address_tx(self.nodes[2], address)
            for address in [oracle_address1, oracle_address2]
        ]
        ...
        assert_raises_rpc_error(
            -32600,
            "oracle <{}> not found".format(oracle_id2),
            self.nodes[0].removeoracle,
            oracle_id2,
        )
        ...
        self.assert_compare_oracle_data(
        ...
        self.assert_compare_oracle_data(
            oracle_data_json=self.nodes[1].getoracledata(oracle_id2),
        ...
        assert_raises_rpc_error(
            -1,
            "no live oracles for specified request",
            self.nodes[1].getprice,
            {"currency": "USD", "token": "GOLD"},
        )
        assert_raises_rpc_error(
            -1,
        ...
        oracle1_prices = [
            {"currency": "USD", "tokenAmount": "10.1@PT"},
        ...
        oracle_data_json = self.nodes[1].getoracledata(oracle_id1)
        self.assert_compare_oracle_data(
            oracle_data_json=oracle_data_json,
            oracle_id=oracle_id1,
            price_feeds=price_feeds1,
            weightage=10,
            token_prices=oracle1_prices,
        )  # now we have prices
        ...
        assert_equal(
            self.nodes[1].getprice({"currency": "USD", "token": "DUSD"}),
            decimal.Decimal("1.00000000"),
        )
        ...
        self.assert_compare_oracle_data(oracle_data_json=oracle1_data, weightage=15)

        # check only prices
        self.assert_compare_oracle_data(
            oracle_data_json=oracle1_data, price_feeds=price_feeds1
        )

        oracle2_data = self.nodes[1].getoracledata(oracle_id2)

        self.assert_compare_oracle_data(
            oracle_data_json=oracle2_data,
            oracle_id=oracle_id2,
            price_feeds=price_feeds2,
            weightage=15,
        )
        ...
        assert_equal(
            len(
                self.nodes[1].listprices(
                    {"start": 0, "including_start": True, "limit": 100}
                )
            ),
            4,
        )
        assert_equal(
            len(
                self.nodes[1].listprices(
                    {"start": 0, "including_start": False, "limit": 100}
                )
            ),
            3,
        )
        assert_equal(
            len(
                self.nodes[1].listprices(
                    {"start": 1, "including_start": True, "limit": 100}
                )
            ),
            3,
        )
        assert_equal(
            len(
                self.nodes[1].listprices(
                    {"start": 1, "including_start": False, "limit": 2}
                )
            ),
            2,
        )
        ...
        assert_raises_rpc_error(
            -1,
            "start index greater than number of prices available",
            self.nodes[1].listprices,
            {"start": 100, "including_start": False, "limit": 1},
        )

        # === check get prices methods complex case ===

        oracle1_prices = [
            {"currency": "USD", "tokenAmount": "11.5@PT"},
        ...
        ) + 310  # add 5 minutes +10s for slow tests case
        assert_raises_rpc_error(
            -8,
        ...
        self.nodes[2].setoracledata(
            oracle_id3, timestamp - 7200, [{"currency": "USD", "tokenAmount": "7@PT"}]
        )

        self.synchronize(node=2)

        pt_in_usd_raw_prices = self.nodes[1].listlatestrawprices(
            {"currency": "USD", "token": "PT"}
        )
        ...
        assert_raises_rpc_error(
            -20,
            "oracle <{}> not found".format(invalid_oracle_id),
            self.nodes[1].getoracledata,
            invalid_oracle_id,
        )
        ...
        assert_raises_rpc_error(
            ...
            invalid_oracle_id,
            timestamp,
            [{"currency": "USD", "tokenAmount": "10@PT"}],
        )
