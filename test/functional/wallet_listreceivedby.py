...
class ReceivedByTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(len(res), 6 + num_cb_reward_addresses)  # Right now 2 entries
        ...
        res = self.nodes[1].listreceivedbyaddress(0, True, True)
        assert_equal(len(res), 9 + num_cb_reward_addresses)  # Became 3 entries
        ...
        assert_raises_rpc_error(
            -4, "Address not found in wallet", self.nodes[0].getreceivedbyaddress, addr
        )
        ...
        received_by_label_json = [
            r for r in self.nodes[1].listreceivedbylabel() if r["label"] == label
        ][0]
