...
class CommunityDevelopmentFunds(DefiTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            [
                ...
                "-bayfrontheight=60",
                "-eunosheight=70",
                "-fortcanningheight=80",
                "-fortcanninghillheight=90",
                "-fortcanningroadheight=100",
                "-fortcanningcrunchheight=110",
                "-fortcanningspringheight=120",
                "-fortcanninggreatworldheight=130",
                "-grandcentralheight=201",
                "-subsidytest=1",
            ],
            [
                "-dummypos=0",
    ...
    def run_test(self):
        ...
        node0.generate(199)
        assert_equal(
            node0.listcommunitybalances()["CommunityDevelopmentFunds"], Decimal("0")
        )
        ...
        node1.utxostoaccount(
            {"2NCWAKfEehP3qibkLKYQjXaWMK23k4EDMVS": str(balanceLessFee) + "@0"}
        )
        ...
        assert_equal(
            node1.getbalances()["mine"]["immature"], foundation["mine"]["immature"]
        )
        ...
        assert_equal(
            node1.getbalances()["mine"]["immature"],
            foundation["mine"]["immature"] - Decimal("19.887464"),
        )

        # check that funds are not trasfered yet before governance is activated
        assert_equal(
            Decimal(
                node1.getaccount("2NCWAKfEehP3qibkLKYQjXaWMK23k4EDMVS")[0].split("@")[0]
            ),
        ...
        assert_equal(
            node0.listcommunitybalances()["CommunityDevelopmentFunds"],
        ...
        assert_equal(
            before_hardfork + Decimal("19.887464"), after_hardfork + foundationBalance
        )
        ...
        assert_equal(
            node1.listcommunitybalances()["CommunityDevelopmentFunds"],
            balanceLessFee + Decimal("{:.8f}".format(7 * 19.887464)),
        )
        assert_equal(
            node2.listcommunitybalances()["CommunityDevelopmentFunds"],
            balanceLessFee + Decimal("{:.8f}".format(7 * 19.887464)),
        )
        ...
        assert_equal(
            node0.getaccount("mkzZWPwBVgdnwLSmXKW5SuUFMpm6C5ZPcJ"), ["46.24546710@DFI"]
        )
