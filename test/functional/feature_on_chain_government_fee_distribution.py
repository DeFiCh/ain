...
class CFPFeeDistributionTest(DefiTestFramework):
    ...
    def test_cfp_fee_distribution(
        ...
        assert_equal(
            self.nodes[0].getburninfo()["feeburn"], Decimal(expectedFee * burnPct / 100)
        )
        ...
        if changeFeeAndBurnPCT:
            ...
            self.nodes[0].setgov(
                {"ATTRIBUTES": {"v0/gov/proposals/fee_burn_pct": "40%"}}
            )
            self.nodes[0].generate(1)

        expectedAmount = Decimal(expectedFee * (100 - burnPct) / 100 / 3).quantize(
            Decimal("1E-8"), rounding=ROUND_DOWN
        )

        # Calculate cycle
        cycleAlignment = proposalCreationHeight + (
            VOTING_PERIOD - proposalCreationHeight % VOTING_PERIOD
        )
        votingCycles = 0
        for cycle in range(cycles):
            ...
            assert_equal(
                self.nodes[0].gettokenbalances(), [f"{expectedAmount * votingCycles}@0"]
            )
            history = self.nodes[0].listaccounthistory(
                mn0["ownerAuthAddress"], {"txtype": "ProposalFeeRedistribution"}
            )
            ...
            assert_equal(
                self.nodes[1].gettokenbalances(), [f"{expectedAmount * votingCycles}@0"]
            )
            history = self.nodes[0].listaccounthistory(
                mn1["ownerAuthAddress"], {"txtype": "ProposalFeeRedistribution"}
            )
            assert_equal(len(history), votingCycles)
            for i in range(votingCycles):
                assert_equal(history[i]["amounts"][0], "{}@DFI".format(expectedAmount))
            ...
            assert_equal(
                self.nodes[2].gettokenbalances(), [f"{expectedAmount * votingCycles}@0"]
            )
            history = self.nodes[0].listaccounthistory(
                mn2["ownerAuthAddress"], {"txtype": "ProposalFeeRedistribution"}
            )
            assert_equal(len(history), votingCycles)
            for i in range(votingCycles):
                assert_equal(history[i]["amounts"][0], "{}@DFI".format(expectedAmount))
            ...
            history = self.nodes[0].listaccounthistory(
                mn3["ownerAuthAddress"], {"txtype": "ProposalFeeRedistribution"}
            )
    ...
    def run_test(self):
        ...
        self.test_cfp_fee_distribution(
            amount=50, expectedFee=10, burnPct=50, vote="yes"
        )
        self.test_cfp_fee_distribution(
            amount=100, expectedFee=10, burnPct=50, vote="yes"
        )
        self.test_cfp_fee_distribution(
            amount=1000,
            expectedFee=10,
        ...
        self.test_cfp_fee_distribution(
            amount=1000, expectedFee=10, burnPct=50, vote="no", changeFeeAndBurnPCT=True
        )
        ...
        self.test_cfp_fee_distribution(
            amount=1000, expectedFee=10, burnPct=30, vote="neutral"
        )
        ...
        self.test_cfp_fee_distribution(
            amount=1000, expectedFee=20, burnPct=30, vote="yes", cycles=1
        )
        self.test_cfp_fee_distribution(
            amount=1000,
            expectedFee=20,
