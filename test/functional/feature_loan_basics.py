    ...
    def run_test(self):
        ...
        try:
            self.nodes[0].takeloan(
                {"vaultId": setLoanTokenTSLA, "amounts": "1@" + symbolTSLA}
            )
        ...
        try:
            self.nodes[0].takeloan({"vaultId": vaultId, "amounts": "1@" + symbolBTC})
        except JSONRPCException as e:
            errorString = e.error["message"]
        assert "Loan token with id (" + idBTC + ") does not exist" in errorString

        try:
            self.nodes[0].takeloan({"vaultId": vaultId, "amounts": "1@AAAA"})
        except JSONRPCException as e:
            errorString = e.error["message"]
        assert "Invalid Defi token: AAAA" in errorString

        try:
            self.nodes[1].takeloan({"vaultId": vaultId, "amounts": "1@" + symbolTSLA})
        except JSONRPCException as e:
            errorString = e.error["message"]
        assert "Incorrect authorization for" in errorString

        try:
            self.nodes[0].takeloan({"vaultId": vaultId, "amounts": "1@" + symbolTSLA})
        except JSONRPCException as e:
            errorString = e.error["message"]
        assert (
            "Loan cannot be taken on token with id ("
            + idTSLA
            + ') as "mintable" is currently false'
            in errorString
        )

        setLoanTokenTSLA = self.nodes[0].updateloantoken(idTSLA, {"mintable": True})
        ...
        # transfer
        self.nodes[0].addpoolliquidity(
            {account0: ["1@" + symbolTSLA, "300@" + symboldUSD]}, account0, []
        )
        self.nodes[0].addpoolliquidity(
            {account0: ["1@" + symbolGOOGL, "400@" + symboldUSD]}, account0, []
        )
        ...
        assert_equal(
            self.nodes[0].getaccount(account0, {}, True)[idGOOGL], Decimal("2")
        )
        ...
        try:
            self.nodes[0].paybackloan(
                {"vaultId": vaultId2, "from": account0, "amounts": "0.5@" + symbolTSLA}
            )
        except JSONRPCException as e:
            errorString = e.error["message"]
        ...
        assert_equal(
            sorted(vaultInfo["interestAmounts"]),
            sorted(["0.00001064@GOOGL", "0.00000456@TSLA"]),
        )
        assert_equal(
            self.nodes[0].getaccount(account0, {}, True)[idTSLA], Decimal("1.00000000")
        )
        assert_equal(
            self.nodes[0].getaccount(account0, {}, True)[idGOOGL], Decimal("2.00000000")
        )
        ...
        assert_equal(
            sorted(self.nodes[0].listaccounthistory(account0)[0]["amounts"]),
            sorted(["-1.00001064@GOOGL", "-0.50000456@TSLA"]),
        )
        assert_equal(
            self.nodes[0].getaccount(account0, {}, True)[idTSLA], Decimal("0.49999544")
        )
        assert_equal(
            self.nodes[0].getaccount(account0, {}, True)[idGOOGL], Decimal("0.99998937")
        )
        ...
        vaultInfo = self.nodes[0].getvault(vaultId)
        assert_equal(
            self.nodes[0].getburninfo()["paybackburn"], ["0.00186822@" + symbolDFI]
        )
        assert_equal(
            sorted(vaultInfo["loanAmounts"]),
        ...
        self.nodes[0].minttokens(
            ["0.00001083@" + symbolTSLA, "0.00002659@" + symbolGOOGL]
        )
        ...
        self.nodes[0].paybackloan(
            {"vaultId": vaultId, "from": account0, "amounts": vaultInfo["loanAmounts"]}
        )
        ...
        assert_greater_than_or_equal(
            self.nodes[0].getburninfo()["paybackburn"], ["0.00443685@" + symbolDFI]
        )
        ...
        self.nodes[0].paybackloan(
            {"vaultId": vaultId1, "from": account0, "amounts": "500@" + symboldUSD}
        )
        ...
        # take loan
        self.nodes[0].takeloan(
            {"vaultId": vaultId3, "amounts": ["10@TSLA", "10@GOOGL"]}
        )
        self.nodes[0].generate(1)

        address2 = self.nodes[0].getnewaddress()
        self.nodes[0].sendtokenstoaddress(
            {}, {address2: ["5@TSLA", "5@GOOGL"]}
        )  # split into two address
        self.nodes[0].generate(1)
        ...
        self.nodes[0].paybackloan(
            {"vaultId": vaultId3, "from": "*", "amounts": "5@" + symbolTSLA}
        )
        self.nodes[0].generate(1)

        vault = self.nodes[0].getvault(vaultId3)
        assert_equal(
            sorted(vault["loanAmounts"]),
            sorted(["5.00002853@" + symbolTSLA, "10.00003993@" + symbolGOOGL]),
        )

        self.nodes[0].sendtokenstoaddress(
            {}, {address2: ["5@" + symbolTSLA, "10@" + symbolGOOGL]}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            sorted(vault["loanAmounts"]),
            sorted(["0.00003425@" + symbolTSLA, "0.00005324@" + symbolGOOGL]),
        )
        ...
        # take loan
        self.nodes[0].takeloan({"vaultId": vaultId4, "amounts": "0.00000001@TSLA"})
        self.nodes[0].generate(1)
        ...
        self.nodes[0].paybackloan(
            {
                ...
                "amounts": vault["loanAmounts"][
                    0
                ],  # Has to pay whole loanAmounts to be able to close vault
            }
        )
        self.nodes[0].generate(1)
