...
class PriceUpdateTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": vaultId1, "amounts": str(loanAmount) + "@TSLA"}
        )
        self.nodes[0].generate(1)
        ...
        try:
            self.nodes[0].paybackloan(
                {"vaultId": vaultId1, "from": account, "amounts": ["0.5@TSLA"]}
            )
        except JSONRPCException as e:
            errorString = e.error["message"]
        assert (
        ...
        self.nodes[0].generate(1)
        vaultBeforeUpdate = self.nodes[0].getvault(vaultId1, True)
        expectedInterestAfterOneBlock = Decimal(amountInterestTSLA) + Decimal(
            interestPerBlockTSLA
        )
        realInterestAfterOneBlock = Decimal(
            vaultBeforeUpdate["interestAmounts"][0].split("@")[0]
        )
        ...
        assert_equal(
            vaultAfterUpdate["collateralRatio"],
            vaultBeforeUpdate["nextCollateralRatio"],
        )
        ...
        realInterestAfterOneBlock = Decimal(
            vaultAfterUpdate["interestAmounts"][0].split("@")[0]
        )
        assert_equal(realInterestAfterOneBlock, expectedInterestAfterOneBlock)
        ...
        vaultAfterUpdate = self.nodes[0].getvault(vaultId1, True)
        expectedInterestAfterOneBlock = Decimal(
            Decimal(amountInterestTSLA) + Decimal(interestPerBlockTSLA)
        ).quantize(Decimal("0.00000001"), rounding=ROUND_DOWN)
        realInteresAfterOneBlock = Decimal(
            vaultAfterUpdate["interestAmounts"][0].split("@")[0]
        )
