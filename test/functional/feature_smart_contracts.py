...
class SmartContractTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(
            -5,
            "Incorrect authorization for {}".format(invalid_address),
            self.nodes[0].executesmartcontract,
            dfip,
            str(btc_amount) + "@2",
            invalid_address,
        )
        ...
        assert_raises_rpc_error(
            -8,
            "Specified smart contract not found",
            self.nodes[0].executesmartcontract,
            "DFIP9999",
            str(dfi_amount) + "@0",
        )
        assert_raises_rpc_error(
            -8,
            "BTC source address must be provided for DFIP2201",
            self.nodes[0].executesmartcontract,
            dfip,
            str(btc_amount) + "@2",
        )
        ...
        self.nodes[0].accounttoaccount(
            self.nodes[0].get_genesis_keys().ownerAuthAddress, {address: "20000@BTC"}
        )
        self.nodes[0].generate(1)
        ...
        assert_raises_rpc_error(
            -32600,
            "called before FortCanningHill height",
            self.nodes[0].executesmartcontract,
            dfip,
            str(btc_amount) + "@2",
            address,
        ...
        assert_raises_rpc_error(
            -32600,
            "DFIP2201 smart contract is not enabled",
            self.nodes[0].executesmartcontract,
            dfip,
            str(btc_amount) + "@2",
            address,
        ...
        assert_raises_rpc_error(
            -32600,
            "Only Bitcoin can be swapped in DFIP2201",
            self.nodes[0].executesmartcontract,
            dfip,
            str(btc_amount) + "@1",
            address,
        ...
        assert_raises_rpc_error(
            -32600,
            "fixedIntervalPrice with id <BTC/USD> not found",
            self.nodes[0].executesmartcontract,
            dfip,
            str(btc_amount) + "@2",
            address,
        ...
        assert_raises_rpc_error(
            -32600,
            "Below minimum swapable amount, must be at least 0.00001000 BTC",
            self.nodes[0].executesmartcontract,
            dfip,
            "0.00000999@2",
            address,
        ...
        self.nodes[0].importprivkey(
            "cMv1JaaZ9Mbb3M3oNmcFvko8p7EcHJ8XD7RCQjzNaMs7BWRVZTyR"
        )
        ...
        assert_equal(
            result[0]["address"], "bcrt1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqdku202"
        )
        ...
        block = self.nodes[0].getblock(
            self.nodes[0].getblockhash(self.nodes[0].getblockcount() - 100)
        )
        ...
        assert_equal(
            balance + staker_reward + community_reward - Decimal("18336.225") + fee,
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/params/dfip2201/premium": "0.00000000"}}
        )
        self.nodes[0].generate(1)
        assert_raises_rpc_error(
            -32600,
            "amount 18336.22500000 is less than 18336.22500001",
            self.nodes[0].executesmartcontract,
            dfip,
            "18336.22500001@2",
            address,
        ...
        self.nodes[0].generate(1)
        assert_equal(
            18336.22500000, float(self.nodes[0].getaccount(address)[0].split("@")[0])
        )
        ...
        prices = [
            {"currency": "USD", "tokenAmount": "2@DFI"},
        ...
        self.nodes[0].generate(1)
        assert_equal(
            2049.999795, float(self.nodes[0].getaccount(address)[0].split("@")[0])
        )
        ...
        self.nodes[0].generate(1)
        assert_equal(
            2099.99979, float(self.nodes[0].getaccount(address)[0].split("@")[0])
        )
        ...
        self.nodes[0].generate(1)
        assert_equal(
            2001.9997998, float(self.nodes[0].getaccount(address)[0].split("@")[0])
        )

        # Test 0.000001% premium
        self.rollback_to(block)
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/params/dfip2201/premium": "0.00000001"}}
        )
        self.nodes[0].generate(1)
        self.nodes[0].executesmartcontract(dfip, "0.1@2", address)
        self.nodes[0].generate(1)
        assert_equal(
            2000.00002, float(self.nodes[0].getaccount(address)[0].split("@")[0])
        )
        ...
        self.rollback_to(block)
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/params/dfip2201/minswap": "0.00000001"}}
        )
        self.nodes[0].generate(1)
        self.nodes[0].executesmartcontract(dfip, "0.00000001@2", address)
        self.nodes[0].generate(1)
        assert_equal(
            0.000205, float(self.nodes[0].getaccount(address)[0].split("@")[0])
        )
