...
class EstimateCollateralTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].setcollateraltoken(
            {"token": idBTC, "factor": 0.8, "fixedIntervalPriceId": "BTC/USD"}
        )
        ...
        try:
            ...
            errorString = e.error["message"]
        assert (
            "total split between collateral tokens = 0.80000000 vs expected 1.00000000"
            in errorString
        )
        ...
        self.nodes[0].takeloan(
            {"vaultId": vaultId1, "amounts": "10@TSLA"}
        )  # should be able to take loan amount from estimatecollateral
        self.nodes[0].generate(1)

        vault1 = self.nodes[0].getvault(vaultId1)
        assert_equal(
            vault1["collateralRatio"], 200
        )  # vault collateral ratio should be equal to estimatecollateral targetRatio

        vaultId2 = self.nodes[0].createvault(ownerAddress1)
        estimatecollateral = self.nodes[0].estimatecollateral(
            "10@TSLA", 200, {"BTC": 0.5, "DFI": 0.5}
        )
        ...
        estimatecollateral = self.nodes[0].estimatecollateral(
            ["10@TSLA", "10@TWTR"], 200, {"BTC": 0.5, "DFI": 0.5}
        )
