...
class MultiWalletTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            sorted(map(lambda w: w["name"], self.nodes[0].listwalletdir()["wallets"])),
            [
                "",
                os.path.join("sub", "w5"),
                "w",
        ...
        wallets = sorted(
            map(lambda w: w["name"], self.nodes[0].listwalletdir()["wallets"])
        )
        assert_equal(
            wallets,
            [
        ...
        assert_raises_rpc_error(
            -18, "Wallet wallets not found.", self.nodes[0].loadwallet, "wallets"
        )
        ...
        assert_raises_rpc_error(
            -4, "Wallet w2 already exists.", self.nodes[0].createwallet, "w2"
        )
