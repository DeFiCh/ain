...
class MigrateV1Test(DefiTestFramework):
    ...
    def test_migration_on_fork(self):
        ...
        [current_amount, current_symbol] = (
            self.nodes[0]
            .getgov("ATTRIBUTES")["ATTRIBUTES"]["v0/live/economy/dfip2203_current"][1]
            .split("@")
        )
        [burnt_amount, burnt_symbol] = (
            self.nodes[0]
            .getgov("ATTRIBUTES")["ATTRIBUTES"]["v0/live/economy/dfip2203_burned"][1]
            .split("@")
        )
        [minted_amount, minted_symbol] = (
            self.nodes[0]
            .getgov("ATTRIBUTES")["ATTRIBUTES"]["v0/live/economy/dfip2203_minted"][1]
            .split("@")
        )
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/oracles/splits/{self.nodes[0].getblockcount() + 2}": f"{self.idTSLA}/{multiplier}"
                }
            }
        )
        ...
        assert_equal(
            Decimal(current_amount) * multiplier, Decimal(current_split_amount)
        )
