...
class FoundationMemberTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-32600, "Cannot be set before GrandCentralHeight", self.nodes[0].setgov,
                                {"ATTRIBUTES": {'v0/params/foundation/members': [address]}})
        assert_raises_rpc_error(-32600, "Cannot be set before GrandCentralHeight", self.nodes[0].setgov,
        ...
        assert_equal(self.nodes[0].getgov('ATTRIBUTES')['ATTRIBUTES']['v0/params/foundation/members'],
                     ["bcrt1qyrfrpadwgw7p5eh3e9h3jmu4kwlz4prx73cqny", "bcrt1qyeuu9rvq8a67j86pzvh5897afdmdjpyankp4mu",
                      "msER9bmJjyEemRpQoS8YYVL21VyZZrSgQ7", "mwsZw8nF7pKxWH8eoKL9tPxTpaFkz7QeLU"])

        # Add already existant address
        assert_raises_rpc_error(-32600, "Member to add already present", self.nodes[0].setgov,
                                {"ATTRIBUTES": {'v0/params/foundation/members': [address]}})

        # Remove an address that is not present
        assert_raises_rpc_error(-32600, "Member to remove not present", self.nodes[0].setgov,
                                {"ATTRIBUTES": {'v0/params/foundation/members': [f"-{self.nodes[0].getnewaddress()}"]}})

        # Add nonsense address
        assert_raises_rpc_error(-5, "Invalid address provided", self.nodes[0].setgov,
                                {"ATTRIBUTES": {'v0/params/foundation/members': ["NOTANADDRESS"]}})
        ...
        assert_raises_rpc_error(-32600, "tx not from foundation member", self.nodes[0].setgov,
                                {"ATTRIBUTES": {'v0/params/foundation/members': [address]}})
        ...
        self.nodes[0].setgov({"ATTRIBUTES": {
            'v0/params/foundation/members': [f"-{address}", "+2MwHamtynMqvstggG5XsVPVoAKroa4CgwFs"],
        ...
        assert_equal(attributes['v0/params/foundation/members'],
                     ["bcrt1qyrfrpadwgw7p5eh3e9h3jmu4kwlz4prx73cqny", "bcrt1qyeuu9rvq8a67j86pzvh5897afdmdjpyankp4mu",
                      "2MwHamtynMqvstggG5XsVPVoAKroa4CgwFs", "msER9bmJjyEemRpQoS8YYVL21VyZZrSgQ7"])
        assert_equal(attributes['v0/params/dfip2201/active'], 'false')

        # Set stored lock
        activation_height = self.nodes[1].getblockcount() + 5
        self.nodes[1].setgovheight({"ATTRIBUTES": {
            'v0/params/foundation/members': ["-2MwHamtynMqvstggG5XsVPVoAKroa4CgwFs",
                                             "2N3kCSytvetmJybdZbqV5wK3Zzekg9SiSe2"],
            'v0/params/dfip2203/active': 'false'}}, activation_height)
        self.nodes[1].generate(1)

        # Check pending changes show as expected
        attributes = self.nodes[1].listgovs()[8]
        assert_equal(attributes[1][str(activation_height)]['v0/params/foundation/members'],
                     ["-2MwHamtynMqvstggG5XsVPVoAKroa4CgwFs", "2N3kCSytvetmJybdZbqV5wK3Zzekg9SiSe2"])
        ...
        assert_equal(attributes['v0/params/foundation/members'],
                     ["bcrt1qyrfrpadwgw7p5eh3e9h3jmu4kwlz4prx73cqny", "bcrt1qyeuu9rvq8a67j86pzvh5897afdmdjpyankp4mu",
                      "2N3kCSytvetmJybdZbqV5wK3Zzekg9SiSe2", "msER9bmJjyEemRpQoS8YYVL21VyZZrSgQ7"])
