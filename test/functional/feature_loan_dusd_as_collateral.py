...
class LoanDUSDCollateralTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].addpoolliquidity(
            {mn_address: ["10000@" + symbolDFI, "8000@" + symbolDUSD]}, mn_address
        )
        self.nodes[0].addpoolliquidity(
            {mn_address: ["10000@" + symbolDFI, "8000@" + symbolBTC]}, mn_address
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].setcollateraltoken(
            {"token": symbolDFI, "factor": 1, "fixedIntervalPriceId": "DFI/USD"}
        )

        self.nodes[0].setcollateraltoken(
            {"token": symbolBTC, "factor": 1, "fixedIntervalPriceId": "BTC/USD"}
        )
        ...
        self.nodes[0].accounttoaccount(
            mn_address, {vault_address: str(collateral) + "@" + symbolDUSD}
        )
        self.nodes[0].accounttoaccount(
            mn_address, {vault_address: str(collateral) + "@" + symbolDFI}
        )
        self.nodes[0].accounttoaccount(
            mn_address, {vault_address: str(collateral) + "@" + symbolBTC}
        )
        self.nodes[0].generate(1)
        ...
        assert_raises_rpc_error(
            -32600,
            "Collateral token with id (1) does not exist!",
            self.nodes[0].deposittovault,
            vault_id,
            vault_address,
            str(collateral) + "@" + symbolDUSD,
        )
        ...
        self.nodes[0].deposittovault(
            vault_id, vault_address, str(collateral) + "@" + symbolDFI
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            vault["collateralValue"], collateral * token_factor_dusd + collateral
        )
        ...
        self.nodes[0].takeloan(
            {"vaultId": vault_id, "amounts": str(loan_dusd) + "@" + symbolDUSD}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            vault["collateralAmounts"], ["2000.00000000@DFI", "2000.00000000@DUSD"]
        )
        ...
        self.nodes[0].generate(
            215 - self.nodes[0].getblockcount()
        )  # move to fortcanningroad height
        ...
        assert_raises_rpc_error(
            ...
            self.nodes[0].takeloan,
            {"vaultId": vault_id, "amounts": "333@" + symbolDUSD},
        )

        # Set DUSD collateral factor back to 1
        self.nodes[0].setcollateraltoken(
            {"token": symbolDUSD, "factor": 1, "fixedIntervalPriceId": "DUSD/USD"}
        )
