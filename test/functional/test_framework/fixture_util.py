    ...
    def setup_default_tokens(test: DefiTestFramework, my_tokens=None):
        ...
        if my_tokens is not None:
            ...
            for token in my_tokens:
                ...
                token["wallet"].minttokens(
                    str(token["amount"]) + "@" + token["symbolId"]
                )
            ...
            test.nodes[0].createtoken(
                {
                    "symbol": "GOLD",
                    "name": "shiny gold",
                    "collateralAddress": test.nodes[0]
                    .get_genesis_keys()
                    .ownerAuthAddress,  # collateralGold
                }
            )
            test.nodes[0].generate(1)
            test.sync_blocks()
            test.nodes[1].createtoken(
                {
                    "symbol": "SILVER",
                    "name": "just silver",
                    "collateralAddress": test.nodes[1]
                    .get_genesis_keys()
                    .ownerAuthAddress,  # collateralSilver
                }
            )
