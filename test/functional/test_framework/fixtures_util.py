...
from .nodes_util import get_id_token
...
def setup_default_tokens(test : DefiTestFramework, my_tokens=None):
    ...
    if my_tokens is not None:
        ...
        # mint tokens
        for token in my_tokens:
            token["tokenId"] = get_id_token(test.nodes, token["symbol"])
        ...
        symbolGOLD = "GOLD#" + get_id_token(test.nodes, "GOLD")
        symbolSILVER = "SILVER#" + get_id_token(test.nodes, "SILVER")
