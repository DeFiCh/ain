...
class TokenAmount:
    def __init__(self, number_with_symbol: str = None, number: Decimal = None, symbol: str = None):
        if number_with_symbol:
            if not self._validate(number_with_symbol):
                raise ValueError("String must be in the format NumberWith8DigitPrecision@SYMBOL")
            ...
            if not self._validate_number(number):
                raise ValueError("Number must have up to 8 digit precision after the decimal point")
            self.number_with_symbol = f"{number:.8f}@{symbol}"
        else:
            raise ValueError("Invalid arguments. Provide either a single string or number and symbol.")
