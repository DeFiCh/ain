...
def bitflipper(expr):
    ...
    def fn(ctx):
        ...
        return (
            int.from_bytes(sub, "little") ^ (1 << random.randrange(len(sub) * 8))
...
def spenders_taproot_active():
        ...
        failure={
            "sighash": override(
                ...
                leafversion=random.choice(
                    [x & 0xFE for x in range(0x100) if x & 0xFE != 0xC0]
                ),
            )
        },
        **ERR_SIG_SCHNORR
    ...
    PARTNER_MERKLE_FN = [
        ...
        lambda h: (int.from_bytes(h, "little") ^ (1 << random.randrange(256))).to_bytes(
            32, "little"
        ),
    ]
    ...
    add_spender(
        ...
        **SINGLE_SIG,
        key=secs[0],
        failure={"pubkey_inner": bitflipper(default_pubkey_inner)},
        **ERR_WITNESS_PROGRAM_MISMATCH
    )
    ...
    csa_low_val = random.randrange(0, 16)  # Within range for OP_n
    ...
    scripts = [
        ...
        (
            "t4",
            CScript([OP_0, OP_SWAP, OP_1, pubs[1], OP_1, OP_CHECKMULTISIGVERIFY, OP_1]),
        ),
        ...
        (
            "t9",
            CScript([csa_low_val, pubs[1], OP_CHECKSIGADD, csa_low_result, OP_EQUAL]),
        ),
        ...
        (
            "t14",
            CScript(
                [pubs[1], OP_CHECKSIGVERIFY, OP_0, pubs[0], OP_CHECKSIGADD, OP_NOT]
            ),
        ),
        # 15) OP_CHECKSIGADD with empty key that needs invalid signature
        (
            "t15",
            CScript([pubs[1], OP_CHECKSIGVERIFY, OP_0, OP_0, OP_CHECKSIGADD, OP_NOT]),
        ),
        ...
        (
            "t20",
            CScript([pubs[1], OP_CHECKSIGVERIFY, OP_0, OP_0, OP_CHECKSIGVERIFY, OP_1]),
        ),
        # 21) Script that grows the stack to 1000 elements
        (
            "t21",
            CScript(
                [pubs[1], OP_CHECKSIGVERIFY, OP_1] + [OP_DUP] * 999 + [OP_DROP] * 999
            ),
        ),
        ...
        (
            "t22",
            CScript(
                [pubs[1], OP_CHECKSIGVERIFY, OP_1] + [OP_DUP] * 1000 + [OP_DROP] * 1000
            ),
        ),
        ...
        (
            "t25",
            CScript(
                [random_bytes(MAX_SCRIPT_ELEMENT_SIZE), OP_DROP, pubs[1], OP_CHECKSIG]
            ),
        ),
        ...
        (
            "t28",
            CScript([csa_high_val, pubs[1], OP_CHECKSIGADD, csa_high_result, OP_EQUAL]),
        ),
    ...
    add_spender(
        ...
        **common,
        ...
        failure={"inputs": [getter("sign"), random_bytes(MAX_SCRIPT_ELEMENT_SIZE + 1)]},
        **ERR_PUSH_LIMIT
    )
    add_spender(
        spenders,
    ...
    SIGOPS_RATIO_SCRIPTS = [
        ...
        lambda n, pk: (
            CScript([OP_DROP, pk] + [OP_2DUP, OP_CHECKSIGVERIFY] * n + [OP_CHECKSIG]),
            n + 1,
        ),
    ...
    for annex in [None, bytes([ANNEX_TAG]) + random_bytes(random.randrange(1000))]:
        for hashtype in [SIGHASH_DEFAULT, SIGHASH_ALL]:
            for pubkey in [
                pubs[1],
                random_bytes(random.choice([x for x in range(2, 81) if x != 32])),
            ]:
                for fn_num, fn in enumerate(SIGOPS_RATIO_SCRIPTS):
                    ...
                    add_spender(
                        ...
                        standard=standard,
                        failure={
                            "inputs": [getter("sign"), random_bytes(dummylen - 1)]
                        },
                        **ERR_SIGOPS_RATIO
                    )
    ...
    for opval in range(76, 0x100):
        ...
        scripts = [
            ...
            (
                "bigpush_success",
                CScript([random_bytes(MAX_SCRIPT_ELEMENT_SIZE + 1), OP_DROP, opcode]),
            ),
            (
                "bigpush_nop",
                CScript([random_bytes(MAX_SCRIPT_ELEMENT_SIZE + 1), OP_DROP, OP_NOP]),
            ),
            ...
            for witv0 in [False, True]:
                for hashtype in VALID_SIGHASHES_ECDSA + [
                    ...
                    add_spender(
                        ...
                        witv0=witv0,
                        standard=standard,
                        script=CScript([pubkey1, OP_CHECKSIG]),
                        **SINGLE_SIG,
                        key=eckey1,
                        failure={"key": eckey2},
...
def spenders_taproot_inactive():
    ...
    add_spender(
        spenders,
        "inactive/scriptpath_invalidsig",
        key=sec,
        tap=tap,
        leaf="pk",
        standard=False,
    ...
    add_spender(
        spenders,
        "inactive/scriptpath_invalidcb",
        key=sec,
        tap=tap,
        leaf="pk",
        standard=False,
        inputs=[getter("sign")],
        controlblock=bitflipper(default_controlblock),
    )
    add_spender(
        spenders,
...
def dump_json_test(tx, input_utxos, idx, success, failure):
    ...
    def dump_witness(wit):
        return OrderedDict(
            [("scriptSig", wit[0].hex()), ("witness", [x.hex() for x in wit[1]])]
        )
...
class TaprootTest(DefiTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [
            ["-whitelist=127.0.0.1", "-par=1"],
            ["-whitelist=127.0.0.1", "-par=1", "-df23height=1"],
        ]
    ...
    def block_submit(
        ...
        extra_output_script = CScript(
            [OP_CHECKSIG]
            * ((MAX_BLOCK_SIGOPS_WEIGHT - sigops_weight) // WITNESS_SCALE_FACTOR)
        )
    ...
    def test_spenders(self, node, spenders, input_counts):
        ...
        while done < len(spenders):
            ...
            change_goal = (
                (1.0 - 0.6 * next_progress) / (1.0 - 0.6 * cur_progress) * balance
            )
            self.log.debug(
            ...
            for i in range(count_this_tx):
                ...
                amount = int(
                    random.randrange(int(avg * 0.85 + 0.5), int(avg * 1.15 + 0.5)) + 0.5
                )
                balance -= amount
            ...
            ss = BytesIO(
                bytes.fromhex(node.signrawtransactionwithwallet(ToHex(fund_tx))["hex"])
            )
        ...
        while left:
            ...
            acceptable = [
                ...
                if n <= left
                and (left - n > max(input_counts) or (left - n) in [0] + input_counts)
            ]
            ...
            self.log.debug(
                "Test: %s" % (", ".join(utxo.spender.comment for utxo in input_utxos))
            )
            ...
            num_outputs = random.choice(
                range(
                    ...
                    1
                    + min(
                        4, 4 if first_mismatch_input is None else first_mismatch_input
                    ),
                )
            )
            ...
            for fail_input in list(range(len(input_utxos))) + [None]:
                ...
                is_standard_tx = (
                    fail_input is None
                    and all(utxo.spender.is_standard for utxo in input_utxos)
                    and tx.nVersion >= 1
                    and tx.nVersion <= 2
                )
                tx.rehash()
                ...
                if is_standard_tx:
                    ...
                    assert_raises_rpc_error(
                        -26, None, node.sendrawtransaction, tx.serialize().hex(), 0
                    )
                # Submit in a block
                self.block_submit(
                    node,
    ...
    def run_test(self):
        ...
        self.test_spenders(
            self.nodes[1], spenders_taproot_active(), input_counts=[1, 2, 2, 2, 2, 3]
        )

        # Transfer % of funds to pre-taproot node.
        addr = self.nodes[0].getnewaddress()
        self.nodes[1].sendtoaddress(
            address=addr, amount=int(self.nodes[1].getbalance() * 70000000) / 100000000
        )
        ...
        while block_count == self.nodes[1].getblockcount():
            self.nodes[1].setmocktime(mint_time)
            mint_time = mint_time + 1
            self.nodes[1].generatetoaddress(1, address, 1)
        self.sync_blocks()

        # Pre-taproot activation tests.
        self.log.info("Pre-activation tests...")
        self.test_spenders(
            self.nodes[0], spenders_taproot_inactive(), input_counts=[1, 2, 2, 2, 2, 3]
        )
