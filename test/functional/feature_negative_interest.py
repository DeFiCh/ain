...
class NegativeInterestTest(DefiTestFramework):
    ...
    def vault_interest_zero(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId0, "amounts": "1@" + self.symboldUSD}
        )
        self.nodes[0].generate(1)
    ...
    def vault_interest_over_zero_and_lower_than_loanscheme(self):
        ...
        # Take loan
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId1, "amounts": "1@" + self.symboldUSD}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(vault["interestPerBlockValue"]),
            Decimal("0.000000009512937595129375"),
        )
    ...
    def vault_loantoken_interest_zero(self):
        ...
        # Take loan
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId2, "amounts": "1@" + self.symbolTSLA}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(vault["interestPerBlockValue"]),
            Decimal("0.000000190258751902587510"),
        )
    ...
    def vault_with_negative_interest(self):
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idTSLA}/loan_minting_interest": "-3"}}
        )
        self.nodes[0].generate(1)
        # Init vault (create, deposit takeloan)
        # Create
        ...
        # Take loan
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId3, "amounts": "1@" + self.symbolTSLA}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(vault["interestPerBlockValue"]), Decimal("-1.90258751902587510E-7")
        )
    ...
    def vault_with_big_negative_interest(self):
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idTSLA}/loan_minting_interest": "-30"}}
        )
        self.nodes[0].generate(1)
        # Init vault (create, deposit takeloan)
        # Create
        ...
        # Take loan
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId4, "amounts": "1@" + self.symbolTSLA}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(vault["interestPerBlockValue"]),
            Decimal("-0.000002758751902587519020"),
        )
    ...
    def takeloan_after_interest_increase(self):
        ...
        expected_IPB = Decimal(
            Decimal("0.31") / Decimal(1051200) * totalLoanAmount
    ...
    def takeloan_after_interest_decrease(self):
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idTSLA}/loan_minting_interest": "-50"}}
        )
        self.nodes[0].generate(1)
        loanAmount = 1
        totalLoanAmount += loanAmount
        ...
        assert_equal(
            Decimal(vault["interestPerBlockValue"]),
            Decimal("-0.000009322675393835616430"),
        )
    ...
    def payback_interest_zero(self):
        ...
        assert_equal(
            Decimal(vault["interestPerBlockValue"]),
            Decimal("-0.000004661331637176560120"),
        )
        ...
        assert_equal(
            Decimal(loanAmount),
            Decimal(loanAmountBefore) - Decimal("1") + Decimal(interestAmount),
        )

        [balanceTSLAafter, _] = self.nodes[0].getaccount(self.account0)[2].split("@")
        assert_equal(
            Decimal(balanceTSLAafter), Decimal(balanceTSLAbefore) - Decimal("1")
        )
    ...
    def payback_after_interest_increase(self):
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idTSLA}/loan_minting_interest": "1"}}
        )
        self.nodes[0].generate(1)
    ...
    def check_interest_amounts_after_interest_changes(self):
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idTSLA}/loan_minting_interest": "0"}}
        )
        self.nodes[0].generate(1)
    ...
    def updates_of_vault_scheme(self):
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idTSLA}/loan_minting_interest": "-2"}}
        )
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId6, "amounts": "1@" + self.symbolTSLA}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(loanAmount),
            Decimal(loanAmountBefore) - Decimal("0.1") + Decimal(interestAmount),
        )

        [balanceTSLAafter, _] = self.nodes[0].getaccount(self.account0)[2].split("@")
        assert_equal(
            Decimal(balanceTSLAafter), Decimal(balanceTSLAbefore) - Decimal("0.1")
        )
    ...
    def total_interest_with_multiple_takeloan(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId7, "amounts": "1@" + self.symbolTSLA}
        )
        self.nodes[0].generate(10)
        vault = self.nodes[0].getvault(self.vaultId7, verbose)
        assert_equal(vault["interestPerBlockValue"], "0.000000000000000000000000")
        ...
        # Take GOLD loan
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId7, "amounts": "1@" + self.symbolGOLD}
        )
        self.nodes[0].generate(1)
        vault = self.nodes[0].getvault(self.vaultId7, verbose)
        assert_equal(vault["interestPerBlockValue"], "0.000000380517503805175030")
        assert_equal(
            vault["interestsPerBlock"],
            ["0.000000038051750380517503@TSLA", "0.000000000000000000000000@GOLD"],
        )
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idGOLD}/loan_minting_interest": "-1"}}
        )
        self.nodes[0].generate(1)
        vault = self.nodes[0].getvault(self.vaultId7, verbose)
        ...
        assert_equal(
            vault["interestsPerBlock"],
            ["0.000000038051750380517503@TSLA", "0.000000019025875190258751@GOLD"],
        )
    ...
    def vault_status_with_negative_interest(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId7, "amounts": "0.43839516@" + self.symbolGOLD}
        )
        self.nodes[0].generate(1)
        verbose = True
        vault = self.nodes[0].getvault(self.vaultId7, verbose)
        ...
        self.nodes[0].generate(1)
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idGOLD}/loan_minting_interest": "-3"}}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idTSLA}/loan_minting_interest": "-10"}}
        )
        self.nodes[0].generate(1)
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idGOLD}/loan_minting_interest": "-10"}}
        )
        self.nodes[0].generate(1)
    ...
    def payback_interests_and_continue_with_negative_interest(self):
        ...
        assert_equal(
            Decimal(loanAmount),
            Decimal(loanAmountBefore) - Decimal("0.00001000") + Decimal(interestAmount),
        )

        [balanceTSLAafter, _] = self.nodes[0].getaccount(self.account0)[2].split("@")
        assert_equal(
            Decimal(balanceTSLAafter),
            Decimal(balanceTSLAbefore) - Decimal("0.00001000"),
        )
    ...
    def let_loan_be_paid_by_negative_interests(self):
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.iddUSD}/loan_minting_interest": "-1000000"
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        # Take loan
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId8, "amounts": "5@" + self.symboldUSD}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId8, "amounts": "0.00000001@" + self.symboldUSD}
        )
        self.nodes[0].generate(1)
        storedLoans1 = self.nodes[0].getloantokens(self.vaultId8)
        storedAmount1 = get_decimal_amount(storedLoans1[0])
        storedInterest1 = self.nodes[0].getstoredinterest(
            self.vaultId8, self.symboldUSD
        )

        assert storedAmount > storedAmount1
        expectedAmount = Decimal(
            Decimal("5")
            + 11 * Decimal(storedInterest["interestPerBlock"])
            + Decimal("0.00000001")
        ).quantize(Decimal("1E-8"), ROUND_UP)
    ...
    def various_payback_tests(self):
        ...
        # Take loan
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId9, "amounts": "10@" + self.symboldUSD}
        )
        self.nodes[0].generate(10)
        ...
        assert_equal(
            ...
            Decimal(balanceDUSDbefore) - Decimal("9.04870720"),
        )
        ...
        accountInfo = self.nodes[0].getaccount(self.account)
        assert_equal(
            get_decimal_amount(accountInfo[0]),
            Decimal("10.00000000") + Decimal(balanceDUSDafter),
        )
        vault = self.nodes[0].getvault(self.vaultId9, verbose)
        assert_equal(vault["loanValue"], Decimal("9.04870720"))
        assert_equal(vault["interestPerBlockValue"], "-0.095129280821917808219178")
        ...
        assert_equal(
            Decimal(loanAmount),
            Decimal(loanAmountBefore) - Decimal("0.00000001") + Decimal(interestAmount),
        )
        ...
        expectedAmount = Decimal(
            Decimal("10")
            + 10 * Decimal(storedInterest["interestPerBlock"])
            - Decimal("0.00000001")
        ).quantize(Decimal("1E-8"), ROUND_UP)
        ...
        assert_equal(
            Decimal(balanceDUSDafter),
            Decimal(balanceDUSDbefore) - Decimal("0.00000001"),
        )
        ...
        assert_equal(
            Decimal(loanAmount),
            Decimal(loanAmountBefore) - Decimal("1.00000000") + Decimal(interestAmount),
        )

        [balanceDUSDafter, _] = self.nodes[0].getaccount(self.account)[0].split("@")
        assert_equal(
            Decimal(balanceDUSDafter),
            Decimal(balanceDUSDbefore) - Decimal("1.00000000"),
        )
