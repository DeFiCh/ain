...
class FuturesTest(DefiTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            [
                ...
                "-fortcanningcrunchheight=150",
                "-fortcanningroadheight=150",
                "-fortcanningspringheight=500",
                "-subsidytest=1",
            ]
        ]
    ...
    def setup_test(self):
        ...
        self.oracle_id = self.nodes[0].appointoracle(
            oracle_address, self.price_feeds, 10
        )
        ...
        self.prices.append(
            {
                "premiumPrice": Decimal(
                    str(Decimal(str(self.price_tsla)) * Decimal("1.05000000"))
                ),
                "discountPrice": Decimal(
                    str(Decimal(str(self.price_tsla)) * Decimal("0.95000000"))
                ),
            }
        )
        self.prices.append(
            {
                "premiumPrice": Decimal(
                    str(Decimal(str(self.price_googl)) * Decimal("1.05000000"))
                ),
                "discountPrice": Decimal(
                    str(Decimal(str(self.price_googl)) * Decimal("0.95000000"))
                ),
            }
        )
        self.prices.append(
            {
                "premiumPrice": Decimal(
                    str(Decimal(str(self.price_twtr)) * Decimal("1.05000000"))
                ),
                "discountPrice": Decimal(
                    str(Decimal(str(self.price_twtr)) * Decimal("0.95000000"))
                ),
            }
        )
        self.prices.append(
            {
                "premiumPrice": Decimal(
                    str(Decimal(str(self.price_msft)) * Decimal("1.05000000"))
                ),
                "discountPrice": Decimal(
                    str(Decimal(str(self.price_msft)) * Decimal("0.95000000"))
                ),
            }
        )
    ...
    def futures_setup(self):
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idDFI}/fixed_interval_price_id": f"{self.symbolDFI}/USD"
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            result["v0/params/dfip2203/block_period"], str(self.futures_interval)
        )

        # Disable DUSD
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{str(self.idDUSD)}/dfip2203": "false"}}
        )
        self.nodes[0].generate(1)
        ...
        next_futures_block = self.nodes[0].getblockcount() + (
            self.futures_interval
            - (self.nodes[0].getblockcount() % self.futures_interval)
        )
    ...
    def test_dtoken_to_dusd(self):
        ...
        self.nodes[0].accounttoaccount(
            self.address, {address_msft: f"1@{self.symbolMSFT}"}
        )
        self.nodes[0].accounttoaccount(
            self.address, {address_googl: f"1@{self.symbolGOOGL}"}
        )
        self.nodes[0].accounttoaccount(
            self.address, {address_tsla: f"1@{self.symbolTSLA}"}
        )
        self.nodes[0].accounttoaccount(
            self.address, {address_twtr: f"1@{self.symbolTWTR}"}
        )
        self.nodes[0].generate(1)
        ...
        assert_raises_rpc_error(
            -32600,
            f"Could not get source loan token {self.idBTC}",
            self.nodes[0].futureswap,
            self.address,
            f"1@{self.symbolBTC}",
        )
        assert_raises_rpc_error(
            -32600,
            f"DFIP2203 currently disabled for token {self.idDUSD}",
            self.nodes[0].futureswap,
            self.address,
            f"1@{self.symbolDUSD}",
            int(self.idDUSD),
        ...
        assert_raises_rpc_error(
            -32600,
            f"Could not get destination loan token {self.idBTC}. Set valid destination.",
            self.nodes[0].futureswap,
            self.address,
            f"1@{self.symbolDUSD}",
            int(self.idBTC),
        ...
        assert_raises_rpc_error(
            -32600,
            "Destination should not be set when source amount is dToken or DFI",
            self.nodes[0].futureswap,
            self.address,
            f"1@{self.symbolTSLA}",
            int(self.idBTC),
        ...
        assert_equal(
            result["values"][0]["source"], f'{Decimal("1.00000000")}@{self.symbolMSFT}'
        )
        ...
        assert_equal(
            result["values"][0]["source"], f'{Decimal("1.00000000")}@{self.symbolGOOGL}'
        )
        assert_equal(result["values"][0]["destination"], self.symbolDUSD)
        ...
        assert_equal(
            result["values"][0]["source"], f'{Decimal("1.00000000")}@{self.symbolTSLA}'
        )
        assert_equal(result["values"][0]["destination"], self.symbolDUSD)
        ...
        assert_equal(
            result["values"][0]["source"], f'{Decimal("1.00000000")}@{self.symbolTWTR}'
        )
        assert_equal(result["values"][0]["destination"], self.symbolDUSD)
        ...
        assert_equal(
            result["v0/live/economy/dfip2203_current"],
            [
                f"1.00000000@{self.symbolTSLA}",
                f"1.00000000@{self.symbolGOOGL}",
                f"1.00000000@{self.symbolTWTR}",
        ...
        new_total_dusd = Decimal(
            self.nodes[0].gettoken(self.idDUSD)[self.idDUSD]["minted"]
        )
        assert_equal(
            total_dusd
            + self.prices[0]["discountPrice"]
            + self.prices[1]["discountPrice"]
            + self.prices[2]["discountPrice"]
            + self.prices[3]["discountPrice"],
            new_total_dusd,
        )
        ...
        result = self.nodes[0].listaccounthistory(
            "all",
            {
                "maxBlockHeight": self.nodes[0].getblockcount(),
                "depth": 0,
                "txtype": "q",
            },
        )
        ...
        self.list_history.append(
            {
                ...
                "swaps": [
                    {
                        "address": address_tsla,
                        "destination": f'{self.prices[0]["discountPrice"]}@{self.symbolDUSD}',
                    },
                    {
                        "address": address_googl,
                        "destination": f'{self.prices[1]["discountPrice"]}@{self.symbolDUSD}',
                    },
                    {
                        "address": address_twtr,
                        "destination": f'{self.prices[2]["discountPrice"]}@{self.symbolDUSD}',
                    },
                    {
                        "address": address_msft,
                        "destination": f'{self.prices[3]["discountPrice"]}@{self.symbolDUSD}',
                    },
                ],
            }
        )
    ...
    def test_dusd_to_dtoken(self):
        ...
        self.nodes[0].futureswap(
            address_msft,
            f'{self.prices[3]["premiumPrice"]}@{self.symbolDUSD}',
            self.idMSFT,
        )
        self.nodes[0].generate(1)
        self.nodes[0].futureswap(
            address_twtr,
            f'{self.prices[2]["premiumPrice"]}@{self.symbolDUSD}',
            self.idTWTR,
        )
        self.nodes[0].generate(1)
        self.nodes[0].futureswap(
            address_googl,
            f'{self.prices[1]["premiumPrice"]}@{self.symbolDUSD}',
            self.symbolGOOGL,
        )
        self.nodes[0].generate(1)
        self.nodes[0].futureswap(
            address_tsla,
            f'{self.prices[0]["premiumPrice"]}@{self.symbolDUSD}',
            self.symbolTSLA,
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            result[0]["source"], f'{self.prices[0]["premiumPrice"]}@{self.symbolDUSD}'
        )
        assert_equal(result[0]["destination"], self.symbolTSLA)
        assert_equal(result[1]["owner"], address_googl)
        assert_equal(
            result[1]["source"], f'{self.prices[1]["premiumPrice"]}@{self.symbolDUSD}'
        )
        assert_equal(result[1]["destination"], self.symbolGOOGL)
        assert_equal(result[2]["owner"], address_twtr)
        assert_equal(
            result[2]["source"], f'{self.prices[2]["premiumPrice"]}@{self.symbolDUSD}'
        )
        assert_equal(result[2]["destination"], self.symbolTWTR)
        assert_equal(result[3]["owner"], address_msft)
        assert_equal(
            result[3]["source"], f'{self.prices[3]["premiumPrice"]}@{self.symbolDUSD}'
        )
        ...
        assert_equal(
            result["v0/live/economy/dfip2203_current"],
            [
                f"3992.10000000@{self.symbolDUSD}",
                f"1.00000000@{self.symbolTSLA}",
                f"1.00000000@{self.symbolGOOGL}",
                f"1.00000000@{self.symbolTWTR}",
                f"1.00000000@{self.symbolMSFT}",
        ...
        assert_equal(
            result["v0/live/economy/dfip2203_burned"],
            [
                f"1.00000000@{self.symbolTSLA}",
                f"1.00000000@{self.symbolGOOGL}",
                f"1.00000000@{self.symbolTWTR}",
        ...
        assert_equal(
            result["v0/live/economy/dfip2203_minted"],
            [
        ...
        total_googl = Decimal(
            self.nodes[0].gettoken(self.idGOOGL)[self.idGOOGL]["minted"]
        )
        ...
        new_total_tsla = Decimal(
            self.nodes[0].gettoken(self.idTSLA)[self.idTSLA]["minted"]
        )
        new_total_googl = Decimal(
            self.nodes[0].gettoken(self.idGOOGL)[self.idGOOGL]["minted"]
        )
        new_total_twtr = Decimal(
            self.nodes[0].gettoken(self.idTWTR)[self.idTWTR]["minted"]
        )
        new_total_msft = Decimal(
            self.nodes[0].gettoken(self.idMSFT)[self.idMSFT]["minted"]
        )
        ...
        self.list_history.append(
            {
                ...
                "swaps": [
                    {
                        "address": address_tsla,
                        "destination": f"1.00000000@{self.symbolTSLA}",
                    },
                    {
                        "address": address_googl,
                        "destination": f"1.00000000@{self.symbolGOOGL}",
                    },
                    {
                        "address": address_twtr,
                        "destination": f"1.00000000@{self.symbolTWTR}",
                    },
                    {
                        "address": address_msft,
                        "destination": f"1.00000000@{self.symbolMSFT}",
                    },
                ],
            }
        )
    ...
    def check_swap_block_range(self):
        ...
        assert_raises_rpc_error(
            -32600,
            "amount 0.00000000 is less than 0.00000001",
            self.nodes[0].withdrawfutureswap,
            address,
            f'{Decimal("0.00000001")}@{self.symbolDUSD}',
            int(self.idTSLA),
        )
        ...
        result = self.nodes[0].getaccount(address)
        assert_equal(
            result, [f"913.50000000@{self.symbolDUSD}", f"1.00000000@{self.symbolTSLA}"]
        )
        ...
        assert_equal(
            result,
            [
                f"4905.60000000@{self.symbolDUSD}",
                f"1.00000000@{self.symbolTSLA}",
                f"1.00000000@{self.symbolGOOGL}",
                f"1.00000000@{self.symbolTWTR}",
                f"1.00000000@{self.symbolMSFT}",
    ...
    def check_multiple_swaps(self):
        ...
        assert_equal(
            result,
            [
                f"6810.30000000@{self.symbolDUSD}",
                f"1.00000000@{self.symbolTSLA}",
                f"1.00000000@{self.symbolGOOGL}",
                f"1.00000000@{self.symbolTWTR}",
                f"1.00000000@{self.symbolMSFT}",
    ...
    def check_withdrawals(self):
        ...
        assert_raises_rpc_error(
            -32600,
            f'amount 0.00000000 is less than {self.prices[2]["premiumPrice"] * 2}',
            self.nodes[0].withdrawfutureswap,
            address_tsla,
            f'{self.prices[2]["premiumPrice"] * 2}@{self.symbolDUSD}',
            int(self.idTWTR),
        )
        assert_raises_rpc_error(
            -32600,
        ...
        self.nodes[0].withdrawfutureswap(
            address_tsla,
            f'{self.prices[0]["premiumPrice"] * 2}@{self.symbolDUSD}',
            int(self.idTSLA),
        )
        self.nodes[0].generate(1)
        ...
        # Withdraw one Sat
        self.nodes[0].withdrawfutureswap(
            address_msft, f'{Decimal("0.00000001")}@{self.symbolDUSD}', int(self.idMSFT)
        )
        self.nodes[0].generate(1)
        ...
        result = self.nodes[0].getaccount(address_tsla)
        assert_equal(
            result, [f'{self.prices[0]["premiumPrice"] * 2}@{self.symbolDUSD}']
        )
        result = self.nodes[0].getaccount(address_twtr)
        assert_equal(
            result,
        ...
        result = self.nodes[0].getaccount(address_msft)
        assert_equal(
            result, [f"0.00000001@{self.symbolDUSD}", f"1.99999999@{self.symbolMSFT}"]
        )
        ...
        assert_equal(
            result,
            [
                f"7468.64999999@{self.symbolDUSD}",
                f"1.00000000@{self.symbolTSLA}",
                f"1.00000000@{self.symbolGOOGL}",
                f"1.00000000@{self.symbolTWTR}",
                f"1.00000000@{self.symbolMSFT}",
    ...
    def check_minimum_swaps(self):
        ...
        self.nodes[0].accounttoaccount(
            self.address,
            {address: f'{self.prices[0]["premiumPrice"]}@{self.symbolDUSD}'},
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            result,
            [
                f'{self.prices[0]["premiumPrice"] - Decimal("0.00000001")}@{self.symbolDUSD}'
            ],
        )
        ...
        assert_equal(
            result,
            [
                f"7468.65000000@{self.symbolDUSD}",
                f"1.00000000@{self.symbolTSLA}",
                f"1.00000000@{self.symbolGOOGL}",
                f"1.00000000@{self.symbolTWTR}",
                f"1.00000000@{self.symbolMSFT}",
        ...
        self.nodes[0].futureswap(
            address, f"{min_purchase}@{self.symbolDUSD}", int(self.idTSLA)
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            result,
            [
                f"7468.65000914@{self.symbolDUSD}",
                f"1.00000000@{self.symbolTSLA}",
                f"1.00000000@{self.symbolGOOGL}",
                f"1.00000000@{self.symbolTWTR}",
                f"1.00000000@{self.symbolMSFT}",
    ...
    def check_gov_var_change(self):
        ...
        assert_equal(
            result,
            [
                f"7468.65000915@{self.symbolDUSD}",
                f"1.00000000@{self.symbolTSLA}",
                f"1.00000000@{self.symbolGOOGL}",
                f"1.00000000@{self.symbolTWTR}",
                f"1.00000000@{self.symbolMSFT}",
        ...
        assert_raises_rpc_error(
            -32600,
            "Cannot set block period while DFIP2203 is active",
            self.nodes[0].setgov,
            {
                "ATTRIBUTES": {
                    "v0/params/dfip2203/block_period": f"{self.futures_interval}"
        ...
        # Disable GOOGL
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{str(self.idGOOGL)}/dfip2203": "false"}}
        )
        self.nodes[0].generate(1)
        ...
        # Enable GOOGL
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{str(self.idGOOGL)}/dfip2203": "true"}}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            result,
            [
                f"8382.15000915@{self.symbolDUSD}",
                f"1.00000000@{self.symbolTSLA}",
                f"1.00000000@{self.symbolGOOGL}",
                f"1.00000000@{self.symbolTWTR}",
                f"1.00000000@{self.symbolMSFT}",
    ...
    def rpc_history(self):
        ...
        for history in self.list_history:
            result = self.nodes[0].listaccounthistory(
                "all", {"maxBlockHeight": history["height"], "depth": 0, "txtype": "q"}
            )
            for history_entry in history["swaps"]:
                ...
                for result_entry in result:
                    ...
                    if result_entry["owner"] == history_entry["address"]:
                        ...
                        assert_equal(
                            result_entry["amounts"], [history_entry["destination"]]
                        )
                        found = True
                assert found
        ...
        result = self.nodes[0].listaccounthistory(
            self.list_history[0]["swaps"][0]["address"], {"txtype": "q"}
        )
        assert_equal(len(result), 1)
        ...
        assert_equal(
            result[0]["amounts"], [self.list_history[0]["swaps"][0]["destination"]]
        )
    ...
    def start_block(self):
        ...
        assert_raises_rpc_error(
            ...
            {
                "ATTRIBUTES": {
                    "v0/params/dfip2203/start_block": f"{self.nodes[0].getblockcount() + 1}"
                }
            },
        )
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/params/dfip2203/start_block": f"{self.start_block}"}}
        )
        self.nodes[0].generate(1)
        ...
        assert_raises_rpc_error(
            -32600,
            f"DFIP2203 not active until block {self.start_block}",
            self.nodes[0].futureswap,
            address,
            f'{self.prices[0]["premiumPrice"]}@{self.symbolDUSD}',
            int(self.idTSLA),
        ...
        next_futures_block = (
            ...
            + (
                self.futures_interval
                - (
                    (self.nodes[0].getblockcount() - self.start_block)
                    % self.futures_interval
                )
            )
            + self.futures_interval
    ...
    def dfi_to_dusd(self):
        ...
        assert_raises_rpc_error(
            -32600,
            f"DFIP2206F not currently active",
            self.nodes[0].futureswap,
            address,
            f"1@{self.symbolDFI}",
        )
        ...
        assert_equal(
            result["v0/params/dfip2206f/block_period"], f"{self.futures_interval_dusd}"
        )
        assert_equal(
            result["v0/params/dfip2206f/start_block"], f"{self.start_block_dusd}"
        )

        # Test cannot create a future swap until active
        assert_raises_rpc_error(
            -32600,
            f"DFIP2206F not active until block {self.start_block_dusd}",
            self.nodes[0].futureswap,
            address,
            f"1@{self.symbolDFI}",
            f"{self.symbolDUSD}",
        ...
        next_futures_block = (
            self.nodes[0].getblockcount()
            + (
                self.futures_interval_dusd
                - (
                    (self.nodes[0].getblockcount() - self.start_block_dusd)
                    % self.futures_interval_dusd
                )
            )
            + self.futures_interval_dusd
        ...
        result = self.nodes[0].listgovs()[8][0]["ATTRIBUTES"]
        assert_equal(
            result["v0/live/economy/dfip2206f_current"],
            [f"2.00000000@{self.symbolDFI}"],
        )
        assert_equal(
            result["v0/live/economy/dfip2206f_burned"], [f"2.00000000@{self.symbolDFI}"]
        )
        assert_equal(
            result["v0/live/economy/dfip2206f_minted"],
            [f'{Decimal("0.99000000") * 2}@{self.symbolDUSD}'],
        )
    ...
    def check_withdrawals_dusd(self):
        ...
        assert_raises_rpc_error(
            -32600,
            "amount 2.00000000 is less than 2.00000001",
            self.nodes[0].withdrawfutureswap,
            address,
            f"2.00000001@{self.symbolDFI}",
            self.symbolDUSD,
        )

        # Withdraw both contracts
        self.nodes[0].withdrawfutureswap(
            address, f"2.00000000@{self.symbolDFI}", self.symbolDUSD
        )
        self.nodes[0].generate(1)
        ...
        # Withdraw frm GOOGL everything but one Sat
        self.nodes[0].withdrawfutureswap(
            address, f"0.99999999@{self.symbolDFI}", self.symbolDUSD
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].withdrawfutureswap(
            address, f"1.00000001@{self.symbolDFI}", self.symbolDUSD
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].withdrawfutureswap(
            address, f"0.99999996@{self.symbolDFI}", self.symbolDUSD
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            result["v0/live/economy/dfip2206f_current"],
            [f"2.00000003@{self.symbolDFI}"],
        )
        assert_equal(
            result["v0/live/economy/dfip2206f_burned"], [f"2.00000003@{self.symbolDFI}"]
        )
        assert_equal(
            result["v0/live/economy/dfip2206f_minted"],
