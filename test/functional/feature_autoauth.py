...
class TokensAutoAuthTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        n0.updatetoken("GOLD", {"isDAT": False})
        assert_equal(len(n0.getrawmempool()), 2)
        n0.generate(1, 1000000, coinbase)
        ...
        n0.addpoolliquidity(
            {collateralGold: "100@GOLD#1", collateralSilver: "500@SILVER"}, poolShare
        )
        assert_equal(len(n0.getrawmempool()), 2)
        n0.generate(1, 1000000, coinbase)
        assert_equal(len(n0.getrawmempool()), 0)
        ...
        assert_equal(len(n0.getrawmempool()), 0)
        assert_equal(
            len(n0.getaccount(poolShare, {}, True)), 3
        )  # so gold and silver appears
        ...
        n0.updatepoolpair({"pool": "GS", "status": True})
        n0.generate(1, 1000000, coinbase)
