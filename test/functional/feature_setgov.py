...
class GovsetTest (DefiTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-32600, "ATTRIBUTES: Cannot be set before FortCanningHill", self.nodes[0].setgov, {"ATTRIBUTES":{'token/5/payback_dfi':'true'}})
        ...
        assert_raises_rpc_error(-5, "Empty key", self.nodes[0].setgov, {"ATTRIBUTES":{'':'true'}})
        assert_raises_rpc_error(-5, "Empty value", self.nodes[0].setgov, {"ATTRIBUTES":{'token/15/payback_dfi':''}})
        assert_raises_rpc_error(-5, "Incorrect key for <type>. Object of ['<type>/ID/<key>','value'] expected", self.nodes[0].setgov, {"ATTRIBUTES":{'token/payback_dfi':'true'}})
        assert_raises_rpc_error(-5, "Unrecognised type argument provided, valid types are: poolpairs, token,", self.nodes[0].setgov, {"ATTRIBUTES":{'unrecognised/5/payback_dfi':'true'}})
        assert_raises_rpc_error(-5, "Unrecognised type argument provided, valid types are: payback_dfi, payback_dfi_fee_pct,", self.nodes[0].setgov, {"ATTRIBUTES":{'token/5/unrecognised':'true'}})
