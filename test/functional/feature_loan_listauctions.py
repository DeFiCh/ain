...
class LoanTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        # take loan
        for id in vaultsId:
            self.nodes[0].takeloan({"vaultId": id, "amounts": "10@TSLA"})
        self.nodes[0].generate(1)
        ...
        self.nodes[0].generate(
            12
        )  # if price is invalid, auctions are blocked so listauction is empty. We need 2 cicles of price update.

        # Auction tests
        auctionlist = self.nodes[0].listauctions()
        assert_equal(
            len(auctionlist), vault_number
        )  # all vaults should be under liquidation
        ...
        auctionlist = self.nodes[0].listauctions(
            {
                "start": {
                    "vaultId": secondAuction["vaultId"],
                    "height": secondAuction["liquidationHeight"],
                }
            }
        )
