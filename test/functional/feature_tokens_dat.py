...
class TokensBasicTest (DefiTestFramework):
    ...
    def run_test(self):
        ...
        # Get token
        assert_equal(self.nodes[0].gettoken("GOLD")["isDAT"], True)

        # 6 Checking that it will not sync
        self.sync_blocks([self.nodes[0], self.nodes[2]])
        tokens = self.nodes[2].listtokens()
        assert_equal(len(tokens), 3)
        assert_equal(tokens[2]["isDAT"], False) # not synced cause new tx type (from node 0)
        ...
        tokens = self.nodes[2].listtokens()
        assert_equal(len(tokens), 3)
        assert_equal(tokens[2]["isDAT"], True) # synced now
        ...
        tokens = self.nodes[0].listtokens()
        assert_equal(len(tokens), 3)
        assert_equal(tokens[2]["isDAT"], False)
        ...
        assert_equal(self.nodes[0].gettoken('128')["isDAT"], False)
        assert_equal(self.nodes[1].gettoken('128')["isDAT"], False)
        ...
        self.sync_blocks([self.nodes[0], self.nodes[1]])
        assert_equal(self.nodes[0].gettoken('128')["isDAT"], False) # tx not applyed cause "old"
        assert_equal(self.nodes[1].gettoken('128')["isDAT"], True)
        ...
        assert_equal(token["symbol"], "gold")
        assert_equal(token["symbolKey"], "gold#128")
        assert_equal(token["isDAT"], False)
        ...
        self.nodes[0].generate(1)
        token = self.nodes[0].gettoken('128')
        assert_equal(token["symbol"], "goldy")
        assert_equal(token["symbolKey"], "goldy")
        assert_equal(token["isDAT"], True)
        ...
        self.nodes[0].generate(1)
        token = self.nodes[0].gettoken('128')
        assert_equal(token["name"], "new name")
        assert_equal(token["mintable"], False)
        assert_equal(token["tradeable"], False)
        assert_equal(token["finalized"], True)
        ...
        assert_equal(tokens[4]["symbol"], "TEST")
        assert_equal(tokens[4]["name"], "TEST token copy")
        assert_equal(tokens[4]["isDAT"], False)
