...
class TokensBasicTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(len(tokens), 3)
        assert_equal(
            tokens["128"]["isDAT"], False
        )  # not synced cause new tx type (from node 0)
        ...
        )  # warning! simple '-reindex' not works!
        ...
        self.nodes[1].importprivkey(
            self.nodes[0].get_genesis_keys().ownerPrivKey
        )  # there is no need to import the key, cause node#1 is founder itself... but it has no utxos for auth
        self.nodes[1].updatetoken("128", {"isDAT": True})
        self.nodes[1].generate(1)
        self.sync_blocks([self.nodes[0], self.nodes[1]])
        assert_equal(
            self.nodes[0].gettoken("128")["128"]["isDAT"], False
        )  # tx not applyed cause "old"
        ...
        assert_equal(
            self.nodes[0].gettoken("goldy"), token
        )  # can do it w/o '#'' cause it should be DAT
        ...
        self.nodes[0].createtoken(
            {
                ...
                "isDAT": False,
                "collateralAddress": self.nodes[0]
                .get_genesis_keys()
                .ownerAuthAddress,  # !from founders!!
            },
            [],
        )
