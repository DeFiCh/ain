...
class GetStoredInterestTest(DefiTestFramework):
    ...
    def goto_gw_height(self):
        ...
        assert_equal(
            blockchainInfo["softforks"]["fortcanninggreatworld"]["active"], True
        )
    ...
    def goto_setup_height(self):
        ...
        blockchainInfo = self.nodes[0].getblockchaininfo()
        assert_equal(
            blockchainInfo["softforks"]["fortcanninggreatworld"]["active"], False
        )

    # Default token = 1 = dUSD
    def set_token_interest(self, token="1", interest=0):
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{token}/loan_minting_interest": str(interest)}}
        )
        self.nodes[0].generate(1)
    ...
    def update_vault_IPB_negative_ITH_positive(self, do_revert=True):
        ...
        self.nodes[0].takeloan(
            {"vaultId": vaultId, "amounts": f"{loanAmount}@" + self.symboldUSD}
        )
        self.nodes[0].generate(1)

        # Check expected IPB and ITH
        storedInterest = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(
            Decimal("0.01") * Decimal(loanAmount) / BLOCKS_PER_YEAR
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        ...
        expected_IPB1 = Decimal(
            Decimal("0.02") * Decimal(loanAmount) / BLOCKS_PER_YEAR
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB1, Decimal(storedInterest1["interestPerBlock"]))
        assert_equal(
            Decimal(storedInterest["interestToHeight"]) + expected_IPB_tmp,
        ...
        storedInterest = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(
            Decimal("-0.5") / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB, Decimal(storedInterest["interestPerBlock"]))
        assert_greater_than_or_equal(
            Decimal(storedInterest["interestToHeight"]), Decimal(0)
        )
        self.nodes[0].generate(1)
        ...
        storedInterest = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(
            Decimal("-0.51") / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB, Decimal(storedInterest["interestPerBlock"]))
        assert_greater_than_or_equal(
            Decimal(0), Decimal(storedInterest["interestToHeight"])
        )
    ...
    def update_vault_IPB_positive_ITH_negative(self, do_revert=True):
        ...
        storedInterest = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(
            Decimal("-0.04") / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB, Decimal(storedInterest["interestPerBlock"]))
        assert_equal(Decimal(0), Decimal(storedInterest["interestToHeight"]))
        ...
        storedInterest1 = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB1 = Decimal(
            Decimal("-0.03") / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB1, Decimal(storedInterest1["interestPerBlock"]))
        assert_equal(
            Decimal(storedInterest["interestToHeight"]) + expected_IPB_tmp,
        ...
        storedInterest = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(
            Decimal("0.49") / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB, Decimal(storedInterest["interestPerBlock"]))
        assert_greater_than_or_equal(
            Decimal(storedInterest["interestToHeight"]), Decimal(0)
    ...
    def update_vault_IPB_and_ITH_negative(self, do_revert=True):
        ...
        storedInterest = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(
            Decimal("-0.03") * Decimal(loanAmount) / BLOCKS_PER_YEAR
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB, Decimal(storedInterest["interestPerBlock"]))
        assert_equal(
            Decimal(storedInterest["interestToHeight"]),
            Decimal("-0.000000418569254185692533"),
        )
        # Update vault
        self.nodes[0].updatevault(vaultId, {"loanSchemeId": "LOAN2"})
        self.nodes[0].generate(1)
        storedInterest1 = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB1 = Decimal(
            Decimal("-0.02") / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert storedInterest["interestPerBlock"] != storedInterest1["interestPerBlock"]
        assert_equal(
            Decimal(storedInterest1["interestPerBlock"]), Decimal(expected_IPB1)
        )
        assert_equal(
            Decimal(storedInterest1["interestToHeight"]),
        ...
        assert_equal(
            Decimal(storedInterest2["interestToHeight"]),
            Decimal(storedInterest["interestToHeight"]) + Decimal(expected_IPB),
        )
    ...
    def update_vault_IPB_and_ITH_positive(self, do_revert=True):
        ...
        storedInterest = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(
            Decimal("0.01") / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB, Decimal(storedInterest["interestPerBlock"]))
        assert_equal(
            Decimal(storedInterest["interestToHeight"]),
            Decimal("0.000000209284627092846261"),
        )
        # Update vault
        self.nodes[0].updatevault(vaultId, {"loanSchemeId": "LOAN2"})
        self.nodes[0].generate(1)
    ...
    def takeloan_IPB_and_ITH_positive(self, do_revert=True):
        ...
        # Take another loan
        loanAmount = 2
        self.nodes[0].takeloan({"vaultId": vaultId, "amounts": "1@" + self.symboldUSD})
        self.nodes[0].generate(1)
        storedInterest1 = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB1 = Decimal(
            Decimal("0.02") / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ...
        self.nodes[0].generate(1)
        storedInterest2 = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB2 = Decimal(
            Decimal("0.02") / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert (
            storedInterest1["interestPerBlock"] != storedInterest2["interestPerBlock"]
        )
        assert_equal(
            Decimal(storedInterest2["interestPerBlock"]), Decimal(expected_IPB2)
        )
        assert_equal(
            Decimal(storedInterest2["interestToHeight"]),
    ...
    def takeloan_IPB_and_ITH_negative(self, do_revert=True):
        ...
        storedInterest = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(
            Decimal("-0.05") / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB, Decimal(storedInterest["interestPerBlock"]))
        assert_equal(Decimal(0), Decimal(storedInterest["interestToHeight"]))
    ...
    def takeloan_IPB_negative_ITH_positive(self, do_revert=True):
        ...
        # Check expected IPB and ITH
        storedInterest = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(Decimal("0.02") / BLOCKS_PER_YEAR * loanAmount).quantize(
        ...
        storedInterest = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(
            Decimal("-0.02") / Decimal(1051200 * Decimal(loanAmount))
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        ...
        assert_equal(
            Decimal(storedInterest["interestToHeight"]),
            Decimal("0.000000076103500761034999"),
        )  # expected_ITH = expected_ITH + expected_IPB off by 1E-24 rounding difference
    ...
    def payback_loan_IPB_positive_and_ITH_positive(self, do_revert=True):
        ...
        expected_ITH = Decimal(expected_IPB * 2 - Decimal(interestAmount) / 2).quantize(
            Decimal("1E-24"), ROUND_DOWN
        )
        assert_equal(Decimal(storedInterest["interestToHeight"]), expected_ITH)
        assert_equal(Decimal(storedInterest["interestPerBlock"]), expected_IPB)
        ...
        self.nodes[0].paybackloan(
            {
                "vaultId": vaultId,
                "from": self.account0,
                "amounts": f"{loanAmount / 2 + float(interestAmount)}@"
                + self.symboldUSD,
            }
        )
        self.nodes[0].generate(1)
        storedInterest1 = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB1 = Decimal(
            Decimal("0.02") / 2 / BLOCKS_PER_YEAR * Decimal(loanAmount)
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB1, Decimal(storedInterest1["interestPerBlock"]))
        assert_equal(Decimal(storedInterest1["interestToHeight"]), 0)
        assert_equal(
            Decimal(storedInterest1["interestPerBlock"]),
            (Decimal(storedInterest["interestPerBlock"]) / 2).quantize(
                Decimal("1E-24"), ROUND_DOWN
            ),
        )
    ...
    def payback_loan_IPB_positive_and_ITH_negative(self, do_revert=True):
        ...
        storedInterest1 = self.nodes[0].getstoredinterest(vaultId, self.symboldUSD)
        expected_IPB = Decimal(
            Decimal("0.025") * Decimal(loanAmount) / BLOCKS_PER_YEAR
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(
            Decimal(storedInterest1["interestToHeight"]), 0
        )  # negative interest was used for payback
        assert_equal(Decimal(storedInterest1["interestPerBlock"]), expected_IPB)
    ...
    def payback_loan_IPB_negative_and_ITH_positive(self, do_revert=True):
        ...
        self.nodes[0].paybackloan(
            {"vaultId": vaultId, "from": self.account0, "amounts": loanAmount}
        )
        self.nodes[0].generate(1)
    ...
    def vault_in_liquidation_negative_interest(self, do_revert=True):
        ...
        self.nodes[0].takeloan(
            {"vaultId": vaultId, "amounts": f"{loanAmount}@" + self.symbolTSLA}
        )
        self.nodes[0].generate(1)
        # Check expected IPB and ITH
        storedInterestTSLA = self.nodes[0].getstoredinterest(vaultId, self.symbolTSLA)
        expected_IPB_TSLA = Decimal(
            Decimal("100000.01") * Decimal(loanAmount) / BLOCKS_PER_YEAR
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        ...
        assert_equal(
            storedInterestDUSD["interestPerBlock"],
            storedInterestDUSD1["interestPerBlock"],
        )
        ...
        blocks_to_price_update = (
            fixedPrice["nextPriceBlock"] - self.nodes[0].getblockcount()
        )
        ...
        self.nodes[0].placeauctionbid(
            vaultId, 1, newOwner, "11@TSLA"
        )  # above 5% and leave vault with some loan to exit liquidation state
        ...
        self.nodes[0].generate(
            liquidationHeight - self.nodes[0].getblockcount() + 1
        )  # let auction end
        ...
        loanAmounts = self.nodes[0].getloantokens(vaultId)
        expected_IPB_DUSD = Decimal(
            Decimal("0.02") * get_decimal_amount(loanAmounts[0]) / BLOCKS_PER_YEAR
        ).quantize(Decimal("1E-24"), ROUND_DOWN)
        assert_equal(expected_IPB_DUSD, Decimal(storedInterestDUSD["interestPerBlock"]))
        assert_equal(
            storedInterestDUSD["interestToHeight"], "-0.000000285387405821917806"
        )
        ...
        assert_equal(
            accountInfo, ["9.53410975@DFI", "10.00000000@DUSD", "9.00000000@TSLA"]
        )
    ...
    def create_pool_pairs(self):
        ...
        self.nodes[0].addpoolliquidity(
            {self.account0: ["100@TSLA", "1000@DUSD"]}, self.account0
        )
        self.nodes[0].generate(1)

        self.nodes[0].addpoolliquidity(
            {self.account0: ["100@DFI", "1000@DUSD"]}, self.account0
        )
        self.nodes[0].generate(1)

        self.nodes[0].addpoolliquidity(
            {self.account0: ["100@GOLD", "1000@DUSD"]}, self.account0
        )
        self.nodes[0].generate(1)
