...
class PaybackDFILoanTest(DefiTestFramework):
    ...
    def setup_loan_tokens(self):
        ...
        self.nodes[0].setcollateraltoken(
            {"token": self.idBTC, "factor": 1, "fixedIntervalPriceId": "BTC/USD"}
        )
        self.nodes[0].generate(1)
    ...
    def setup_poolpairs(self):
        ...
        self.nodes[0].addpoolliquidity(
            {
                self.addr_pool_DFI_DUSD: [
                    "5000000@" + self.symbolDFI,
                    "20000000@" + self.symboldUSD,
                ]
            },
            self.account0,
        )
        ...
        self.nodes[0].addpoolliquidity(
            {
                self.addr_pool_TSLA_DUSD: [
                    "4000000000@" + self.symbolTSLA,
                    "20000000@" + self.symboldUSD,
                ]
            },
            self.account0,
    ...
    def payback_DUSD_with_BTC(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId1, "amounts": "100@" + self.symboldUSD}
        )
        self.nodes[0].generate(1)
        ...
        try:
            self.nodes[0].paybackloan(
                {"vaultId": self.vaultId1, "from": self.account0, "amounts": "1@BTC"}
            )
    ...
    def payback_with_DFI_prior_to_atribute_activation(self):
        ...
        try:
            self.nodes[0].paybackloan(
                {"vaultId": self.vaultId1, "from": self.account0, "amounts": "1@DFI"}
            )
        except JSONRPCException as e:
            errorString = e.error["message"]
    ...
    def setgov_attribute_to_false_and_payback(self):
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/token/" + self.iddUSD + "/payback_dfi": "false"}}
        )
        self.nodes[0].generate(1)
        ...
        assert_raises_rpc_error(
            -32600,
            "Payback of loan via DFI token is not currently active",
            self.nodes[0].paybackloan,
            {"vaultId": self.vaultId1, "from": self.account0, "amounts": "1@DFI"},
        )
    ...
    def setgov_attribute_to_true_and_payback_with_dfi(self):
        # Enable loan payback
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/token/" + self.iddUSD + "/payback_dfi": "true"}}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].paybackloan(
            {"vaultId": self.vaultId1, "from": self.addr_DFI, "amounts": "1@DFI"}
        )
        self.nodes[0].generate(1)
        ...
        )  # paybackfee defaults to 1% of total payback -> 0.01 DFI
        assert_equal(
            info["dfipaybacktokens"], ["3.96000000@DUSD"]
        )  # 4 - penalty (0.01DFI->0.04USD)
        ...
        assert_equal(
            Decimal(amountAfter) - Decimal(interestAfter),
            Decimal(amountBefore) - Decimal("3.96"),
        )  # no payback fee

    def test_5pct_penalty(self):
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/token/" + self.iddUSD + "/payback_dfi_fee_pct": "0.05"}}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(amountAfter) - Decimal(interestAfter),
            (Decimal(amountBefore) - (Decimal("3.8"))),
        )  # 4$ in DFI - 0.05fee = 3.8$ paid back
    ...
    def overpay_loan_in_DFI(self):
        ...
        self.nodes[0].paybackloan(
            {"vaultId": self.vaultId1, "from": self.addr_DFI, "amounts": "250@DFI"}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            info["dfipaybacktokens"], ["100.00001113@DUSD"]
        )  # Total loan in vault1 + previous dfipaybacktokens
        [new_amount, _] = info["paybackburn"][0].split("@")
        assert_equal(Decimal(new_amount), Decimal(old_amount) + Decimal("24.27368714"))

        attribs = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attribs["v0/live/economy/dfi_payback_tokens"],
            ["1.27368435@DFI", "100.00001113@DUSD"],
        )
        ...
        assert_equal(
            Decimal(balanceDFIBefore) - Decimal(balanceDFIAfter),
            (Decimal(amountBefore) / Decimal("3.8")).quantize(
    ...
    def take_new_loan_payback_exact_amount_in_DFI(self):
        ...
        assert_equal(len(vaultAfter["interestAmounts"]), 0)
        assert_equal(
            Decimal(balanceDFIBefore) - Decimal(balanceDFIAfter), Decimal("26.31579449")
        )
    ...
    def new_vault_payback_TSLA_loan_with_DFI(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId2, "amounts": "10@" + self.symbolTSLA}
        )
        self.nodes[0].generate(1)
        ...
        assert_raises_rpc_error(
            -32600,
            "There is no loan on token (DUSD) in this vault!",
            self.nodes[0].paybackloan,
            {"vaultId": self.vaultId2, "from": self.addr_DFI, "amounts": "10@DFI"},
        )
        # Should not be able to payback loan before DFI payback enabled
        assert_raises_rpc_error(
            -32600,
            ...
            self.nodes[0].paybackloan,
            {
                ...
                "from": self.addr_DFI,
                "loans": [{"dToken": self.idTSLA, "amounts": "1@DFI"}],
            },
        )

    def setgov_enable_dfi_payback_and_dfi_fee_pct(self):
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/token/" + self.idTSLA + "/payback_dfi": "true"}}
        )
        self.nodes[0].generate(1)

        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/token/" + self.idTSLA + "/payback_dfi_fee_pct": "0.01"}}
        )
        self.nodes[0].generate(1)
    ...
    def setgov_enable_dTSLA_to_dBTC_payback(self):
        ...
        assert_equal(
            attributes["v0/token/" + self.idTSLA + "/loan_payback/" + self.idBTC],
            "true",
        )
        assert_equal(
            attributes[
                "v0/token/" + self.idTSLA + "/loan_payback_fee_pct/" + self.idBTC
            ],
            "0.25",
        )
    ...
    def payback_TSLA_with_1_dfi(self):
        ...
        self.nodes[0].paybackloan(
            {
                "vaultId": self.vaultId2,
                "from": self.addr_DFI,
                "loans": [{"dToken": self.idTSLA, "amounts": "0.01@DFI"}],
            }
        )
        self.nodes[0].generate(1)
        ...
        [balanceDFIAfter, _] = self.nodes[0].getaccount(self.addr_DFI)[0].split("@")
        assert_equal(
            Decimal(balanceDFIBefore) - Decimal(balanceDFIAfter), Decimal("0.01")
        )

    def take_dUSD_loan_and_payback_with_1_dfi(self):
        [balanceDFIBefore, _] = self.nodes[0].getaccount(self.addr_DFI)[0].split("@")
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId2, "amounts": "100@" + self.symboldUSD}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].paybackloan(
            {
                "vaultId": self.vaultId2,
                "from": self.addr_DFI,
                "loans": [{"dToken": self.iddUSD, "amounts": "1@DFI"}],
            }
        )
        self.nodes[0].generate(1)
    ...
    def payback_TSLA_and_dUSD_with_1_dfi(self):
        ...
        self.nodes[0].paybackloan(
            {
                ...
                "from": self.addr_DFI,
                "loans": [
                    {"dToken": self.idTSLA, "amounts": "0.001@DFI"},
                    {"dToken": self.iddUSD, "amounts": "1@DFI"},
                ],
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(amountTSLAAfter) - Decimal(interestTSLAAfter),
            (Decimal(amountTSLABefore) - Decimal("0.792")),
        )
        assert_equal(
            Decimal(amountDUSDAfter) - Decimal(interestDUSDAfter),
            (Decimal(amountDUSDBefore) - Decimal("3.8")),
        )

        [balanceDFIAfter, _] = self.nodes[0].getaccount(self.addr_DFI)[0].split("@")
        assert_equal(
            Decimal(balanceDFIBefore) - Decimal(balanceDFIAfter), Decimal("1.001")
        )
    ...
    def payback_TSLA_with_10_dfi(self):
        ...
        self.nodes[0].paybackloan(
            {
                "vaultId": self.vaultId2,
                "from": self.addr_DFI,
                "loans": [{"dToken": self.idTSLA, "amounts": "10@DFI"}],
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(balanceDFIBefore) - Decimal(balanceDFIAfter), Decimal("0.00162627")
        )
    ...
    def payback_dUSD_with_dfi(self):
        ...
        self.nodes[0].paybackloan(
            {
                "vaultId": self.vaultId2,
                "from": self.addr_DFI,
                "loans": [{"dToken": self.iddUSD, "amounts": "25@DFI"}],
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(balanceDFIBefore) - Decimal(balanceDFIAfter), Decimal("24.31579139")
        )
        assert_equal(len(vaultAfter["loanAmounts"]), 0)
        assert_equal(len(vaultAfter["interestAmounts"]), 0)
    ...
    def payback_TSLA_with_1_dBTC(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId3, "amounts": "10000000@" + self.symbolTSLA}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].paybackloan(
            {
                ...
                "from": self.addr_BTC,
                "loans": [{"dToken": self.idTSLA, "amounts": "0.5@BTC"}],
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(amountAfter) - Decimal(interestAfter),
            (Decimal(amountBefore) - Decimal("3750000")),
        )  # add 25% fee for payback with BTC

        [balanceBTCAfter, _] = self.nodes[0].getaccount(self.addr_BTC)[0].split("@")
        assert_equal(
            Decimal(balanceBTCBefore) - Decimal(balanceBTCAfter), Decimal("0.5")
        )
        ...
        [new_amount, _] = info["paybackburn"][0].split("@")
        assert_equal(
            Decimal(new_amount), Decimal(old_amount) + Decimal("6209.54767138")
        )
    ...
    def payback_dUSD_with_dUSD(self):
        ...
        self.nodes[0].paybackloan(
            {
                "vaultId": self.vaultId2,
                "from": self.addr_DUSD,
                "loans": [{"dToken": self.iddUSD, "amounts": "100@DUSD"}],
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(amountAfter) - Decimal(interestAfter),
            (Decimal(amountBefore) - Decimal("100")),
        )

        [balanceDUSDAfter, _] = self.nodes[0].getaccount(self.addr_DUSD)[0].split("@")
        assert_equal(
            Decimal(balanceDUSDBefore) - Decimal(balanceDUSDAfter), Decimal("100")
        )
    ...
    def payback_TSLA_with_1sat_dBTC(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId4, "amounts": "0.00000001@" + self.symbolTSLA}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].paybackloan(
            {
                ...
                "from": self.addr_BTC,
                "loans": [{"dToken": self.idTSLA, "amounts": "0.00000001@BTC"}],
            }
        )
        self.nodes[0].generate(1)
        ...
        [balanceBTCAfter, _] = self.nodes[0].getaccount(self.addr_BTC)[0].split("@")
        assert_equal(
            Decimal(balanceBTCBefore) - Decimal(balanceBTCAfter), Decimal("0.00000001")
        )
    ...
    def multipayback_DUSD_with_DFI_and_DUSD(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId5, "amounts": "100@" + self.symboldUSD}
        )
        self.nodes[0].generate(1)

        [balanceDFIBefore, _] = (
            self.nodes[0].getaccount(self.addr_DFI_DUSD)[0].split("@")
        )
        [balanceDUSDBefore, _] = (
            self.nodes[0].getaccount(self.addr_DFI_DUSD)[1].split("@")
        )
        ...
        self.nodes[0].paybackloan(
            {
                ...
                "loans": [{"dToken": self.iddUSD, "amounts": ["70@DUSD", "10@DFI"]}],
            }
        )
        # self.nodes[0].paybackloan({
        #    'vaultId': self.vaultId5,
        ...
        assert_equal(
            Decimal(balanceDUSDBefore) - Decimal(balanceDUSDAfter),
            Decimal("62.00000191"),
        )  # balanceAfter = 71DUSD - (100.00000191DUSD loan+interests - 38DUSD (40DFI - 5% fee)
        assert_equal(
            Decimal(balanceDFIBefore) - Decimal(balanceDFIAfter), Decimal("10")
        )
    ...
    def multipayback_DUSD_with_DFI_and_DUSD_Pre_FCR(self):
        ...
        self.nodes[0].takeloan(
            {"vaultId": self.vaultId6, "amounts": "100@" + self.symboldUSD}
        )
        self.nodes[0].generate(1)
