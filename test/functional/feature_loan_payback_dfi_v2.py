...
class PaybackDFILoanTest (DefiTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            ['-txnotokens=0', '-amkheight=50', '-bayfrontheight=50', '-bayfrontgardensheight=1', '-eunosheight=50',
                '-fortcanningheight=50', '-fortcanninghillheight=50', '-fortcanningroadheight=50', '-simulatemainnet', '-txindex=1']
        ]
    ...
    def run_test(self):
        ...
        info = self.nodes[0].getburninfo()
        assert_equal(info['dfipaybackfee'], Decimal('10.48421532'))
        assert_equal(info['dfipaybacktokens'], ['2000.00091102@DUSD'])

        attribs = self.nodes[0].getgov('ATTRIBUTES')['ATTRIBUTES']
        assert_equal(attribs['v0/live/economy/dfi_payback_tokens'], ['10.48421532@DFI', '2000.00091102@DUSD'])
        ...
        info = self.nodes[0].getburninfo()
        assert_equal(info['dfipaybackfee'], Decimal('21.01053711'))
        assert_equal(info['dfipaybacktokens'], ['4000.00205258@DUSD'])
        ...
        # block height 393, FortCanningRoad active
        # Test TSLA DFI payback
        ...
        self.nodes[0].setgov({"ATTRIBUTES":{'v0/token/' + idTSLA + '/payback_dfi_fee_pct':'0.01'}})
        self.nodes[0].generate(1)

        vaultBefore = self.nodes[0].getvault(vaultId2)
        [amountBefore, _] = vaultBefore['loanAmounts'][0].split('@')
        [balanceDFIBefore, _] = self.nodes[0].getaccount(account0)[0].split('@')
        ...
        vaultAfter = self.nodes[0].getvault(vaultId2)
        [amountAfter, _] = vaultAfter['loanAmounts'][0].split('@')
        [interestAfter, _] = vaultAfter['interestAmounts'][0].split('@')
        [balanceDFIAfter, _] = self.nodes[0].getaccount(account0)[0].split('@')
        ...
        [balanceDFIAfter, _] = self.nodes[0].getaccount(account0)[0].split('@')
        assert_equal(Decimal(balanceDFIBefore) - Decimal(balanceDFIAfter), Decimal('4'))
        ...
        assert_equal(Decimal(balanceDFIBefore) - Decimal(balanceDFIAfter), Decimal('8.53684423'))
        assert_equal(len(vaultAfter['loanAmounts']), 0)
        assert_equal(len(vaultAfter['interestAmounts']), 0)
