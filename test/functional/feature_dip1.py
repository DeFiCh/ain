...
class Dip1Test(DefiTestFramework):
    ...
    def run_test(self):
        assert_equal(
            self.nodes[0].getblockchaininfo()["softforks"]["amk"]["active"], False
        )
        ...
        assert_equal(
            self.nodes[0].getblockchaininfo()["softforks"]["amk"]["active"], True
        )  # not active IRL, so getblockchaininfo works like "height+1"
        ...
        )  # foundationAddress (2NCWAKfEehP3qibkLKYQjXaWMK23k4EDMVS) privkey
        assert_equal(self.nodes[0].getbalances()["mine"]["immature"], Decimal("89.9"))

        assert_equal(
            self.nodes[0].listcommunitybalances()["AnchorReward"], Decimal("0.1")
        )
        ...
        assert_equal(
            self.nodes[0].listcommunitybalances()["AnchorReward"], Decimal("0.3")
        )
        assert_equal(self.nodes[0].listcommunitybalances()["Burnt"], 30)
        assert_equal(
            self.nodes[1].listcommunitybalances()["AnchorReward"], Decimal("0.2")
        )
        ...
        assert_equal(
            self.nodes[1].listcommunitybalances()["AnchorReward"], Decimal("0.3")
        )
