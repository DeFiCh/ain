...
class TokenSplitTest(DefiTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            [
                ...
                f"-fortcanningcrunchheight={self.fort_canning_crunch}",
                "-subsidytest=1",
            ]
        ]
    ...
    def setup_test_pools(self):
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/poolpairs/{self.idGD}/token_a_fee_pct": "0.01",
        ...
        for _ in range(100):
            ...
            self.nodes[0].addpoolliquidity(
                {
                    self.address: [
                        f"{str(amount)}@{self.idDUSD}",
                        f"{str(amount)}@{self.idGOOGL}",
                    ]
                },
                self.address,
            )
        ...
        for vault in range(100):
            ...
            for _ in range(1, 4):
                ...
                self.nodes[0].takeloan(
                    {"vaultId": vault_id, "amounts": f"{loan}@{self.symbolNVDA}"}
                )
                self.nodes[0].generate(1)
    ...
    def check_pool_split(
        ...
        assert_equal(
            result["creationTx"],
            self.nodes[0].getblock(self.nodes[0].getbestblockhash())["tx"][2],
        )
        assert_equal(result["creationHeight"], self.nodes[0].getblockcount())
    ...
    def token_split(self):
        ...
        self.nodes[0].generate(
            self.fort_canning_crunch - 1 - self.nodes[0].getblockcount()
        )

        # Mined in fork height, still using old code
        self.nodes[0].updateloantoken(self.idGOOGL, {"name": "AAAA", "interest": 1})
        self.nodes[0].generate(1)
        ...
        assert_equal(
            result[f"v0/token/{self.idGOOGL}/fixed_interval_price_id"],
            f"{self.symbolGOOGL}/USD",
        )
        ...
        self.nodes[0].updateloantoken(
            self.idGOOGL, {"name": self.symbolGOOGL, "interest": 0}
        )
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idTSLA}/dfip2203": "true",
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/oracles/splits/{str(self.nodes[0].getblockcount() + 2)}": f"{self.idTSLA}/2"
                }
            }
        )
        ...
        assert_equal(
            result[f"v0/token/{self.idTSLA}/loan_payback/{self.idDUSD}"], "true"
        )
        assert_equal(
            result[f"v0/token/{self.idGOOGL}/loan_payback/{self.idTSLA}"], "true"
        )
        assert_equal(
            result[f"v0/token/{self.idTSLA}/loan_payback/{self.idTSLA}"], "true"
        )
        assert_equal(
            result[f"v0/token/{self.idTSLA}/loan_payback_fee_pct/{self.idDUSD}"], "0.25"
        )
        assert_equal(
            result[f"v0/token/{self.idGOOGL}/loan_payback_fee_pct/{self.idTSLA}"],
            "0.25",
        )
        assert_equal(
            result[f"v0/token/{self.idTSLA}/loan_payback_fee_pct/{self.idTSLA}"], "0.25"
        )
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/oracles/splits/{str(self.nodes[0].getblockcount() + 2)}": f"{self.idTSLA}/-3"
                }
            }
        )
        self.nodes[0].generate(2)
    ...
    def pool_split(self):
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/oracles/splits/{str(self.nodes[0].getblockcount() + 2)}": f"{self.idGOOGL}/2"
                }
            }
        )
        self.nodes[0].generate(2)
        ...
        balances = self.nodes[0].gettokenbalances(
            {"start": int(self.idGD), "including_start": True, "limit": 1}, False, True
        )
        ...
        assert_equal(
            result[0]["amounts"],
            [f'-{self.poolGDTotal - Decimal("0.00001")}@{self.symbolGD}/v1'],
        )
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/oracles/splits/{str(self.nodes[0].getblockcount() + 2)}": f"{self.idGOOGL}/-3"
                }
            }
        )
        self.nodes[0].generate(2)
    ...
    def execute_vault_split(self, token_id, token_symbol, multiplier, suffix):
        ...
        if multiplier < 0:
            minted = truncate(
                str(
                    self.nodes[0].gettoken(token_id)[token_id]["minted"]
                    / abs(multiplier)
                ),
                8,
            )
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/oracles/splits/{str(self.nodes[0].getblockcount() + 2)}": f"{token_id}/{multiplier}"
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        for [vault_id, pre_vault] in self.vault_balances:
            ...
            if multiplier < 0:
                pre_loan_scaled = Decimal(
                    truncate(str(pre_loan_amount / abs(multiplier)), 8)
                )
            else:
        ...
        if multiplier < 0:
            expected_interest = Decimal(
                truncate(str(pre_get_interest["interestPerBlock"] / abs(multiplier)), 8)
            )
        else:
        ...
        if (
            current_interest != expected_interest
            and current_interest_round != expected_interest
        ):
    ...
    def vault_split(self):
        ...
        # Deposit to vault
        self.nodes[0].deposittovault(
            self.skip_vault, self.address, f"100@{self.symbolDFI}"
        )
        self.nodes[0].generate(1)

        # Take loan
        self.nodes[0].takeloan(
            {"vaultId": self.skip_vault, "amounts": f"100@{self.symbolNVDA}"}
        )
        self.nodes[0].generate(1)
        ...
        # Check split history
        result = self.nodes[0].listvaulthistory(
            self.vault_id, {"maxBlockHeight": self.nodes[0].getblockcount(), "depth": 1}
        )
    ...
    def check_govvar_deletion(self):
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/oracles/splits/{split_height}": f"{self.idTSLA}/2"}}
        )
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/oracles/splits/500000": f"{self.idTSLA}/2"}}
        )
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/oracles/splits/1000000": f"{self.idTSLA}/2"}}
        )
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/oracles/splits/1000001": f"{self.idNVDA}/2"}}
        )
        self.nodes[0].generate(1)
    ...
    def check_future_swap_refund(self):
        ...
        self.nodes[0].accounttoaccount(
            self.address,
            {address_msft: [f"1@{self.symbolMSFT}", f"1@{self.symbolDUSD}"]},
        )
        self.nodes[0].accounttoaccount(
            self.address,
            {address_locked: [f"1@{self.symbolMSFT}", f"1@{self.symbolDUSD}"]},
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/oracles/splits/{str(self.nodes[0].getblockcount() + 2)}": f"{self.idMSFT}/2"
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_raises_rpc_error(
            -32600,
            "Cannot create future swap for locked token",
        ...
        result = self.nodes[0].getaccount(address_msft)
        assert_equal(
            result, [f"1.00000000@{self.symbolDUSD}", f"2.00000000@{self.symbolMSFT}"]
        )

        # Check future swap and token split account history
        result = self.nodes[0].listaccounthistory(
            address_msft, {"maxBlockHeight": self.nodes[0].getblockcount(), "depth": 0}
        )
        assert_equal(result[0]["owner"], address_msft)
    ...
    def migrate_auction_batches(self):
        ...
        # Take loans
        self.nodes[0].takeloan({"vaultId": vault1, "amounts": "1@MSFT"})
        self.nodes[0].takeloan({"vaultId": vault2, "amounts": "1@MSFT"})
        self.nodes[0].takeloan({"vaultId": vault2, "amounts": "1@MSFT"})
        self.nodes[0].takeloan({"vaultId": vault3, "amounts": "2@GOOGL"})
        self.nodes[0].generate(1)
        self.nodes[0].takeloan({"vaultId": vault4, "amounts": "1@NVDA"})
        self.nodes[0].generate(1)
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/oracles/splits/{str(self.nodes[0].getblockcount() + 3)}": f"{self.idTWTR}/2"
                }
            }
        )
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/oracles/splits/{str(self.nodes[0].getblockcount() + 4)}": f"{self.idNVDA}/2"
                }
            }
        )
        ...
        assert_equal(
            vault1_result["batches"][0]["loan"], f"2.00000036@{self.symbolMSFT}"
        )
        assert_equal(
            vault1_result["batches"][0]["highestBid"]["amount"],
            f"2.20000000@{self.symbolMSFT}",
        )
        assert_equal(
            vault1_result["batches"][0]["collaterals"], [f"1.00000000@{self.symbolDFI}"]
        )
        assert_equal(
            vault2_result["batches"][0]["loan"], f"4.00000070@{self.symbolMSFT}"
        )
        assert_equal(
            vault2_result["batches"][0]["collaterals"], [f"2.00000000@{self.symbolDFI}"]
        )
        assert_equal(
            vault3_result["batches"][0]["loan"], f"2.00000035@{self.symbolGOOGL}"
        )
        assert_equal(
            vault3_result["batches"][0]["collaterals"],
