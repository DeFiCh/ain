...
class PaybackDFILoanTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        self.nodes[0].takeloan({"vaultId": vaultId, "amounts": "2000@" + symboldUSD})
        self.nodes[0].generate(1)

        # Check total DUSD loan amount
        attributes = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attributes["v0/live/economy/loans"], [f"2000.00000000@{symboldUSD}"]
        )
        ...
        assert_raises_rpc_error(
            -32600,
            "Loan token with id (1) does not exist!",
            self.nodes[0].paybackloan,
            {"vaultId": vaultId, "from": account0, "amounts": "1@BTC"},
        )
        ...
        assert_raises_rpc_error(
            ...
            self.nodes[0].paybackloan,
            {"vaultId": vaultId, "from": account0, "amounts": "1@DFI"},
        )
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/token/" + iddUSD + "/payback_dfi": "false"}}
        )
        self.nodes[0].generate(1)
        ...
        # Enable loan payback
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/token/" + iddUSD + "/payback_dfi": "true"}}
        )
        self.nodes[0].generate(1)
        ...
        attributes = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attributes["v0/live/economy/loans"], [f"1990.11141553@{symboldUSD}"]
        )
        ...
        assert_equal(
            ...
            (Decimal(amountBefore) - (10 * Decimal("0.99"))),
        )

        # Test 5% penalty
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/token/" + iddUSD + "/payback_dfi_fee_pct": "0.05"}}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(amountAfter) - Decimal(interestAfter),
            (Decimal(amountBefore) - (10 * Decimal("0.95"))),
        )
        ...
        self.nodes[0].paybackloan(
            {"vaultId": vaultId, "from": account0, "amounts": "250@DFI"}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            ...
            ["10.48430642@DFI", "2000.01822015@DUSD"],
        )
        ...
        assert_equal(
            ...
            (Decimal(amountBefore) / Decimal("9.5")).quantize(
        ...
        self.nodes[0].paybackloan(
            {"vaultId": vaultId, "from": account0, "amounts": "210.52871906@DFI"}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].takeloan({"vaultId": vaultId2, "amounts": "10@" + symbolTSLA})
        self.nodes[0].generate(1)
        ...
        assert_raises_rpc_error(
            ...
            self.nodes[0].paybackloan,
            {"vaultId": vaultId2, "from": account0, "amounts": "10@DFI"},
        )
        ...
        self.nodes[0].takeloan({"vaultId": vaultId6, "amounts": "100@DUSD"})
        self.nodes[0].generate(1)

        # Check total DUSD loan amount is just the new loan amount
        attributes = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attributes["v0/live/economy/loans"], [f"100.00000000@{symboldUSD}"]
        )
        ...
        self.nodes[0].paybackloan(
            {"vaultId": vaultId2, "from": account0, "amounts": ["1@TSLA"]}
        )
        self.nodes[0].generate(1)
        ...
        [balanceDUSDAfter, _] = self.nodes[0].getaccount(burnAddress)[1].split("@")
        assert_equal(
            Decimal(balanceDUSDAfter) - Decimal(balanceDUSDBefore), Decimal("0")
        )
        assert_equal(
            Decimal(balanceDFIAfter) - Decimal(balanceDFIBefore), Decimal("0.95477661")
        )
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/params/dfip2206a/dusd_loan_burn": "true"}}
        )
        self.nodes[0].generate(1)
        ...
        [balanceDUSDAfter, _] = self.nodes[0].getaccount(burnAddress)[1].split("@")
        assert_equal(
            Decimal(balanceDUSDAfter) - Decimal(balanceDUSDBefore),
            Decimal("9.68801108"),
        )
        assert_equal(Decimal(balanceDFIAfter) - Decimal(balanceDFIBefore), Decimal("0"))
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/token/" + idTSLA + "/payback_dfi": "true"}}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(balanceDFIAfter) - Decimal(balanceDFIBefore), Decimal("1.00000000")
        )
        ...
        self.nodes[0].takeloan({"vaultId": vaultId2, "amounts": "100@DUSD"})
        self.nodes[0].generate(10)
        ...
        self.nodes[0].paybackloan(
            {"vaultId": vaultId2, "from": account0, "amounts": ["10@DUSD"]}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(balanceDUSDAfter) - Decimal(balanceDUSDBefore), Decimal("10")
        )
        assert_equal(Decimal(balanceDFIAfter) - Decimal(balanceDFIBefore), Decimal("0"))
