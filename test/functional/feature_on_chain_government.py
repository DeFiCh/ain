...
class OnChainGovernanceTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            result["nonutxo"][0]["CommunityDevelopmentFunds"], Decimal("19.88746400")
        )

        # Check errors
        assert_raises_rpc_error(
            -32600,
            "proposal cycles can be between 1 and 100",
            self.nodes[0].creategovcfp,
            {
                "title": "test",
                "context": context,
        ...
        try:
            self.nodes[0].creategovvoc({"title": title, "context": "a", "amounts": 100})
        except JSONRPCException as e:
            errorString = e.error["message"]
        ...
        # Create CFP
        cfp1 = self.nodes[0].creategovcfp(
            {
                "title": title,
                "context": context,
                "amount": 100,
        ...
        assert_raises_rpc_error(
            -5, "Incorrect authorization", self.nodes[0].votegov, cfp1, mn1, "yes"
        )

        assert_raises_rpc_error(
            -8,
            "Decision supports yes or no. Neutral is currently disabled because of issue https://github.com/DeFiCh/ain/issues/1704",
        ...
        assert_raises_rpc_error(
            None,
            "does not mine at least one block",
            self.nodes[3].votegov,
            cfp1,
            mn3,
            "yes",
        )
        ...
        cycle1 = (
            creationHeight
            + (votingPeriod - creationHeight % votingPeriod)
            + votingPeriod
        )
        ...
        assert_equal(
            self.nodes[0].listcommunitybalances()["CommunityDevelopmentFunds"],
            bal + Decimal("19.887464") - Decimal(100),
        )
        ...
        # cannot vote by non owning masternode
        assert_raises_rpc_error(
            -5, "Incorrect authorization", self.nodes[0].votegov, propId, mn1, "yes"
        )
        ...
        # not votes on 2nd cycle makes proposal to rejected
        result = self.nodes[0].listgovproposals(
            {"pagination": {"start": propId, "including_start": True}}
        )[0]
        assert_equal(result["status"], "Rejected")
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {"v0/gov/proposals/voc_emergency_quorum": "80.01%"}}
        )
        self.nodes[0].generate(1)
        self.sync_blocks()
        ...
        assert_equal(
            len(self.nodes[0].listgovproposals("all", "rejected", 0, {"limit": 1})), 1
        )
        assert_equal(
            len(self.nodes[0].listgovproposals("all", "rejected", 0, {"limit": 0})), 4
        )
        assert_equal(
            len(self.nodes[0].listgovproposals("all", "rejected", 0, {"limit": 10})), 4
        )
        assert_equal(
            self.nodes[0].listgovproposals(
                "all", "rejected", 0, {"start": tx, "including_start": True}
            )[0]["proposalId"],
            tx,
        )

        assert_equal(
            len(self.nodes[0].listgovproposals({"type": "all", "status": "voting"})), 0
        )
        assert_equal(
            len(self.nodes[0].listgovproposals({"type": "all", "status": "completed"})),
            1,
        )
        assert_equal(
            len(self.nodes[0].listgovproposals({"type": "all", "status": "rejected"})),
            4,
        )
        ...
        assert_equal(
            len(
                self.nodes[0].listgovproposals(
                    {"type": "all", "status": "rejected", "pagination": {"limit": 1}}
                )
            ),
            1,
        )
        assert_equal(
            len(
                self.nodes[0].listgovproposals(
                    {"type": "all", "status": "rejected", "pagination": {"limit": 0}}
                )
            ),
            4,
        )
        assert_equal(
            len(
                self.nodes[0].listgovproposals(
                    {"type": "all", "status": "rejected", "pagination": {"limit": 10}}
                )
            ),
            4,
        )
            ...
            self.nodes[0].listgovproposals(
                {
                    "type": "all",
                    "status": "rejected",
                    "pagination": {"start": tx, "including_start": True},
                }
            )[0]["proposalId"],
            tx,
        )
        ...
        assert_equal(
            len(
                self.nodes[1].listgovproposalvotes(
                    {
                        "proposalId": tx,
                        "masternode": "all",
                        "cycle": -1,
                ...
                self.nodes[1].listgovproposalvotes(
                    {
                        ...
                        "masternode": "all",
                        "cycle": -1,
                        "pagination": {"start": 0, "including_start": True},
                    }
                )
            ),
            3,
                ...
                self.nodes[1].listgovproposalvotes(
                    {
                        ...
                        "masternode": "all",
                        "cycle": -1,
                        "pagination": {"start": 0, "including_start": True, "limit": 2},
                    }
                )
            ),
            2,
                ...
                self.nodes[1].listgovproposalvotes(
                    {
                        ...
                        "masternode": "all",
                        "cycle": -1,
                        "pagination": {"start": 0, "including_start": True, "limit": 1},
                    }
                )
            ),
            1,
                ...
                self.nodes[1].listgovproposalvotes(
                    {
                        ...
                        "masternode": "all",
                        "cycle": -1,
                        "pagination": {"start": 0, "including_start": True, "limit": 0},
                    }
                )
            ),
            3,
        ...
        assert_equal(
            len(self.nodes[0].listgovproposalvotes(propId, "all", -1, {"limit": 2})), 2
        )

        tx1 = self.nodes[0].creategovcfp(
            {
                "title": "1111",
                "context": context,
                "amount": 50,
        ...
        assert_equal(
            self.nodes[0].listgovproposals(
                {
                    "cycle": 1,
                    "pagination": {"start": cfp1, "including_start": True, "limit": 1},
                }
            )[0]["proposalId"],
            cfp1,
        )
        ...
        assert_equal(
            self.nodes[0].listgovproposals(
                {
                    "cycle": 6,
                    "pagination": {"start": tx2, "including_start": True, "limit": 1},
                }
            )[0]["proposalId"],
            tx2,
        )
        assert_equal(
            self.nodes[0].listgovproposals(
                {
                    "cycle": 6,
                    "pagination": {"start": tx3, "including_start": True, "limit": 1},
                }
            )[0]["proposalId"],
            tx3,
        )
        ...
        assert_equal(
            self.nodes[0].listgovproposals(
                {
                    "status": "voting",
                    "pagination": {"start": tx1, "including_start": True, "limit": 1},
                }
            )[0]["proposalId"],
            tx1,
        )
        ...
        assert_equal(
            self.nodes[0].listgovproposals(
                {
                    "status": "voting",
                    "pagination": {"start": tx1, "including_start": False, "limit": 1},
                }
            ),
            nextProposal,
        )
    ...
    def test_aggregation(self, propId):
        ...
        votes_aggregate = self.nodes[0].listgovproposalvotes(
            propId, "all", -1, {}, True
        )[0]
    ...
    def aggregate_all_votes(self):
        ...
        proposalVotes = self.nodes[0].listgovproposalvotes(
            self.proposalId, "all", 0, {}, True
        )
        filteredVotes = list(
            filter(lambda vote: vote["proposalId"] == self.proposalId, votes)
        )
    ...
    def test_valid_votes(self):
        ...
        validVotes = self.nodes[0].listgovproposalvotes(
            propId, "all", 1, {}, False, True
        )
        invalidVotes = self.nodes[0].listgovproposalvotes(
            propId, "all", 1, {}, False, False
        )
