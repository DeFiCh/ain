...
class AuctionsTest(DefiTestFramework):
    ...
    def run_test(self):
        ...
        oracle1_prices = [
            ...
            {"currency": "USD", "tokenAmount": "100@MSFT"},
        ...
        self.nodes[0].addpoolliquidity(
            {account: ["1000@DUSD", "1000@TSLA"]}, account, []
        )

        # Add liquidity to DUSD-GOOGL
        self.nodes[0].addpoolliquidity(
            {account: ["1000@DUSD", "1000@GOOGL"]}, account, []
        )

        # Add liquidity to DUSD-TWTR
        self.nodes[0].addpoolliquidity(
            {account: ["1000@DUSD", "1000@TWTR"]}, account, []
        )

        # Add liquidity to DUSD-MSFT
        self.nodes[0].addpoolliquidity(
            {account: ["1000@DUSD", "1000@MSFT"]}, account, []
        )

        # Add liquidity to DUSD-DFI
        self.nodes[0].addpoolliquidity(
            {account: ["1000@DUSD", "1000@DFI"]}, account, []
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].takeloan({"vaultId": vaultId1, "amounts": "5@TSLA"})
        self.nodes[0].generate(1)
        # trigger liquidation
        oracle1_prices = [
            {"currency": "USD", "tokenAmount": "101@TSLA"},
            {"currency": "USD", "tokenAmount": "100@DFI"},
            {"currency": "USD", "tokenAmount": "100@BTC"},
        ]
        timestamp = calendar.timegm(time.gmtime())
        ...
        self.nodes[0].takeloan({"vaultId": vaultId2, "amounts": "60@TSLA"})
        self.nodes[0].generate(1)
        ...
        assert_equal(
            vault2["batches"][0]["collaterals"], ["49.99999980@DFI", "49.99999980@BTC"]
        )
        assert_equal(
            vault2["batches"][1]["collaterals"], ["10.00000020@DFI", "10.00000020@BTC"]
        )
        ...
        assert_equal(
            interest[0]["interestPerBlock"],
            Decimal(vault2["interestAmounts"][0].split("@")[0]),
        )
        assert_greater_than(
            Decimal(vault2["collateralAmounts"][0].split("@")[0]), Decimal(10.00000020)
        )
        assert_equal(vault2["informativeRatio"], Decimal("264.70081509"))
        self.nodes[0].paybackloan(
            {"vaultId": vaultId2, "from": account, "amounts": vault2["loanAmounts"]}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].takeloan({"vaultId": vaultId3, "amounts": "60@TSLA"})
        self.nodes[0].generate(1)
        ...
        assert_equal(len(auctionlist), 1)
        assert_greater_than(
            Decimal(vault3["collateralAmounts"][0].split("@")[0]), Decimal(10.00000020)
        )

        self.nodes[0].paybackloan(
            {"vaultId": vaultId3, "from": account, "amounts": vault3["loanAmounts"]}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].generate(1)
        self.nodes[0].takeloan({"vaultId": vaultId4, "amounts": "5@TSLA"})
        self.nodes[0].generate(1)
        oracle1_prices = [
            {"currency": "USD", "tokenAmount": "101@TSLA"},
            {"currency": "USD", "tokenAmount": "100@DFI"},
        ...
        self.nodes[0].generate(1)
        assert_equal(
            Decimal(self.nodes[0].getaccount(account)[0].split("@")[0]),
            collateralAmount + accountDFIBalance,
        )
        ...
        self.nodes[0].generate(1)
        self.nodes[0].takeloan({"vaultId": vaultId5, "amounts": "100@TSLA"})
        self.nodes[0].generate(1)
        oracle1_prices = [
            {"currency": "USD", "tokenAmount": "330@TSLA"},
            {"currency": "USD", "tokenAmount": "220@DFI"},
        ...
        # Take TSLA loan
        self.nodes[0].takeloan({"vaultId": vaultId6, "amounts": "172@TSLA"})

        # Take GOOGL loan
        self.nodes[0].takeloan({"vaultId": vaultId6, "amounts": "18@GOOGL"})
        self.nodes[0].generate(1)
        ...
        # Take TSLA loan
        self.nodes[0].takeloan({"vaultId": vaultId7, "amounts": "80@TSLA"})
        self.nodes[0].generate(1)

        oracle1_prices = [
            {"currency": "USD", "tokenAmount": "129@TSLA"},
            {"currency": "USD", "tokenAmount": "71@DFI"},
        ]
        timestamp = calendar.timegm(time.gmtime())
        self.nodes[0].setoracledata(oracle_id1, timestamp, oracle1_prices)
        self.nodes[0].generate(12)  # let price update and trigger liquidation of vault
        ...
        # Take TSLA loan
        self.nodes[0].takeloan({"vaultId": vaultId7, "amounts": "100@TSLA"})
        # Take GOOGL loan
        self.nodes[0].takeloan({"vaultId": vaultId7, "amounts": "60@GOOGL"})
        self.nodes[0].generate(1)

        oracle1_prices = [
            {"currency": "USD", "tokenAmount": "129@TSLA"},
            {"currency": "USD", "tokenAmount": "129@GOOGL"},
            {"currency": "USD", "tokenAmount": "71@DFI"},
        ...
        # Take TSLA loan
        self.nodes[0].takeloan({"vaultId": vaultId8, "amounts": "25@TSLA"})

        # Take GOOGL loan
        self.nodes[0].takeloan({"vaultId": vaultId8, "amounts": "25@GOOGL"})

        # Take TWTR loan
        self.nodes[0].takeloan({"vaultId": vaultId8, "amounts": "25@TWTR"})

        # Take MSFT loan
        self.nodes[0].takeloan({"vaultId": vaultId8, "amounts": "25@MSFT"})
        self.nodes[0].generate(1)

        vault8 = self.nodes[0].getvault(vaultId8)

        oracle1_prices = [
            {"currency": "USD", "tokenAmount": "300@TSLA"},
