...
class NegativeInterestTest(DefiTestFramework):
    ...
    def setup_test_oracles(self):
        ...
        self.oracle_id2 = self.nodes[0].appointoracle(
            self.oracle_address2, price_feed, 10
        )
        self.nodes[0].generate(1)
    ...
    def test_negative_interest(self):
        ...
        self.nodes[0].accounttoaccount(
            self.address, {vault_address: f"1000@{self.symbolDFI}"}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idDUSD}/loan_minting_interest": "-5"}}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            Decimal(loanAmount),
            Decimal(loanAmountBefore) - Decimal("1.9999") + Decimal(interestAmount),
        )

        [balanceDUSDafter, _] = self.nodes[0].getaccount(vault_address)[1].split("@")
        assert_equal(
            Decimal(balanceDUSDafter), Decimal(balanceDUSDbefore) - Decimal("1.9999")
        )
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idDUSD}/loan_minting_interest": "-8000000"
                }
            }
        )
