...
class PoolPairTest(DefiTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            [
                ...
                "-amkheight=50",
                "-bayfrontheight=50",
                "-bayfrontgardensheight=0",
                "-dakotaheight=160",
                "-fortcanningheight=163",
                "-fortcanninghillheight=170",
                "-fortcanningroadheight=177",
    ...
    def init_accounts(self):
        ...
        self.nodes[1].accounttoaccount(
            self.accountSN1, {self.accountGN0: "1000@" + self.symbolSILVER}
        )
        self.nodes[1].generate(1)
        # Transferring GOLD from N0 Account to N1 Account
        self.nodes[0].accounttoaccount(
            self.accountGN0, {self.accountSN1: "200@" + self.symbolGOLD}
        )
        self.nodes[0].generate(1)

        self.silverCheckN0 = self.nodes[0].getaccount(self.accountGN0, {}, True)[
            self.idSilver
        ]
    ...
    def test_add_liquidity_from_different_nodes(self):
        ...
        silverCheckN0 = self.nodes[0].getaccount(self.accountGN0, {}, True)[
            self.idSilver
        ]
    ...
    def turn_on_pool_and_swap(self):
        ...
        self.goldCheckPS = self.nodes[2].getaccount(self.accountSN1, {}, True)[
            self.idGold
        ]
    ...
    def test_swap_and_live_dex_data(self):
        ...
        # silver is tokenB
        assert_equal(
            attributes["v0/live/economy/dex/%s/total_swap_b" % (self.idGS)],
            Decimal("9.0"),
        )
        assert_equal(
            attributes["v0/live/economy/dex/%s/total_commission_b" % (self.idGS)],
            Decimal("1.0"),
        )

        goldCheckN0 = self.nodes[2].getaccount(self.accountGN0, {}, True)[self.idGold]
        silverCheckN0 = self.nodes[2].getaccount(self.accountGN0, {}, True)[
            self.idSilver
        ]

        goldCheckN1 = self.nodes[2].getaccount(self.accountSN1, {}, True)[self.idGold]
        silverCheckN1 = self.nodes[2].getaccount(self.accountSN1, {}, True)[
            self.idSilver
        ]
        ...
        assert_equal(goldCheckN0, 700)
        assert_equal(
            str(silverCheckN0), "490.49999997"
        )  # TODO: calculate "true" values with trading fee!
        assert_equal(list_pool["1"]["reserveA"] + goldCheckN1, 300)
        assert_equal(
            Decimal(self.goldCheckPS) + Decimal(self.psTestAmount), Decimal(goldCheckN1)
        )
    ...
    def test_max_price(self):
        ...
        assert_raises_rpc_error(
            -26,
            "Price is higher than indicated",
            self.nodes[0].poolswap,
            {
                "from": self.accountGN0,
                "tokenFrom": self.symbolSILVER,
        ...
        attributes = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attributes["v0/live/economy/dex/%s/total_swap_a" % (self.idGS)],
            Decimal("180.0"),
        )
        assert_equal(
            attributes["v0/live/economy/dex/%s/total_commission_a" % (self.idGS)],
            Decimal("20.0"),
        )
        assert_equal(
            attributes["v0/live/economy/dex/%s/total_swap_b" % (self.idGS)],
    ...
    def test_fort_canning_max_price_change(self):
        ...
        self.silver_per_gold = round(
            (self.swap_from * self.coin) / (silver_received * self.coin), 8
        )
    ...
    def one_satoshi_swap(self):
        ...
        assert_equal(
            self.nodes[0].getaccount(new_dest, {}, True)[self.idBTC],
            Decimal("0.00000001"),
        )
    ...
    def test_two_satoshi_round_up(self, FCP=False):
        ...
        attributes = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attributes["v0/live/economy/dex/%s/total_swap_a" % (self.idBL)],
            Decimal("0.0"),
        )
        assert_equal(
            attributes["v0/live/economy/dex/%s/total_commission_a" % (self.idBL)],
            Decimal("0.0"),
        )
        assert_equal(
            attributes["v0/live/economy/dex/%s/total_swap_b" % (self.idBL)],
            Decimal("0.00000189"),
        )
        assert_equal(
            attributes["v0/live/economy/dex/%s/total_commission_b" % (self.idBL)],
            Decimal("1E-8"),
        )

        expected_round_up = FCP and Decimal("0.00000001") or Decimal("0.00000002")
        assert_equal(
            self.nodes[0].getaccount(new_dest, {}, True)[self.idBTC], expected_round_up
        )
    ...
    def set_token_and_pool_fees(self):
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    "v0/poolpairs/%s/token_a_fee_pct" % (self.idGS): "0.05",
                    "v0/poolpairs/%s/token_b_fee_pct" % (self.idGS): "0.08",
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        attributes = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attributes["v0/poolpairs/%s/token_a_fee_pct" % (self.idGS)], "0.05"
        )
        assert_equal(
            attributes["v0/poolpairs/%s/token_b_fee_pct" % (self.idGS)], "0.08"
        )
    ...
    def test_listaccounthistory_and_burninfo(self):
        ...
        assert_equal(
            self.nodes[0].listaccounthistory(
                self.accountGN0, {"token": self.symbolGOLD}
            )[0]["amounts"],
            ["-200.00000000@" + self.symbolGOLD],
        )

        assert_equal(
            self.nodes[0].getburninfo()["dexfeetokens"].sort(),
            [
                "%.8f" % (dexinfee) + self.symbolGOLD,
                "%.8f" % (dexoutfee) + self.symbolSILVER,
            ].sort(),
        )

        attributes = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attributes["v0/live/economy/dex/%s/fee_burn_a" % (self.idGS)], dexinfee
        )
        assert_equal(
            attributes["v0/live/economy/dex/%s/fee_burn_b" % (self.idGS)], dexoutfee
        )
    ...
    def update_comission_and_fee_to_1pct_pool1(self):
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    "v0/poolpairs/%s/token_a_fee_pct" % (self.idGS): "0.01",
                    "v0/poolpairs/%s/token_b_fee_pct" % (self.idGS): "0.01",
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        attributes = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attributes["v0/poolpairs/%s/token_a_fee_pct" % (self.idGS)], "0.01"
        )
        assert_equal(
            attributes["v0/poolpairs/%s/token_b_fee_pct" % (self.idGS)], "0.01"
        )
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    "v0/poolpairs/%s/token_a_fee_pct" % (self.idBL): "0.01",
                    "v0/poolpairs/%s/token_b_fee_pct" % (self.idBL): "0.01",
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            attributes["v0/poolpairs/%s/token_a_fee_pct" % (self.idBL)], "0.01"
        )
        assert_equal(
            attributes["v0/poolpairs/%s/token_b_fee_pct" % (self.idBL)], "0.01"
        )
    ...
    def update_comission_and_fee_to_1pct_pool2(self):
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    "v0/token/%s/dex_in_fee_pct" % (self.idLTC): "0.02",
                    "v0/token/%s/dex_out_fee_pct" % (self.idBTC): "0.05",
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        attributes = self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"]
        assert_equal(
            attributes["v0/live/economy/dex/%s/fee_burn_b" % (self.idBL)],
            round(dexinfee, 8),
        )
        assert_equal(
            attributes["v0/live/economy/dex/%s/fee_burn_a" % (self.idBL)],
            Decimal(str(round(dexoutfee, 8))),
        )
    ...
    def test_swap_to_self(self):
        ...
        self.nodes[0].accounttoaccount(
            self.accountGN0, {address: f"1@{self.symbolLTC}"}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            self.nodes[0].getaccount(address), [f"0.00759157@{self.symbolBTC}"]
        )
