...
class VaultTest(DefiTestFramework):
    ...
    def setup_accounts_and_tokens(self):
        ...
        self.nodes[0].setcollateraltoken(
            {"token": self.idDFI, "factor": 1, "fixedIntervalPriceId": "DFI/USD"}
        )

        self.nodes[0].setcollateraltoken(
            {"token": self.idBTC, "factor": 0.8, "fixedIntervalPriceId": "BTC/USD"}
        )
    ...
    def create_poolpairs(self):
        ...
        self.nodes[0].generate(1)
        self.nodes[0].addpoolliquidity(
            {self.accountDFI: ["100@TSLA", "100@DUSD"]}, self.accountDFI, []
        )
        self.nodes[0].generate(1)
    ...
    def createvault_with_invalid_parameters(self):
        ...
        assert (
            "recipient script (ffffffffff) does not solvable/non-standard"
            in errorString
        )
    ...
    def getvault_wrong_vault_address(self):
        try:
            self.nodes[0].getvault(
                "5474b2e9bfa96446e5ef3c9594634e1aa22d3a0722cb79084d61253acbdf87bf"
            )
        except JSONRPCException as e:
            errorString = e.error["message"]
        assert (
            "Vault <5474b2e9bfa96446e5ef3c9594634e1aa22d3a0722cb79084d61253acbdf87bf> not found"
            in errorString
        )
    ...
    def actions_on_vault_with_scheme_to_be_destroyed(self):
        ...
        assert_raises_rpc_error(
            -32600,
            "Cannot set LOAN0002 as loan scheme, set to be destroyed",
            self.nodes[0].createvault,
            self.owner_addresses[0],
            "LOAN0002",
        )
        ...
        assert_raises_rpc_error(
            -32600,
            "Cannot set LOAN0002 as loan scheme, set to be destroyed",
            self.nodes[0].updatevault,
            self.vaults[1],
            {"loanSchemeId": "LOAN0002"},
        )
    ...
    def deposittovault_with_invalid_params(self):
        ...
        try:
            self.nodes[0].deposittovault(
                self.vaults[0], self.accountDFI, "0.000000001@DFI"
            )
        except JSONRPCException as e:
            errorString = e.error["message"]
    ...
    def test_deposittovault(self):
        ...
        self.nodes[0].generate(1)
        self.nodes[0].withdrawfromvault(
            self.vaults[0], self.accountDFI, "0.00000001@DFI"
        )
        self.nodes[0].generate(1)
    ...
    def takeloan_breaking_50pctDFI_rule(self):
        assert_raises_rpc_error(
            ...
            {"vaultId": self.vaults[0], "amounts": "0.1@TSLA"},
        )
    ...
    def takeloan_with_50pctDFI(self):
        ...
        assert_equal(
            acDFI, ["99.30000000@DFI", "173.20507075@DUSD-DFI", "99.99999000@DUSD-TSLA"]
        )
        ...
        acDFI = self.nodes[0].getaccount(self.accountDFI)
        assert_equal(
            acDFI, ["99.00000000@DFI", "173.20507075@DUSD-DFI", "99.99999000@DUSD-TSLA"]
        )
        ...
        self.nodes[0].takeloan({"vaultId": self.vaults[0], "amounts": "0.5@TSLA"})
    ...
    def updatevault_to_scheme_with_lower_collateralization_ratio(self):
        ...
        self.nodes[0].takeloan({"vaultId": self.vaults[1], "amounts": "0.355@TSLA"})
        self.nodes[0].generate(1)
        vault = self.nodes[0].getvault(self.vaults[1])
        ...
        try:
            ...
            errorString = e.error["message"]
        assert (
            "Vault does not have enough collateralization ratio defined by loan scheme - 176 < 200"
            in errorString
        )
        self.nodes[0].generate(1)
    ...
    def test_estimatevault(self):
        vault = self.nodes[0].getvault(self.vaults[1])
        estimatevault = self.nodes[0].estimatevault(
            vault["collateralAmounts"], vault["loanAmounts"]
        )
    ...
    def test_50pctDFI_fresh_vault_takeloan_withdraw(self):
        ...
        self.sync_blocks()
        self.nodes[0].deposittovault(
            vault_id, address, "1.25@BTC"
        )  # 1.25@BTC as collateral factor 0.8
        self.nodes[0].deposittovault(vault_id, self.accountDFI, "1@DFI")
        self.nodes[0].generate(1)
        self.sync_blocks()
        self.nodes[0].takeloan({"vaultId": vault_id, "amounts": "1@TSLA"})
        self.nodes[0].generate(1)
    ...
    def test_50pctDFI_rule_after_BTC_price_increase(self):
        ...
        oracle_prices = [
            {"currency": "USD", "tokenAmount": "1@DFI"},
            {"currency": "USD", "tokenAmount": "1@TSLA"},
            {"currency": "USD", "tokenAmount": "3@BTC"},
        ]
        timestamp = calendar.timegm(time.gmtime())
        self.nodes[0].setoracledata(self.oracles[0], timestamp, oracle_prices)
        self.nodes[0].generate(240)
        ...
        self.nodes[0].withdrawfromvault(
            self.vaults[4], self.owner_addresses[2], "0.5@BTC"
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].takeloan({"vaultId": self.vaults[4], "amounts": "0.10@TSLA"})
        self.nodes[0].generate(1)
    ...
    def overflowed_collateral_value(self):
        ...
        self.nodes[0].generate(1)
        self.nodes[0].deposittovault(
            vault_id, self.accountDFI, "100000000@" + self.symbolDFI
        )
        self.nodes[0].generate(1)

        self.nodes[0].takeloan({"vaultId": vault_id, "amounts": "0.5@TSLA"})
        self.nodes[0].generate(1)

        oracle1_prices = [
            {"currency": "USD", "tokenAmount": "9999999999@DFI"},
            {"currency": "USD", "tokenAmount": "1@TSLA"},
            {"currency": "USD", "tokenAmount": "1@BTC"},
            {"currency": "USD", "tokenAmount": "1@ETH"},
            {"currency": "USD", "tokenAmount": "2@GOOGL"},
        ]
        timestamp = calendar.timegm(time.gmtime())
        self.nodes[0].setoracledata(self.oracles[0], timestamp, oracle1_prices)
        self.nodes[0].generate(240)
        ...
        assert_raises_rpc_error(
            ...
            self.nodes[0].takeloan,
            {"vaultId": vault_id, "amounts": "0.5@TSLA"},
        )
        assert_raises_rpc_error(
            -32600,
            "Value/price too high",
        ...
        self.nodes[0].paybackloan(
            {"vaultId": vault_id, "from": address, "amounts": ["1@TSLA"]}
        )
        self.nodes[0].generate(1)
    ...
    def loan_and_collateral_token_to_govvar(self):
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idETH}/fixed_interval_price_id": "ETH/USD",
                    f"v0/token/{self.idETH}/loan_collateral_enabled": "true",
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idETH}/fixed_interval_price_id": "ETH/USD",
                    f"v0/token/{self.idETH}/loan_collateral_factor": "0.5",
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            self.nodes[0].gettoken(self.idGOOGL)[self.idGOOGL]["isLoanToken"], False
        )
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idGOOGL}/fixed_interval_price_id": "GOOGL/USD",
                    f"v0/token/{self.idGOOGL}/loan_minting_enabled": "true",
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idGOOGL}/fixed_interval_price_id": "GOOGL/USD",
                    f"v0/token/{self.idGOOGL}/loan_minting_interest": "1",
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            self.nodes[0].gettoken(self.idGOOGL)[self.idGOOGL]["isLoanToken"], True
        )
        result = self.nodes[0].listloantokens()
        assert_equal(len(result), 3)
        ...
        oracle_prices = [
            {"currency": "USD", "tokenAmount": "1@DFI"},
            {"currency": "USD", "tokenAmount": "1@TSLA"},
            {"currency": "USD", "tokenAmount": "0.5@BTC"},
            {"currency": "USD", "tokenAmount": "1@ETH"},
            {"currency": "USD", "tokenAmount": "2@GOOGL"},
        ]
        timestamp = calendar.timegm(time.gmtime())
        ...
        self.nodes[0].generate(1)
        self.nodes[0].deposittovault(
            vaultId6, self.nodes[0].get_genesis_keys().ownerAuthAddress, "5@ETH"
        )
        self.nodes[0].generate(1)
        ...
        assert_raises_rpc_error(
            -32600,
            f"Collateral token with id ({self.idGOOGL}) does not exist!",
            self.nodes[0].deposittovault,
            vaultId6,
            self.nodes[0].get_genesis_keys().ownerAuthAddress,
            "1@GOOGL",
        )
        ...
        assert_raises_rpc_error(
            -32600,
            f"Loan token with id ({self.idETH}) does not exist!",
            self.nodes[0].takeloan,
            {"vaultId": vaultId6, "amounts": "1@ETH"},
        )
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idETH}/loan_collateral_factor": "1"}}
        )
        ...
        assert_equal(
            self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"][
                f"v0/token/{self.idETH}/loan_collateral_factor"
            ],
            "1",
        )
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idETH}/fixed_interval_price_id": "BTC/USD"
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"][
                f"v0/token/{self.idETH}/fixed_interval_price_id"
            ],
            "BTC/USD",
        )
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idETH}/loan_collateral_enabled": "false"}}
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            self.nodes[0].getgov("ATTRIBUTES")["ATTRIBUTES"][
                f"v0/token/{self.idETH}/loan_collateral_enabled"
            ],
            "false",
        )
        ...
        assert_raises_rpc_error(
            -32600,
            f"Collateral token ({self.idETH}) is disabled",
            self.nodes[0].deposittovault,
            vaultId6,
            self.nodes[0].get_genesis_keys().ownerAuthAddress,
            "1@ETH",
        ...
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idGOOGL}/loan_minting_interest": "0"}}
        )
        self.nodes[0].generate(1)
        ...
        self.nodes[0].generate(1)
        assert_equal(
            self.nodes[0].getvault(vaultId6)["interestValue"],
            interest + Decimal("0.000000120"),
        )
        ...
        self.nodes[0].setgov(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idGOOGL}/loan_collateral_enabled": "true",
                    f"v0/token/{self.idGOOGL}/loan_collateral_factor": "0.5",
                }
            }
        )
        self.nodes[0].generate(1)
        ...
        assert_equal(
            result["collateralAmounts"],
            ["5.00000000@DFI", "5.00000000@ETH", "1.00000000@GOOGL"],
        )

        # Disable loan token
        self.nodes[0].setgov(
            {"ATTRIBUTES": {f"v0/token/{self.idGOOGL}/loan_minting_enabled": "false"}}
        )
        self.nodes[0].generate(1)
        ...
        assert_raises_rpc_error(
            -32600,
            f'Loan cannot be taken on token with id ({self.idGOOGL}) as "mintable" is currently false',
            self.nodes[0].takeloan,
            {"vaultId": vaultId6, "amounts": "1@GOOGL"},
        )
        ...
        self.nodes[0].setgovheight(
            {
                "ATTRIBUTES": {
                    f"v0/token/{self.idAAPL}/fixed_interval_price_id": "GOOGL/USD",
                    f"v0/token/{self.idAAPL}/loan_minting_enabled": "true",
                    f"v0/token/{self.idAAPL}/loan_minting_interest": "1",
                    f"v0/token/{self.idUSDT}/fixed_interval_price_id": "GOOGL/USD",
                    f"v0/token/{self.idUSDT}/loan_collateral_enabled": "true",
                    f"v0/token/{self.idUSDT}/loan_collateral_factor": "1",
                }
            },
            activation_height,
        )
        ...
        assert_equal(
            result[f"v0/token/{self.idUSDT}/fixed_interval_price_id"], "GOOGL/USD"
        )
        ...
        assert_equal(
            result[f"v0/token/{self.idAAPL}/fixed_interval_price_id"], "GOOGL/USD"
        )
