...
class VaultTest (DefiTestFramework):
    ...
    def takeloan_breaking_50pctDFI_rule(self):
        try:
            ...
            error_str = e.error['message']
        assert("At least 50% of the minimum required collateral must be in DFI or DUSD when taking a loan" in error_str)
    ...
    def test_50pctDFI_rule_after_BTC_price_increase(self):
        ...
        try:
            ...
            errorString = e.error['message']
        assert("At least 50% of the minimum required collateral must be in DFI or DUSD" in errorString)
