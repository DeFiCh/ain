...
//        time++;
//        pblock->nTime = time;
//    } while (!pos::CheckKernelHash(pblock->stakeModifier, pblock->nBits, 1, (int64_t) pblock->nTime, masternodeID, Params().GetConsensus()).hashOk);
...
BOOST_AUTO_TEST_CASE(calc_kernel)
{
    ...
    BOOST_CHECK(pos::CheckKernelHash(stakeModifier, target, 1, coinstakeTime, mnID, Params().GetConsensus()).hashOk);

    uint32_t unattainableTarget = 0x00ffffff;
    BOOST_CHECK(!pos::CheckKernelHash(stakeModifier, unattainableTarget, 1, coinstakeTime, mnID, Params().GetConsensus()).hashOk);
