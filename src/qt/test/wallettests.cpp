...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<DefiAmountField*>("payAmount")->setValue(amount);
...
//
//     src/qt/test/test_defi-qt -platform xcb      # Linux
//     src/qt/test/test_defi-qt -platform windows  # Windows
//     src/qt/test/test_defi-qt -platform cocoa    # macOS
void TestGUI()
{
    ...
    for (int i = 0; i < 5; ++i) {
        test.CreateAndProcessBlock({}, GetScriptForRawPubKey(test.coinbaseKey.GetPubKey()), masternodesID);
    }
...
//    QCOMPARE(transactionTableModel->rowCount({}), 105); // TODO: (temp) !!! qt
...
//    BumpFee(transactionView, txid2, true /* expect disabled */, "already bumped" /* expected error */, false /* cancel */);
    ...
    QString balanceComparison = DefiUnits::formatWithUnit(unit, balance, false, DefiUnits::separatorAlways);
    ...
    // Amount input
    DefiAmountField* amountInput = receiveCoinsDialog.findChild<DefiAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QVERIFY(paymentTextList.at(1).indexOf(QString("URI: defi:")) != -1);
