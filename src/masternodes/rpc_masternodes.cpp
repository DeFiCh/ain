...
UniValue mnToJSON(CCustomCSView& view, uint256 const & nodeId, CMasternode const& node, bool verbose, const std::set<std::pair<CKeyID, uint256>>& mnIds, const CWallet* pwallet)
{
    ...
    if (!verbose) {
        ...
        CTxDestination ownerDest = FromOrDefaultKeyIDToDestination(node.ownerAuthAddress, TxDestTypeToKeyType(node.ownerType), KeyType::MNOwnerKeyType);
        obj.pushKV("ownerAuthAddress", EncodeDestination(ownerDest));
        CTxDestination operatorDest = FromOrDefaultKeyIDToDestination(node.operatorAuthAddress, TxDestTypeToKeyType(node.operatorType), KeyType::MNOperatorKeyType);
        obj.pushKV("operatorAuthAddress", EncodeDestination(operatorDest));
        if (node.rewardAddressType != 0) {
            obj.pushKV("rewardAddress", EncodeDestination(FromOrDefaultKeyIDToDestination(node.rewardAddress, TxDestTypeToKeyType(node.rewardAddressType), KeyType::MNRewardKeyType)));
        }
        else {
