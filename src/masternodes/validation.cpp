...
static void ProcessGovEvents(const CBlockIndex* pindex, CCustomCSView& cache, const CChainParams& chainparams, const CScopedQueueID &evmQueueId) {
    if (pindex->nHeight < chainparams.GetConsensus().DF11FortCanningHeight) {
        return;
    }
...
static Res ProcessEVMQueue(const CBlock &block, const CBlockIndex *pindex, CCustomCSView &cache, const CChainParams& chainparams, const CScopedQueueID &evmQueueId) {
    CKeyID minter;
...
Res ProcessDeFiEventFallible(const CBlock &block, const CBlockIndex *pindex, CCustomCSView &mnview, const CChainParams& chainparams, const CScopedQueueID &evmQueueId) {
...
void ProcessDeFiEvent(const CBlock &block, const CBlockIndex* pindex, CCustomCSView& mnview, const CCoinsViewCache& view, const CChainParams& chainparams, const CreationTxs &creationTxs, const CScopedQueueID &evmQueueId) {
    CCustomCSView cache(mnview);
