...
UniValue evmtx(const JSONRPCRequest& request) {
    ...
    const auto signedTx = evm_try_create_and_sign_tx(result, CreateTransactionContext{chainID, nonce.ToArrayReversed(), gasPrice.ToArrayReversed(), gasLimit.ToArrayReversed(), to, value.ToArrayReversed(), input, privKey});
    if (!result.ok) {
