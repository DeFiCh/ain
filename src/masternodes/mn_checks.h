...
Res ApplyCustomTx(CCustomCSView& mnview, CFutureSwapView &futureSwapView, CUndosView& undosView, const CCoinsViewCache& coins, const CTransaction& tx, const Consensus::Params& consensus, uint32_t height, uint64_t time = 0, uint32_t txn = 0, CHistoryWriters* writers = nullptr) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
Res CustomTxVisit(CCustomCSView& mnview, CFutureBaseView &futureSwapView, const CCoinsViewCache& coins, const CTransaction& tx, uint32_t height, const Consensus::Params& consensus, const CCustomTxMessage& txMessage, uint64_t time = 0, uint32_t txn = 0) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
