...
namespace {
    ...
    UniValue VaultToJSON(const CVaultId& vaultId, const CVaultData& vault) {
        ...
        result.pushKV("collateralValueUSD", collValue);
        result.pushKV("loanValueUSD", loanValue);
        result.pushKV("interestValueUSD", interestValue);
        result.pushKV("collateralRatio", (int)ratio);
        return result;
    }
}
