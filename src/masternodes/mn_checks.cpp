...
class CCustomMetadataParseVisitor
{
    ...
    Res isPostGrandCentralFork() const {
        Require(static_cast<int>(height) >= consensus.GrandCentralHeight, "called before GrandCentral height");
        return Res::Ok();
    }
...
public:
    ...
    Res operator()(const CPoolSwapMessage& obj) const {
        // check auth
        Require(HasAuth(obj.from), "tx must have at least one input from account owner");
    ...
    Res operator()(const CPoolSwapMessageV2& obj) const {
        // check auth
        Require(HasAuth(obj.swapInfo.from), "tx must have at least one input from account owner");
    ...
    Res operator()(const CAnyAccountsToAccountsMessage& obj) const {
        // check auth
        for (const auto& kv : obj.from) {
            Require(HasAuth(kv.first), "tx must have at least one input from account owner");
        }
    ...
    Res operator()(const CGovernanceMessage& obj) const {
        ...
        for(const auto& gov : obj.govs) {
            ...
            if (var->GetName() == "ATTRIBUTES") {
                ...
                Require(var->Validate(mnview), "%s: %s", var->GetName(), res.msg);
                ...
                Require(var->Apply(mnview, height), "%s: %s", var->GetName(), res.msg);
            }

            Require(mnview.SetVariable(*var), "%s: %s", var->GetName(), res.msg);
        }
        return Res::Ok();
    }
    ...
    Res operator()(const CSetOracleDataMessage& obj) const {
        ...
        Require(HasAuth(oracle.val->oracleAddress), "tx must have at least one input from account owner");
            ...
            if (auto collaterals = mnview.GetVaultCollaterals(obj.vaultId))
            {
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    if (static_cast<int>(height) < consensus.FortCanningHillHeight) {
                        Require(totalCollaterals >= collateralsLoans.val->totalCollaterals / 2, "At least 50%% of the collateral must be in DFI");
                    } else {
    ...
    Res operator()(const CLoanTakeLoanMessage& obj) const {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (static_cast<int>(height) < consensus.FortCanningHillHeight) {
                Require(totalCollaterals >= collateralsLoans.val->totalCollaterals / 2, "At least 50%% of the collateral must be in DFI when taking a loan.");
            } else {
                if (arith_uint256(totalCollaterals) * 100 < arith_uint256(collateralsLoans.val->totalLoans) * scheme->ratio / 2)
