...
static constexpr std::string_view ERR_STRING_MIN_COLLATERAL_DFI_PCT =
        "At least 50%% of the minimum required collateral must be in DFI";
static constexpr std::string_view ERR_STRING_MIN_COLLATERAL_DFI_DUSD_PCT =
...
Res CCustomTxVisitor::CheckNextCollateralRatio(const CVaultId& vaultId, const CLoanSchemeData& scheme, const CBalances& collaterals, const bool hasDUSDLoans) const {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        auto collateralsLoans =
                mnview.GetLoanCollaterals(vaultId, collaterals, height, time, useNextPrice, requireLivePrice);
        Require(collateralsLoans);
