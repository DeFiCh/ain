...
Res CLoansConsensus::PaybackWithCollateral(const CVaultData &vault,
    ...
    if (subInterest > collateralDUSD) {
        ...
        res = mnview.SubVaultCollateral(vaultId, {dUsdToken->first, subCollateralAmount});
        if (!res)
            return res;

        res = mnview.DecreaseInterest(height, vaultId, vault.schemeId, dUsdToken->first, 0, subCollateralAmount);
        if (!res)
            return res;
        ...
        if (subLoanAmount > 0) {
            ...
            res = mnview.SubLoanToken(vaultId, {dUsdToken->first, subLoanAmount});
            if (!res)
                return res;
        }
    ...
    auto vaultAssets = mnview.GetVaultAssets(vaultId, *collaterals, height, time);
    if (!vaultAssets)
    ...
    if (!IsVaultPriceValid(mnview, vaultId, height)) return DeFiErrors::VaultInvalidPrice();
