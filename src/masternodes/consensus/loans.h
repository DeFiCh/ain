...
class CLoansConsensus : public CCustomTxVisitor {
    bool IsPaybackWithCollateral(const std::map<DCT_ID, CBalances> &loans) const;
    bool IsTokensMigratedToGovVar() const;
    Res PaybackWithCollateral(const CVaultData &vault, const CVaultId &vaultId, uint32_t height, uint64_t time) const;
public:
