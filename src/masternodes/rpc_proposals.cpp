...
UniValue votegovbatch(const JSONRPCRequest &request) {
    ...
    RPCHelpMan{
            ...
            "\nVote for community proposal with multiple masternodes" + HelpRequiringPassphrase(pwallet) + "\n",
            {
                    ...
                    {"sleepTime", RPCArg::Type::NUM, RPCArg::Optional::OMITTED, "No. of milliseconds to wait between each vote. A small wait time is required for large number of nodes to prevent the node from being too busy to broadcast TXs (default: 500)"}
            },
            RPCResult{"\"hash\"                  (string) The hex-encoded hash of broadcasted transaction\n"},
