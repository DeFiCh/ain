...
UniValue ATTRIBUTES::ExportFiltered(GovVarsFilter filter, const std::string &prefix) const {
    ...
    for (const auto& attribute : attributes) {
        ...
        try {
            ...
            } else if (const auto amount = boost::get<CAmount>(&attribute.second)) {
            ...
            } else if (const auto balances = boost::get<CDexBalances>(&attribute.second)) {
            ...
            } else if (const auto result = boost::get<CFeeDir>(&attribute.second)) {
...
Res ATTRIBUTES::Validate(const CCustomCSView & view) const
{
    ...
    for (const auto& [key, value] : attributes) {
        const auto attrV0 = boost::get<CDataStructureV0>(&key);
        if (!attrV0) {
            return Res::Err("Unsupported version");
        }
        switch (attrV0->type) {
            ...
            case AttributeTypes::Oracles:
                ...
                if (attrV0->typeId == OracleIDs::Splits) {
                    const auto splitMap = boost::get<OracleSplits>(&value);
                    if (!splitMap) {
    ...
    for (const auto& attribute : attributes) {
        ...
        if (attrV0->type == AttributeTypes::Poolpairs) {
            if (attrV0->key == PoolKeys::TokenAFeePCT ||
                ...
                const auto valuePct = boost::get<CAmount>(&attribute.second);
                if (!valuePct) {
            ...
            ||  attrV0->key == TokenKeys::DexOutFeePct) {
                ...
                const auto value = boost::get<bool>(&attribute.second);
                if (!value) {
                    return Res::Err("Unexpected type");
                }
