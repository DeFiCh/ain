...
CPubKey CKey::GetPubKey(const bool forceCompressed) const {
    assert(fValid);
    secp256k1_pubkey pubkey;
    ...
    assert(ret);
    secp256k1_ec_pubkey_serialize(secp256k1_context_sign, (unsigned char*)result.begin(), &clen, &pubkey, fCompressed || forceCompressed ? SECP256K1_EC_COMPRESSED : SECP256K1_EC_UNCOMPRESSED);
...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
    ...
    unsigned char rnd[8];
    std::string str = "DFI key verification\n";
