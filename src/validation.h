...
bool ReplayBlocks(const CChainParams& params, CCoinsView* view, CCustomCSView* cache, CFutureSwapView* futureSwapView);
...
public:
    ...
    DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, CCustomCSView& mnview, CFutureSwapView& futureSwapView, std::vector<CAnchorConfirmMessage> & disconnectedAnchorConfirms);
    bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, CCustomCSView& mnview, CFutureSwapView& futureSwapView,
