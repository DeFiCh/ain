...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, CCustomCSView& mnview, std::vector<CAnchorConfirmMessage> & disconnectedAnchorConfirms)
{
    ...
    if (pindex->nHeight == Params().GetConsensus().DF14FortCanningHillHeight) {
    ...
    if (pindex->nHeight == Params().GetConsensus().DF18FortCanningGreatWorldHeight) {
        auto time = GetTimeMillis();
        LogPrintf("Interest rate reverting ...\n");
    ...
    if (!fIsFakeNet) {
        ...
        if (pindex->nHeight >= Params().GetConsensus().DF10EunosPayaHeight) {
...
Res ApplyGeneralCoinbaseTx(CCustomCSView & mnview, CTransaction const & tx, int height, CAmount nFees, const Consensus::Params& consensus)
{
    ...
    for (const auto& [accountType, accountVal] : consensus.newNonUTXOSubsidies) {
        ...
        if ((height < consensus.DF11FortCanningHeight && accountType == CommunityAccountType::Loan) ||
            (height < consensus.DF20GrandCentralHeight && accountType == CommunityAccountType::Options)) {
                  ...
                  CCoinsViewCache& view, CCustomCSView& mnview, const CChainParams& chainparams, bool & rewardedAnchors, std::array<uint8_t, 20>& beneficiary, bool fJustCheck, const int64_t evmQueueId)
{
    ...
    if (pindex->nHeight >= chainparams.GetConsensus().DF8EunosHeight
    && pindex->nHeight < chainparams.GetConsensus().DF9EunosKampungHeight) {
        bool mutated;
    ...
    if (!fIsFakeNet) {
        ...
        } else if (pindex->nHeight >= Params().GetConsensus().DF7DakotaCrescentHeight) {
    ...
    if (it != checkpoints.begin()) {
        ...
        if (it->first > chainparams.GetConsensus().DF14FortCanningHillHeight) {
