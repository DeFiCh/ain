...
UniValue mnToJSON(CCustomCSView &view,
    ...
    if (!verbose) {
        ...
        const auto timelock = view.GetTimelock(nodeId, node, currentHeight);
...
UniValue resignmasternode(const JSONRPCRequest &request) {
...

    {
        ...
        ownerDest = FromOrDefaultKeyIDToDestination(
...
UniValue updatemasternode(const JSONRPCRequest &request) {
    ...
    if (!metaObj["rewardAddress"].isNull()) {
        ...
        if (!rewardAddress.empty()) {
            ...
            if (rewardDest.index() != PKHashType && rewardDest.index() != ScriptHashType &&
                rewardDest.index() != WitV0KeyHashType) {
                throw JSONRPCError(
                    RPC_INVALID_PARAMETER,
                    "rewardAddress (" + rewardAddress + ") does not refer to a P2SH, P2PKH or P2WPKH address");
            }
        }
    }
    ...
    if (!metaObj["ownerAddress"].isNull()) {
        if (const auto node = view->GetMasternode(nodeId)) {
...
UniValue getmasternode(const JSONRPCRequest &request) {
    ...
    auto hash = view->GetMasternodeIdByOperator(*keyId);
    if (hash) {
    ...
    hash = view->GetMasternodeIdByOwner(*keyId);
    if (hash) {
        if (auto res = printMasternode(*hash)) {
            return *res;
        }
...
UniValue getmasternodeblocks(const JSONRPCRequest &request) {
    ...
    auto masternodeBlocks = [&](const uint256 &masternodeID, int blockHeight) {
        ...
        if (auto tip = ::ChainActive()[blockHeight]; tip) {
    ...
    view->ForEachSubNode([&](const SubNodeBlockTimeKey &key,
                             CLazySerialize<int64_t>) { return masternodeBlocks(key.masternodeID, key.blockHeight); },
                         SubNodeBlockTimeKey{mn_id, 0, std::numeric_limits<uint32_t>::max()});

    view->ForEachMinterNode(
