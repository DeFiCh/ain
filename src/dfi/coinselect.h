...
#ifndef DEFI_DFI_COINSELECT_H
#define DEFI_DFI_COINSELECT_H
...
public:
    ...
    static void SetupArgs(ArgsManager &args) {
        args.AddArg(ARG_STR_WALLET_FAST_SELECT,
                    strprintf("Faster coin select - Enables walletcoinoptskipsolvable and walletcoinopteagerselect. "
                              "This ends up in faster selection but has the disadvantage of not being able to pick "
                              "complex input scripts (default: %u)",
                              DEFAULT_COIN_SELECT_FAST_SELECT),
                    ArgsManager::ALLOW_ANY,
                    OptionsCategory::OPTIONS);
        args.AddArg(ARG_STR_WALLET_COIN_OPT_SKIP_SOLVABLE,
                    strprintf("Coin select option: Skips IsSolvable signable UTXO check (default: %u)",
                              DEFAULT_COIN_SELECT_SKIP_SOLVABLE),
                    ArgsManager::ALLOW_ANY,
                    OptionsCategory::OPTIONS);
        args.AddArg(ARG_STR_WALLET_COIN_OPT_EAGER_SELECT,
                    strprintf("Coin select option: Take fast path and eagerly exit on match even without having "
                              "through the entire set (default: %u)",
                              DEFAULT_COIN_SELECT_EAGER_SELECT),
                    ArgsManager::ALLOW_ANY,
                    OptionsCategory::OPTIONS);
    }
