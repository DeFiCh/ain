...
#ifndef DEFI_DFI_ERRORS_H
#define DEFI_DFI_ERRORS_H
...
public:
    ...
    static Res ICXDisabled() { return Res::Err("Cannot create tx, ICX is not enabled"); }
    ...
    static Res ICXOrderInvalidType() { return Res::Err("invalid order type!"); }

    static Res ICXOrderAmountFromZero() { return Res::Err("order amountFrom must be greater than 0!"); }

    static Res ICXOrderOrderPriceZero() { return Res::Err("order price must be greater than 0!"); }

    static Res ICXOrderAmountsNotEqual() { return Res::Err("order amountToFill does not equal to amountFrom!"); }

    static Res ICXOrderExiryTooSmall(const uint32_t amount) {
        return Res::Err("order expiry must be greater than %d!", amount);
    }

    static Res ICXOfferAmountZero() { return Res::Err("offer amount must be greater than 0!"); }

    static Res ICXSubmitAmountZero() { return Res::Err("Invalid amount, must be greater than 0!"); }

    static Res ICXSubmitInvalidHash() { return Res::Err("Invalid hash, htlc hash is empty and it must be set!"); }

    static Res ICXSubmitTimeout() { return Res::Err("Invalid timeout, must be greater than 0!"); }
    ...
    static Res VaultNoCollateral(const std::string &vaultId = "") {
        return vaultId.empty() ? Res::Err("Vault has no collaterals")
                               : Res::Err("Vault with id %s has no collaterals", vaultId);
    }

    static Res VaultNoDUSDCollateral() { return Res::Err("Vault does not have any DUSD collaterals"); }
    ...
    static Res VaultNeedCollateral() { return Res::Err("Vault cannot have loans without collaterals"); }
    ...
    static Res VaultUnderLiquidation() { return Res::Err("Vault is under liquidation"); }

    static Res LoanNoPaybackOnLiquidation() { return Res::Err("Cannot payback loan on vault under liquidation"); }

    static Res TXMissingInput() { return Res::Err("tx must have at least one input from token owner"); }
    ...
    static Res LoanPaybackDisabled(const std::string &token) {
        return token.empty() ? Res::Err("Payback is not currently active")
                             : Res::Err("Payback of loan via %s token is not currently active", token);
    }
    ...
    static Res GovVarVerifyInt() { return Res::Err("Value must be an integer"); }
    ...
    static Res GovVarVerifyFeeDirection() { return Res::Err("Fee direction value must be both, in or out"); }

    static Res GovVarVariableLength() { return Res::Err("Identifier exceeds maximum length (128)"); }

    static Res GovVarVariableNoVersion() { return Res::Err("Empty version"); }

    static Res GovVarUnsupportedVersion() { return Res::Err("Unsupported version"); }
    ...
    static Res GovVarImportObjectExpected() { return Res::Err("Object of values expected"); }

    static Res GovVarValidateFortCanningHill() { return Res::Err("Cannot be set before FortCanningHill"); }

    static Res GovVarValidateFortCanningEpilogue() { return Res::Err("Cannot be set before FortCanningEpilogue"); }

    static Res GovVarValidateFortCanningRoad() { return Res::Err("Cannot be set before FortCanningRoad"); }

    static Res GovVarValidateFortCanningCrunch() { return Res::Err("Cannot be set before FortCanningCrunch"); }
    ...
    static Res GovVarValidateSplitPool() { return Res::Err("Pool tokens cannot be split"); }

    static Res GovVarValidateSplitDAT() { return Res::Err("Only DATs can be split"); }

    static Res GovVarApplyUnexpectedType() { return Res::Err("Unexpected type"); }

    static Res GovVarApplyInvalidPool(const uint32_t pool) { return Res::Err("No such pool (%d)", pool); }

    static Res GovVarApplyInvalidFactor(const CAmount ratio) {
        return Res::Err("Factor cannot be more than or equal to the lowest scheme rate of %s",
                        GetDecimalString(ratio * CENT));
    }
    ...
    static Res GovVarApplyLockFail() { return Res::Err("Failed to create Gov var for lock"); }

    static Res GovVarApplyCastFail() { return Res::Err("Failed to cast Gov var to ATTRIBUTES"); }

    static Res GovVarEraseLive() { return Res::Err("Live attribute cannot be deleted"); }

    static Res GovVarEraseNonExist(const uint32_t type) { return Res::Err("Attribute {%d} not exists", type); }

    static Res GovVarFailedWrite() { return Res::Err("Cannot write to DB"); }
    ...
    static Res InvalidAuth() { return Res::Err("tx must have at least one input from account owner"); }

    static Res TransferDomainNotEnabled() { return Res::Err("Cannot create tx, transfer domain is not enabled"); }

    static Res TransferDomainEVMNotEnabled() { return Res::Err("Cannot create tx, EVM is not enabled"); }

    static Res TransferDomainSameDomain() { return Res::Err("Cannot transfer inside same domain"); }

    static Res TransferDomainInvalidDomain() { return Res::Err("Cannot transfer inside invalid domain specified"); }

    static Res TransferDomainUnequalAmount() { return Res::Err("Source amount must be equal to destination amount"); }
    ...
    static Res TransferDomainInvalidSourceDomain() { return Res::Err("Invalid domain set for \"src\" argument"); }
    ...
    static Res TransferDomainInvalidDestinationDomain() { return Res::Err("Invalid domain set for \"dst\" argument"); }

    static Res TransferDomainUnknownEdge() { return Res::Err("Unknown transfer domain aspect"); }

    static Res TransferDomainInvalid() { return Res::Err("Invalid transfer domain TX"); }
    ...
    static Res SettingEVMAttributeFailure() { return Res::Err("Failed to set EVM attribute"); }

    static Res SettingEVMAttributeFailure(const std::string &str) {
        return Res::Err("Failed to set EVM attribute: %s", str);
    }

    static Res ScriptUnexpected(const CScript &script) { return Res::Err("Unexpected Script: %s", script.GetHex()); }

    static Res DatabaseRWFailure(const std::string &key) { return Res::Err("DB r/w failure: %s", key); }

    static Res DatabaseKeyNotFound(const std::string &key) { return Res::Err("Key not found: %s", key); }
