...
Res CLoanView::CreateLoanCollateralToken(const CLoanSetCollateralTokenImpl &collToken) {
    ...
    if (collToken.factor > COIN) {
        return Res::Err("setCollateralToken factor must be lower or equal than %s!", GetDecimalString(COIN));
    }
...
Res CLoanView::StoreDelayedLoanScheme(const CLoanSchemeMessage &loanScheme) {
    WriteBy<DelayedLoanSchemeKey>(std::pair<std::string, uint64_t>(loanScheme.identifier, loanScheme.updateHeight),
                                  loanScheme);
...
void CLoanView::ForEachDelayedLoanScheme(
...
std::optional<CInterestRateV3> CLoanView::GetInterestRate(const CVaultId &vaultId,
                                                          const DCT_ID id,
...
CInterestAmount TotalInterestCalculation(const CInterestRateV3 &rate, const uint32_t height) {
    ...
    if (heightDiff != 0 && totalInterest / heightDiff != interestAmount) {
        LogPrintf(
            "WARNING: Overflow detected. This will soon be saturated. (height=%d, height-diff=%d" /* Continued */
            "amount=%s, interest=%s)\n",
            height,
            heightDiff,
            GetInterestPerBlockHighPrecisionString(rate.interestPerBlock),
    ...
    LogPrint(BCLog::LOAN,
             "%s(): CInterestRate{.height=%d, .perBlock=%d, .toHeight=%d}, height %d - totalInterest %d\n",
             __func__,
             rate.height,
             rate.interestPerBlock.negative ? -InterestPerBlock(rate, height) : InterestPerBlock(rate, height),
             rate.interestToHeight.negative ? -FloorInterest(rate.interestToHeight.amount)
                                            : CeilInterest(rate.interestToHeight.amount, height),
             height,
...
void CLoanView::WriteInterestRate(const std::pair<CVaultId, DCT_ID> &pair,
...
Res CLoanView::IncreaseInterest(const uint32_t height,
...
Res CLoanView::DecreaseInterest(const uint32_t height,
                                const CVaultId &vaultId,
                                const std::string &loanSchemeID,
                                const DCT_ID id,
...
void CLoanView::ResetInterest(const uint32_t height,
                              const CVaultId &vaultId,
                              const std::string &loanSchemeID,
                              const DCT_ID id) {
    const auto scheme = GetLoanScheme(loanSchemeID);
...
void CLoanView::ForEachVaultInterest(std::function<bool(const CVaultId &, DCT_ID, CInterestRate)> callback,
                                     const CVaultId &vaultId,
                                     DCT_ID id) {
    ForEach<LoanInterestByVault, std::pair<CVaultId, DCT_ID>, CInterestRate>(
        [&](const std::pair<CVaultId, DCT_ID> &pair, CInterestRate rate) {
            return callback(pair.first, pair.second, rate);
        },
        std::make_pair(vaultId, id));
}

void CLoanView::ForEachVaultInterestV2(std::function<bool(const CVaultId &, DCT_ID, CInterestRateV2)> callback,
                                       const CVaultId &vaultId,
                                       DCT_ID id) {
    ForEach<LoanInterestV2ByVault, std::pair<CVaultId, DCT_ID>, CInterestRateV2>(
        [&](const std::pair<CVaultId, DCT_ID> &pair, CInterestRateV2 rate) {
            return callback(pair.first, pair.second, rate);
        },
        std::make_pair(vaultId, id));
}

void CLoanView::ForEachVaultInterestV3(std::function<bool(const CVaultId &, DCT_ID, CInterestRateV3)> callback,
                                       const CVaultId &vaultId,
                                       DCT_ID id) {
    ForEach<LoanInterestV3ByVault, std::pair<CVaultId, DCT_ID>, CInterestRateV3>(
        [&](const std::pair<CVaultId, DCT_ID> &pair, CInterestRateV3 rate) {
            return callback(pair.first, pair.second, rate);
        },
        std::make_pair(vaultId, id));
}
...
void CLoanView::RevertInterestRateToV1() {
    ...
    ForEach<LoanInterestV2ByVault, std::pair<CVaultId, DCT_ID>, CInterestRateV2>(
        [&](const std::pair<CVaultId, DCT_ID> &pair, CLazySerialize<CInterestRateV2>) {
...
void CLoanView::RevertInterestRateToV2() {
    ...
    ForEach<LoanInterestV3ByVault, std::pair<CVaultId, DCT_ID>, CInterestRateV3>(
        [&](const std::pair<CVaultId, DCT_ID> &pair, const CInterestRateV3 &) {
            pairs.emplace_back(pair);
            return true;
        });
...
std::optional<std::string> TryGetInterestPerBlockHighPrecisionString(const CInterestAmount &value) {
    struct HighPrecisionInterestValue {
        ...
        int64 GetInterestPerBlockSat() const { return int64(value / HIGH_PRECISION_SCALER); }

        int64 GetInterestPerBlockSubSat() const { return int64(value % HIGH_PRECISION_SCALER); }

        int64 GetInterestPerBlockMagnitude() const { return int64(value / HIGH_PRECISION_SCALER / COIN); }
