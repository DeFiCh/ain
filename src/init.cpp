...
#if HAVE_DECL_FORK
    gArgs.AddArg("-daemon", strprintf("Wait for initialization to be finished before exiting. This implies -daemonnowait (default: %d)", DEFAULT_DAEMON), ArgsManager::ALLOW_BOOL, OptionsCategory::OPTIONS);
    gArgs.AddArg("-daemonnowait", strprintf("Run in the background as a daemon and accept commands (default: %d)", DEFAULT_DAEMONNOWAIT), ArgsManager::ALLOW_BOOL, OptionsCategory::OPTIONS);
#else
