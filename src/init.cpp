...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-mint4address", strprintf("Generate coins for selected address instead of masternode's owner"), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    ...
    if(gArgs.GetBoolArg("-gen", DEFAULT_GENERATE)) {
        ...
        if (myIDs)
        {
            pos::ThreadStaker::Args stakerParams{};
            {
                ...
                CTxDestination const mint4address = DecodeDestination(gArgs.GetArg("-mint4address", ""), Params());
                if (IsValidDestination(mint4address))
                    destination = mint4address;
