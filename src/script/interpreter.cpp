...
{
    assert(sigversion == SigVersion::BASE || sigversion == SigVersion::WITNESS_V0 || sigversion == SigVersion::WITNESS_V16);
...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror)
{
    ...
    assert(sigversion == SigVersion::BASE || sigversion == SigVersion::WITNESS_V0 || sigversion == SigVersion::WITNESS_V16 || sigversion == SigVersion::TAPSCRIPT);
    ...
    if ((sigversion == SigVersion::BASE || sigversion == SigVersion::WITNESS_V0 || sigversion == SigVersion::WITNESS_V16) && script.size() > MAX_SCRIPT_SIZE)
    ...
    try
    {
        for (; pc < pend; ++opcode_pos) {
            ...
            if (sigversion == SigVersion::BASE || sigversion == SigVersion::WITNESS_V0 || sigversion == SigVersion::WITNESS_V16) {
            ...
            switch (opcode)
            {
                ...
                case OP_NOTIF:
                {
                    ...
                    if (fExec)
                    {
                        ...
                        if ((sigversion == SigVersion::WITNESS_V0 || sigversion == SigVersion::WITNESS_V16) && (flags & SCRIPT_VERIFY_MINIMALIF)) {
                            if (vch.size() > 1)
...
{
    CHashWriter ss(SER_GETHASH, txTo.nVersion < CTransaction::TOKENS_MIN_VERSION ? SERIALIZE_TRANSACTION_NO_TOKENS : 0);
...
uint256 SignatureHash(const CScript& scriptCode, const T& txTo, unsigned int nIn, int nHashType, const CAmount& amount, SigVersion sigversion, const PrecomputedTransactionData* cache)
{
    ...
    if (sigversion == SigVersion::WITNESS_V0 || sigversion == SigVersion::WITNESS_V16) {
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    } else if (witversion == 16) {
        if (program.size() == WITNESS_V16_ETHHASH_SIZE) {
            // Special case for pay-to-pubkeyhash; signature + pubkey in witness
            if (stack.size() != 2) {
                return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH); // 2 items in witness
            }
            exec_script << OP_DUP << OP_KECCAK << program << OP_EQUALVERIFY << OP_CHECKSIG;
            return ExecuteWitnessScript(stack, exec_script, flags, SigVersion::WITNESS_V16, checker, execdata, serror);
        } else {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_WRONG_LENGTH);
        }
    } else {
