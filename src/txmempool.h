...
public:
    bool operator()(const EvmAddressWithNonce &a, const EvmAddressWithNonce &b) const { return a < b; }
};
...
private:
    ...
    uint64_t totalTxSize;  //!< sum of all mempool tx's virtual sizes. Differs from serialized tx size since witness
...
public:
    ...
    typedef boost::multi_index_container<
        ...
        boost::multi_index::indexed_by<
            // sorted by txid
            boost::multi_index::hashed_unique<boost::multi_index::tag<txid_tag>, mempoolentry_txid, SaltedTxidHasher>,
            // sorted by fee rate
            ...
            boost::multi_index::ordered_non_unique<
                ...
                boost::multi_index::
                    const_mem_fun<CTxMemPoolEntry, const EvmAddressWithNonce &, &CTxMemPoolEntry::GetEVMAddrAndNonce>,
    ...
    std::vector<std::pair<uint256, txiter>> vTxHashes
        GUARDED_BY(cs);  //!< All tx witness hashes/entries in mapTx, in random order
...
private:
    ...
    static void AddToStaged(setEntries &staged,
                            std::vector<CTransactionRef> &vtx,
                            const CTransactionRef tx,
                            std::map<uint256, CTxMemPool::txiter> &mempoolIterMap);
...
public:
     ...
     * check does nothing.
     */
    void xcheck(const CCoinsViewCache *pcoins, CCustomCSView *mnview, const CChainParams &chainparams) const;
    ...
    void addUnchecked(const CTxMemPoolEntry &entry,
                      ...
                      const std::optional<EvmAddressData> evmSender = std::nullopt)
     ...
     */
    void RemoveStaged(const setEntries &stage, bool updateDescendants, MemPoolRemovalReason reason)
        EXCLUSIVE_LOCKS_REQUIRED(cs);
    ...
    bool checkAddressNonceAndFee(const CTxMemPoolEntry &pendingEntry,
                                 const uint64_t &entryFee,
                                 const EvmAddressData &txSender,
                                 bool &senderLimitFlag);
