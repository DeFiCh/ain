...
use sputnikvm::{
    AccountChange, AccountCommitment, Context, HeaderParams, Log, Patch, SeqTransactionVM,
    VMStatus, ValidTransaction, VM,
};
use sputnikvm_network_classic::{EIP150Patch, EIP160Patch, FrontierPatch, HomesteadPatch};
use sputnikvm_network_classic::{
    MainnetEIP150Patch, MainnetEIP160Patch, MainnetFrontierPatch, MainnetHomesteadPatch,
};
...
impl AccountPatch for CustomAccountPatch {
    fn initial_nonce() -> U256 {
        U256::from(unsafe { CUSTOM_INITIAL_NONCE.unwrap() })
    }
    fn initial_create_nonce() -> U256 {
        Self::initial_nonce()
    }
...
pub unsafe extern "C" fn sputnikvm_set_custom_initial_nonce(v: c_u256) {
    let v: U256 = v.into();
    unsafe { CUSTOM_INITIAL_NONCE = Some(v) }
}

fn sputnikvm_new<P: Patch + 'static>(
    transaction: c_transaction,
    header: c_header_params,
) -> *mut Box<VM> {
...
#[no_mangle]
pub extern "C" fn sputnikvm_free(vm: *mut Box<VM>) {
    if vm.is_null() {
        return;
    }
    unsafe {
...
#[no_mangle]
pub extern "C" fn sputnikvm_fire(vm: *mut Box<VM>) -> c_require {
...
#[no_mangle]
pub extern "C" fn sputnikvm_commit_nonexist(vm: *mut Box<VM>, address: c_address) -> bool {
    let mut vm_box = unsafe { Box::from_raw(vm) };
    let ret;
    {
        let vm: &mut VM = vm_box.deref_mut().deref_mut();
...
#[no_mangle]
pub extern "C" fn sputnikvm_logs_len(vm: *mut Box<VM>) -> c_uint {
    let mut vm_box = unsafe { Box::from_raw(vm) };
    let ret;
    {
        let vm: &mut VM = vm_box.deref_mut().deref_mut();
...
#[no_mangle]
pub extern "C" fn sputnikvm_logs_copy_info(vm: *mut Box<VM>, log: *mut c_log, log_len: c_uint) {
    let mut vm_box = unsafe { Box::from_raw(vm) };
    {
        let vm: &mut VM = vm_box.deref_mut().deref_mut();
        let logs = vm.logs();
...
#[no_mangle]
pub extern "C" fn sputnikvm_account_changes_len(vm: *mut Box<VM>) -> c_uint {
    let mut vm_box = unsafe { Box::from_raw(vm) };
    let ret;
    {
        let vm: &mut VM = vm_box.deref_mut().deref_mut();
        ...
        for (i, account) in accounts.enumerate() {
            if i < w.len() {
                w[i] = match account {
                    ...
                    &AccountChange::Nonexist(address) => c_account_change {
                        typ: c_account_change_type::removed,
                        value: c_account_change_value {
                            removed: address.into(),
                        },
                    },
                    &AccountChange::IncreaseBalance(address, amount) => c_account_change {
...
pub extern "C" fn sputnikvm_account_changes_copy_storage(
    vm: *mut Box<VM>,
    address: c_address,
    w: *mut c_account_change_storage,
    wl: c_uint,
) -> bool {
    let mut vm_box = unsafe { Box::from_raw(vm) };
    let mut ret = false;
...
#[no_mangle]
pub extern "C" fn sputnikvm_available_gas(vm: *mut Box<VM>) -> c_gas {
    let mut vm_box = unsafe { Box::from_raw(vm) };
    let ret = {
        let vm: &mut VM = vm_box.deref_mut().deref_mut();
        vm.available_gas().into()
    };
    Box::into_raw(vm_box);
    ret
}
