...
mod ffi {
    extern "Rust" {
        ...
        fn evm_finalise(context: u64, update_state: bool) -> Result<Vec<Vec<u8>>>;
...
fn evm_finalise(context: u64, update_state: bool) -> Result<Vec<Vec<u8>>, Box<dyn Error>> {
    let block_and_failed_txs: Vec<Vec<u8>> = Vec::new();
