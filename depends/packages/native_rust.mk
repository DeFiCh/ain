...
$(package)_rust_wasi_version=1.57.0
$(package)_rust_wasi_download_path=https://static.rust-lang.org/dist
$(package)_rust_wasi_download_file=rust-std-$($(package)_rust_wasi_version)-wasm32-wasi.tar.gz
$(package)_rust_wasi_file_name=rust-std-$($(package)_rust_wasi_version)-wasm32-wasi.tar.gz
$(package)_rust_wasi_sha256_hash=391d814379cf117e927e5deb17bf44e964adee4f5f7208eaee1ddc8927da1fbc

$(package)_extra_sources = $($(package)_rust_wasi_file_name)
...
$(call fetch_file,$(package),$($(package)_rust_wasi_download_path),$($(package)_rust_wasi_download_file),$($(package)_rust_wasi_file_name),$($(package)_rust_wasi_sha256_hash))
endef
...
define $(package)_extract_cmds
  ...
  echo "$($(package)_rust_wasi_sha256_hash)  $($(package)_source_dir)/$($(package)_rust_wasi_file_name)" >> $($(package)_extract_dir)/.$($(package)_file_name).hash && \
  $(build_SHA256SUM) -c $($(package)_extract_dir)/.$($(package)_file_name).hash && \
  mkdir $(canonical_host) && \
  tar --strip-components=1 -xf $($(package)_source) -C $(canonical_host) && \
  mkdir rust_wasi && \
  tar --strip-components=1 -xf $($(package)_source_dir)/$($(package)_rust_wasi_file_name) -C rust_wasi && \
  mkdir buildos && \
...
define $(package)_stage_cmds
  ...
  ../rust_wasi/install.sh --without=rust-docs --destdir=$($(package)_staging_dir) --prefix=$(build_prefix) --disable-ldconfig
endef
...
define $(package)_postprocess_cmds
  cp -r $($(package)_staging_dir)/$(build_prefix) "$(build_prefix)/"
endef