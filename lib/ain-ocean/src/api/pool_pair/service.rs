...
async fn get_usd_per_dfi(ctx: &Arc<AppContext>) -> Result<Decimal> {
  ...
  fn add(p: PoolPairInfo, mut total_usd: Decimal, mut total_dfi: Decimal) -> Result<(Decimal, Decimal)> {
    ...
    if p.id_token_a == "0" {
        total_usd = total_usd.checked_add(reserve_b).ok_or_else(|| Error::OverflowError)?;
        total_dfi = total_dfi.checked_add(reserve_a).ok_or_else(|| Error::OverflowError)?;
    } else if p.id_token_b == "0" {
        total_usd = total_usd.checked_add(reserve_a).ok_or_else(|| Error::OverflowError)?;
        total_dfi = total_dfi.checked_add(reserve_b).ok_or_else(|| Error::OverflowError)?;
    }
  ...
  if !total_usd.is_zero() {
      let res = total_usd.checked_div(total_dfi).ok_or_else(|| Error::UnderflowError)?;
      return Ok(res)
  };
...
pub async fn get_total_liquidity_usd(ctx: &Arc<AppContext>, p: &PoolPairInfo) -> Result<Decimal> {
  ...
  if ["DUSD", "USDT", "USDC"].contains(&a) {
      return reserve_a.checked_mul(dec!(2)).ok_or_else(|| Error::OverflowError)
  };

  if ["DUSD", "USDT", "USDC"].contains(&b) {
      return reserve_b.checked_mul(dec!(2)).ok_or_else(|| Error::OverflowError)
  };
...
async fn get_yearly_custom_reward_usd(ctx: &Arc<AppContext>, p: &PoolPairInfo) -> Result<Decimal> {
  ...
  let rewards = if let Some(custom_rewards) = &p.custom_rewards {
    let total = custom_rewards.iter().fold(dec!(0), | accumulate: Decimal, account: &String | {
      ...
      let yearly = Decimal::from_str(amount).unwrap()
        .checked_mul(dec!(2880)).unwrap() // 60 * 60 * 24 / 30, 30 seconds = 1 block
        .checked_mul(dec!(365)).unwrap() // 1 year
        .checked_mul(dfi_price_usdt).unwrap();
...
async fn get_loan_token_splits(ctx: &Arc<AppContext>) -> Result<Option<serde_json::Value>> {
  let splits = get_gov_cached(ctx, "LP_LOAN_TOKEN_SPLITS".to_string()).await?.get("LP_LOAN_TOKEN_SPLITS").cloned();
  Ok(splits)
}
...
async fn get_block_subsidy(eunos_height: u32, height: u32) -> Result<Decimal> {
  let eunos_height = Decimal::from_u32(eunos_height).ok_or_else(|| Error::DecimalConversionError)?;
  ...
  if height >= eunos_height {
    ...
    while reductions >= dec!(0)  {
      let amount = reduction_amount.checked_mul(block_subsidy).ok_or_else(|| Error::OverflowError)?;
      if amount <= dec!(0.00001) {
        return Ok(dec!(0))
      };
      block_subsidy = block_subsidy.checked_sub(amount).ok_or_else(|| Error::UnderflowError)?;
      reductions = reductions.checked_sub(dec!(1)).ok_or_else(|| Error::UnderflowError)?;
    }
  };
...
async fn get_loan_emission(ctx: &Arc<AppContext>) -> Result<Decimal> {
  ...
  let eunos_height = info.softforks.get("eunos").map(|eunos| eunos.height).unwrap_or_default();
...
pub async fn get_apr(ctx: &Arc<AppContext>, id: &String, p: &PoolPairInfo) -> Result<Option<PoolPairAprResponse>> {
