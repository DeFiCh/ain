...
impl TransactionQueueMap {
    ...
    pub fn get_total_gas_used(&self, context_id: u64) -> Option<U256> {
        self.queues
            .read()
            .unwrap()
            .get(&context_id)
...
impl TransactionQueue {
    pub fn new() -> Self {
        Self {
            ...
            total_gas_used: Mutex::new(U256::zero()),
        }
    }
    ...
    pub fn queue_tx(&self, tx: QueueTxWithNativeHash, gas_used: U256) -> Result<(), QueueError> {
...
mod tests {
    ...
    fn test_invalid_nonce_order() -> Result<(), QueueError> {
        ...
        queue.queue_tx((tx1, H256::from_low_u64_be(1).into()), U256::zero())?;
        queue.queue_tx((tx2, H256::from_low_u64_be(2).into()), U256::zero())?;
        // Should fail as nonce 2 is already queued for this sender
        let queued = queue.queue_tx((tx3, H256::from_low_u64_be(3).into()), U256::zero());
    ...
    fn test_invalid_nonce_order_with_transfer_domain() -> Result<(), QueueError> {
        ...
        queue.queue_tx((tx1, H256::from_low_u64_be(1).into()), U256::zero())?;
        queue.queue_tx((tx2, H256::from_low_u64_be(2).into()), U256::zero())?;
        queue.queue_tx((tx3, H256::from_low_u64_be(3).into()), U256::zero())?;
        // Should fail as nonce 2 is already queued for this sender
        let queued = queue.queue_tx((tx4, H256::from_low_u64_be(4).into()), U256::zero());
        assert!(matches!(queued, Err(QueueError::InvalidNonce { .. })));
        Ok(())
    }
    ...
    fn test_valid_nonce_order() -> Result<(), QueueError> {
        ...
        queue.queue_tx((tx2, H256::from_low_u64_be(2).into()), U256::zero())?;
        queue.queue_tx((tx3, H256::from_low_u64_be(3).into()), U256::zero())?;
        queue.queue_tx((tx4, H256::from_low_u64_be(4).into()), U256::zero())?;
        Ok(())
    }
}
