#!/usr/bin/env bash

export LC_ALL=C

set -Eeuo pipefail

setup_vars() {
    # directories and binaries
    WORK_DIR=${WORK_DIR:-"$(pwd)"}
    DEFID_BIN=${DEFID_BIN:-"${WORK_DIR}/build/src/defid"}
    DEFI_CLI_BIN=${DEFI_CLI_BIN:-"${WORK_DIR}/build/src/defi-cli  -rpcuser=defid -rpcpassword=defid"}

    # fixture
    OWNERAUTHADDR="msER9bmJjyEemRpQoS8YYVL21VyZZrSgQ7"
    OWNERAUTHPRIV="cSCmN1tjcR2yR1eaQo9WmjTMR85SjEoNPqMPWGAApQiTLJH8JF7W"
    OPERATORAUTHADDR="mps7BdmwEF2vQ9DREDyNPibqsuSRZ8LuwQ"
    OPERATORAUTHPRIV="cVNTRYV43guugJoDgaiPZESvNtnfnUW19YEjhybihwDbLKjyrZNV"
    ALICE="0x9b8a4af42140d8a4c153a822f02571a1dd037e89"
    # BOB="0x6C34CBb9219d8cAa428835D2073E8ec88BA0a110"
    PRIVKEY_ALICE="af990cc3ba17e776f7f57fcc59942a82846d75833fa17d2ba59ce6858d886e23"
    PRIVKEY_BOB="17b8cb134958b3d8422b6c43b0732fcdb8c713b524df2d45de12f0c7e214ba35"
    CONTRACT_COUNTER=""
    CONTRACT_COUNTERCALLER=""
}


setup_fixtures() {
    # foundation members
    $DEFI_CLI_BIN -regtest importprivkey "$OWNERAUTHPRIV" owner true
    $DEFI_CLI_BIN -regtest importprivkey "$OPERATORAUTHPRIV" operator true

    # push fixtures
    $DEFI_CLI_BIN -regtest importprivkey "$PRIVKEY_ALICE"
    $DEFI_CLI_BIN -regtest importprivkey "$PRIVKEY_BOB"
    $DEFI_CLI_BIN -regtest generatetoaddress 120 "$OWNERAUTHADDR"
    
    sleep 5
    # Continue with the next line of code
    APPO_ADDR=$($DEFI_CLI_BIN -regtest getnewaddress)
	ORACLE_RESPONSE=$($DEFI_CLI_BIN -regtest appointoracle "$APPO_ADDR" '[{"token": "TA", "currency": "USD"}, {"token": "TB", "currency": "USD"}, {"token": "TC", "currency": "USD"}, {"token": "TD", "currency": "USD"}]' 1)
	echo "Oracle Response: ${ORACLE_RESPONSE}"
	$DEFI_CLI_BIN -regtest generatetoaddress 1 "$OWNERAUTHADDR"

    APPO_ADDR_1=$($DEFI_CLI_BIN -regtest getnewaddress)
    ORACLE_RESPONSE_1=$($DEFI_CLI_BIN -regtest appointoracle "$APPO_ADDR_1" '[{"token": "TA", "currency": "USD"}, {"token": "TB", "currency": "USD"}, {"token": "TD", "currency": "USD"}]' 2)
    echo "Oracle Response: ${ORACLE_RESPONSE_1}"
    $DEFI_CLI_BIN -regtest generatetoaddress 1 "$OWNERAUTHADDR"

    APPO_ADDR_2=$($DEFI_CLI_BIN -regtest getnewaddress)
    ORACLE_RESPONSE_2=$($DEFI_CLI_BIN -regtest appointoracle "$APPO_ADDR_2" '[{"token": "TA", "currency": "USD"}, {"token": "TB", "currency": "USD"}, {"token": "TC", "currency": "USD"}]' 0)
    echo "Oracle Response: ${ORACLE_RESPONSE_1}"
    $DEFI_CLI_BIN -regtest generatetoaddress 1 "$OWNERAUTHADDR"


# Setting oracle data based on specific logs and oracle IDs

	timestamp=$(date +%s)
	$DEFI_CLI_BIN -regtest setoracledata "$ORACLE_RESPONSE" $timestamp '[{"tokenAmount": "1.1@TA", "currency": "USD"}, {"tokenAmount": "2.1@TB", "currency": "USD"}, {"tokenAmount": "3.1@TC", "currency": "USD"}, {"tokenAmount": "4.1@TD", "currency": "USD"}]'
	$DEFI_CLI_BIN -regtest generatetoaddress 1 "$OWNERAUTHADDR"
	sleep 1

	timestamp=$(date +%s)
	$DEFI_CLI_BIN -regtest setoracledata "$ORACLE_RESPONSE" $timestamp '[{"tokenAmount": "1@TA", "currency": "USD"}, {"tokenAmount": "2@TB", "currency": "USD"}, {"tokenAmount": "3@TC", "currency": "USD"}]'
	$DEFI_CLI_BIN -regtest generatetoaddress 1 "$OWNERAUTHADDR"
	sleep 1

	timestamp=$(date +%s)
	$DEFI_CLI_BIN -regtest setoracledata "$ORACLE_RESPONSE" $timestamp '[{"tokenAmount": "0.9@TA", "currency": "USD"}, {"tokenAmount": "1.9@TB", "currency": "USD"}]'
	$DEFI_CLI_BIN -regtest generatetoaddress 1 "$OWNERAUTHADDR"
	sleep 1

    timestamp=$(date +%s)
	$DEFI_CLI_BIN -regtest setoracledata "$ORACLE_RESPONSE_1" $timestamp '[{"tokenAmount": "1.5@TA", "currency": "USD"}, {"tokenAmount": "2.5@TB", "currency": "USD"}, {"tokenAmount": "4.5@TD", "currency": "USD"}]'
	$DEFI_CLI_BIN -regtest generatetoaddress 1 "$OWNERAUTHADDR"
  
    timestamp=$(date +%s)
	$DEFI_CLI_BIN -regtest setoracledata "$ORACLE_RESPONSE_1" $timestamp '[{"tokenAmount": "1.5@TA", "currency": "USD"}, {"tokenAmount": "2.5@TB", "currency": "USD"}, {"tokenAmount": "4.5@TD", "currency": "USD"}]'
	$DEFI_CLI_BIN -regtest generatetoaddress 1 "$OWNERAUTHADDR"

	sleep 1
    timestamp=$(date +%s)
	$DEFI_CLI_BIN -regtest setoracledata "$ORACLE_RESPONSE_2" $timestamp '[{"tokenAmount": "1.25@TA", "currency": "USD"}, {"tokenAmount": "2.25@TB", "currency": "USD"}, {"tokenAmount": "4.25@TC", "currency": "USD"}]'
	$DEFI_CLI_BIN -regtest generatetoaddress 1 "$OWNERAUTHADDR"

}

main() {
    setup_vars
    setup_fixtures
}

main "$@"